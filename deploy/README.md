...
| 平台 | 文件 | 怎么用 |
|---|---|---|
| macOS | [`Gotong.command`](Gotong.command) | Finder 里双击。 |
| Linux / 通用 | [`Gotong.sh`](Gotong.sh) | `chmod +x deploy/Gotong.sh && ./deploy/Gotong.sh`，或 `scp` 到任意机器。 |

两个壳是**极薄的便利包装**：按「源码 checkout → 已装 CLI（`gotong start`）→ `npx @gotong/host`」
顺序找到一种起 host 的方式并把终端交给它。它们**不读任何凭证、不写任何 secret**，只负责把
host 拉起来；开浏览器是 host 在监听之后自己做的（`GOTONG_OPEN_BROWSER`，壳默认设成 `always`），
...
$EDITOR .env.local                    # 至少填 GOTONG_TELEGRAM_BOT_TOKEN
...
sudo $EDITOR /etc/gotong.env         # 域名 + master key（从 secret 注入）
bash scripts/cloud-harden.sh /etc/gotong.env   # 暴露前先跑周界自检，红项清零
