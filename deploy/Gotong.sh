#!/usr/bin/env bash
#
# Gotong.sh — generic launcher for Linux / any POSIX shell.
#
...
# actually listening (GOTONG_OPEN_BROWSER; on Linux it uses xdg-open), so there
# is no race and no double-open — this script never opens a URL of its own.
#
# Twin of deploy/Gotong.command (the macOS double-click variant). Keep the two
...
Could not find a way to start Gotong. Install one of:
  ...
  • Node.js (https://nodejs.org), then re-run to use `npx @gotong/host`
