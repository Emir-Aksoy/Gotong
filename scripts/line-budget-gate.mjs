 ...
 */
const BUDGETS = [
  { file: 'packages/host/src/main.ts', max: 2740 }, // the assembly binary — SLIM 抽 heartbeat-engine 后 2941,棘轮 3040→2980;C-M2-M4b 令牌刷新计时器 2980→2986;C-M2-M5a 出站 OAuth 连接器 CRUD surface 接线(factory construct + serveWeb opt)显式抬 2986→2990;MU-M5 记忆树 git 快照 opt-in 接线(GOTONG_BUTLER_MEMORY_GIT 解析 + sweeper gitSnapshot)显式抬 2990→2996;审计 P1 全局 unhandledRejection 兜底网(installProcessSafetyNet)防后台计时器崩宿主,显式抬 2996→3000;审计⑦-1 抽出 transcript-line.ts(describe 日志渲染器,纯函数无闭包依赖)后 2945,下调 3000→2960——按上文体检记录 1 留余量,不再零余量;审计⑦-2 抽出 butler-env.ts(GOTONG_BUTLER* 旋钮解析)后 2911,再下调 2960→2925;审计⑦-3 抽出 wire-peers.ts(联邦整条腿 250 行)后 2682,再下调 2925→2700;SESS 会话窗 meChatSession 适配器(quick-chat 与 IM 共窗,管家行门控)2713,显式抬 2700→2725 留余量;SDUI 管家优先票 /api/me/agents 投影加 isButler(SESS 同一判定)2728,显式抬 2725→2740 留余量
  { file: 'packages/web/src/server.ts', max: 2395 }, // web route assembly (types → server-types.ts); 2350→2370 C-M2-M3b 出站 OAuth connect 路由对(公开 callback + admin begin 派发),抽 blessed OIDC/SAML 派发风险更高故显式抬棘轮;2370→2381 C-M2-M5a 出站 OAuth 连接器 CRUD 路由组接线(import+re-export+ctx+HandlerCtx 字段+派发块,逻辑在 oauth-connector-admin-routes.ts 不计),同 OIDC/SAML 派发家族;2381→2395 SESS 会话窗 meChatSession surface 穿线(import+opts+ctx 三处)2386,留余量
  { file: 'packages/web/src/me-routes.ts', max: 2885 }, // /me route sprawl (types → me-routes-types.ts); 2850→2885 SESS 会话窗 quick-chat 腿(history 骑 payload + recordReply ok-only 回写,类型在 me-routes-types.ts 不计)2872,留余量
