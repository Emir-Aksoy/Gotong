/**
 * cli.ts — the interactive command-line launcher for codex-deepseek-hub.
 *
 * The sibling of personal-coding-hub's CLI, but pairing Codex with a
 * DeepSeek-backed TUI instead of Claude Code + Codex. Opens a readline loop you
 * keep typing coding goals into, and the DeepSeek router dispatches each one to
 * codex / deepseek-tui live.
 *
 * Same three-auth-layer wiring as index.real.ts (see real-agents.ts):
 *   ① router brain = DeepSeek (chat) via OpenAIProvider — needs DEEPSEEK_API_KEY,
 *      passed EXPLICITLY to the provider.
 *   ② codex        = a real CliParticipant under its OWN CLI login. No key injected.
 *   ③ deepseek-tui = a DeepSeek-backed terminal coder, reading DEEPSEEK_API_KEY from
 *      the ambient env (same DeepSeek account as the router — that's fine, it's yours).
 *
 * Usage:
 *   DEEPSEEK_API_KEY=sk-... pnpm demo:codex-deepseek-hub:cli
 *   DEEPSEEK_API_KEY=sk-... pnpm demo:codex-deepseek-hub:cli -- --cwd /path/to/repo
 *   STUB_CODERS=1 DEEPSEEK_API_KEY=sk-... pnpm demo:codex-deepseek-hub:cli   # dry run
 *
 * Type a goal at `coding-hub>` and press enter to dispatch it. Meta commands
 * (prefix `:`): :help  :roster  :policy <大白话>  :files  :progress  :quit
 *
 * SAFETY: a throwaway git repo by default (or your `--cwd` repo); dangerousCommandGate
 * on; each CLI sandboxed to the workspace (codex workspace-write; the DeepSeek TUI
 * runs in the cwd).
 */
...
const META_HELP = [
  ...
  '    :quit          退出 (别名 :q / :exit)',
  '  其它任何输入都会作为「编码目标」派给路由模型, 由它决定派给 codex / deepseek-tui。',
...
async function main(): Promise<void> {
  ...
  if (!apiKey) {
    console.error('\n[cli] 缺 DEEPSEEK_API_KEY —— 路由模型(DeepSeek)需要它。')
    console.error('  用法: DEEPSEEK_API_KEY=sk-... pnpm demo:codex-deepseek-hub:cli\n')
    process.exit(2)
  }
  ...
  console.log('\n=== codex-deepseek-hub (交互式 CLI) ===')
  console.log(`  router : DeepSeek ${DEEPSEEK_MODEL} @ ${DEEPSEEK_BASE_URL}`)
    ...
    while (queue.length > 0 && !quitting) {
      ...
      if (text.startsWith(':')) {
        ...
        if (name === 'policy') {
          ...
          if (!instruction) {
            console.log('  用法: :policy <大白话>  例: :policy codex 今天不在岗 / 让 deepseek-tui 主理')
            continue
          }
          const edit = applyPolicyEdit(policy, instruction)
          if (!edit.understood) {
            console.log('  没听懂这条分工调整, 换个说法 (例: codex 不在岗 / deepseek-tui 主理 / 预算限单)。')
            continue
          }
...
main().catch((err) => {
  console.error('[codex-deepseek-hub cli] fatal:', err)
  process.exit(1)
})
