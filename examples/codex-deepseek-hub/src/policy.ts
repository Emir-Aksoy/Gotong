 ...
 * "codex 今天不在岗" / "以后让 deepseek-tui 主理" into a policy edit written back to
 ...
 *
 * (This is the codex-deepseek-hub twin of personal-coding-hub/src/policy.ts — same
 * mechanism, just a `codex | deepseek-tui` roster instead of `claude-code | codex`.)
 */
 ...
 * sentences ("codex 不在岗, deepseek-tui 主理") into clauses and applies one intent
 * each, so the deterministic stand-in handles the common multi-part case too. A real
 * LLM would parse the whole sentence; the file contract it produces is identical.
 */
 ...
 * edit changes the model's judgement without touching code (real-agents.ts M2).
 */
