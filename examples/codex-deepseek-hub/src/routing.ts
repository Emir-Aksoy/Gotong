 ...
 * This is the assertable core of 合理地调度: the router dispatches the RIGHT coding
 * agents for THIS goal, *combined with the user's standing arrangements* — not a
 * fixed deepseek-tui → codex pipeline every time.
 *
 * This hub pairs two coders with complementary strengths:
 *
 *   · `deepseek-tui` — a DeepSeek-backed terminal coding agent. DeepSeek's reasoner
 *     is strong at analysis / design, so it LEADS the thinking work: review,
 *     design, multi-file refactor.
 *   · `codex` — OpenAI Codex, the fast implementer: scaffolding, quick fixes, and
 *     turning a plan into code.
 *
 * Two inputs, mirroring 合理调度:
 *
 ...
 *      judgement; the keyword classifier stands in so the demo is deterministic.
 *
...
export type CodingAgent = 'codex' | 'deepseek-tui'

/** Narrow an arbitrary value to a known coding-agent id. */
export function isAgent(x: unknown): x is CodingAgent {
  return x === 'codex' || x === 'deepseek-tui'
}

// Coder aliases for reading names out of plain language (中英). The deepseek-tui
// pattern is tried first (it's the lead) and never matches "codex", so order is safe.
const AGENT_ALIASES: Array<[RegExp, CodingAgent]> = [
  [/deepseek(?:[-\s]?tui)?|ds[-\s]?tui/i, 'deepseek-tui'],
  [/codex/i, 'codex'],
]
...
/**
 * The default roster: the DeepSeek TUI leads analysis / design / refactor (its
 * reasoner's strength); Codex is the fast implementer. Under this policy the
 * routing reproduces the obvious calls (review → DeepSeek TUI, trivial → Codex,
 * feature → both).
 */
export const DEFAULT_CODING_POLICY: RoutingPolicy = {
  profiles: [
    { agent: 'deepseek-tui', strengths: ['review', 'analysis', 'design', 'refactor', 'multi-file'] },
 ...
 * coders ("交给 codex 实现", "deepseek-tui 设计、codex 实现"), that overrides the
...
// so "tidy codex's comments" (mentions a name, no dispatch intent) won't match.
