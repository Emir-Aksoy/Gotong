...
`message/send` 就回一个 Message。它的收口文档（`docs/zh/ledger/V5-H-FINAL.md §七`）把这条列为推迟:

> **A2A task 生命周期作为工作流步**: 若外部 A2A agent 是会**挂起**的 Gotong（返回 Task 而非
...
「外部长任务 A2A agent」由一个注入的 `fetchImpl` 扮演（和 `@gotong/a2a` 单测同一手法）:
...
| demo 用的 | 生产真东西 |
|---|---|
| lifecycle `A2aRemoteParticipant` + 注入 `fetchImpl` | 同一个 `A2aRemoteParticipant`（`@gotong/a2a`，`lifecycle` opt-in），`fetchImpl` 用 `global fetch` 调真端点 |
...
详见 [`docs/zh/ledger/V5-H-FINAL.md`](../../docs/zh/ledger/V5-H-FINAL.md)（§九 Stream H2）。
