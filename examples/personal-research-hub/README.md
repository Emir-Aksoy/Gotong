...
> Gotong **能承担的一个案例**(不是框架功能,代码全在 `examples/`)。承接
...
`template/personal-research-hub.template.yaml`(`gotong.template/v1`)。它声明:
...
```
  template/personal-research-hub.template.yaml   (gotong.template/v1 — 可载入)
...
import { McpClientToolset } from '@gotong/mcp-client'
...
- **凭证**:`OBSIDIAN_API_KEY` 走 `env` 显式注入,模板里是 `${OBSIDIAN_API_KEY}` 占位,
  绝不字面 secret。
- **跨 hub 两层闸**:把这个 wiki 共享给别的 hub 时,MCP server 自身 ACL + Gotong
