...
async function main(): Promise<void> {
  ...
  const provider = new OpenAIProvider({
    ...
    // Note: in Phase 8 the provider dropped its bespoke `maxRetries`
    // option — retries on the streaming path are now handled by the
    // OpenAI SDK's own `maxRetries` plus the agent-level auth-failure
    // hook. DeepSeek's "Premature close" still happens occasionally but
    // the SDK default (2 retries) is usually enough.
  })
