 ...
 *
 * Copied verbatim from `examples/im-bridge-host/src/identity-resolver.ts`. The
 * store's API is sync because better-sqlite3 is sync; bridges want `await` so
 * they can swap in a remote / federated resolver later without API churn. This
 * adapter is the canonical bridge between the two.
 *
