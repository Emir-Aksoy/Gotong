/**
 * Node-side Hub for the remote-python demo. Mirrors examples/remote-agent/host.ts
...
async function main(): Promise<void> {
  ...
  const ws = await serveWebSocket(hub, { port: 4001 })
  console.log(`[host] WebSocket transport listening at ${ws.url}\n`)

  console.log('[host] waiting up to 8s for a Python agent to register...')
  await waitForAgent(hub, 8_000)
  console.log('[host] remote python agents present, dispatching tasks\n')

  const draft = await hub.dispatch({
    from: 'host',
    strategy: { kind: 'capability', capabilities: ['draft'] },
    payload: { topic: 'cross-language agents' },
    title: 'write a draft',
  })
...
async function waitForAgent(hub: Hub, timeoutMs: number): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      stop()
      reject(new Error(`no python agent registered within ${timeoutMs}ms`))
    }, timeoutMs)
