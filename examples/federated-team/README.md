...
```bash
pnpm --filter @gotong/example-federated-team upstream  # :3200 web / :4200 ws
pnpm --filter @gotong/example-federated-team team      # :3300 web
pnpm --filter @gotong/example-federated-team driver    # dispatches the three tasks
```

## Scenario

- **Upstream Hub** (`.gotong-upstream/`) — what a parent organisation runs. Bound to `:4200` for agent connections.
- **Team Hub** (`.gotong-team/`) — a sub-team's own private Hub.
...
- **No special protocol**: the bridge speaks the same v1.x wire protocol as any other remote agent. The "team" abstraction is library code in `@gotong/sdk-node`, not new wire frames.

> **Token caveat**: the launcher greps the upstream admin token from stdout. On second run (the token already lives in `admins.json` and isn't reprinted) you'll get an error message telling you to delete `.gotong-upstream/` to remint.
