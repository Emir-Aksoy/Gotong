/**
 * load-template — proof that personal-coding-hub's "brain" config is a LOADABLE
 * FILE, not a built-in TS literal.
 *
 * It reads the shipped `template/personal-coding-hub.template.yaml` off disk,
 * parses it, and previews the architecture it declares:
 *
 *   [1] LOAD     — the config comes from a file (read + parsed here, live).
 *   [2] mentor   — a methodology-aware coding-mentor agent that reaches a KB.
 ...
 * (packages/web/tests/personal-coding-hub-template.test.ts), which runs this
 * exact file through the real parseTemplate + import route.
 *
 * Run:  pnpm demo:personal-coding-hub:template
 */
...
const TEMPLATE = fileURLToPath(
  new URL('../template/personal-coding-hub.template.yaml', import.meta.url),
)
...
function main(): void {
  ...
  // --- [2] the methodology-aware mentor agent --------------------------------
  section('[2] the methodology-aware mentor agent')
  ...
  console.log('      examples/personal-coding-hub/template/personal-coding-hub.template.yaml)\" \\')
  ...
  section('done')
  console.log('  Loaded from a FILE — mentor + methodology KB slot (pointer); content is the vault.\n')
  process.exit(0)
}
