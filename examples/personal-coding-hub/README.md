...
—— `template/personal-coding-hub.template.yaml`(`gotong.template/v1`)。它声明:
...
```
  template/personal-coding-hub.template.yaml   (gotong.template/v1 — 可载入)
...
上面的 `:cli` / `:real` 已经把这套接好了。想自己拼,把 mock 换成 `@gotong/cli-agent`
的 `CLI_PRESETS` 预设、给各自的 key、换一个真 provider 即可 —— hub 接线一模一样:

```ts
import { CLI_PRESETS } from '@gotong/cli-agent' // 或本仓库 coding-agent-bridge 的 presets
import { AnthropicProvider } from '@gotong/llm-anthropic' // 路由的真 LLM
...
- `examples/oneclick-template/` —— `gotong.template/v1` 可载入模板格式的范本(本 case
  的 `template/` 照它的字段写)。
