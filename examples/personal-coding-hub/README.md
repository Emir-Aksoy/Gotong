...
```bash
pnpm demo:personal-coding-hub            # 6 个路由剧情 + 安全闸:结合「任务 × 安排」分派 + 共享仓库(可跑+自断言)
...
核心证明:**同一个目标在不同安排下派得不同**。前五个剧情只有两个目标 —— 一个功能、
一个琐碎修复 —— 但换了「用户的安排」,路由就把活派给不同的人。这正是「合理地调度」:
分派 = 分析任务 ×结合用户安排,不是固定流水线,也绝不派一个不在岗的 agent。
...
应路由的集合」—— [A] 与 [A2]/[A3] 是同一行目标却断言不同的分派集合,这就是「合理调度」
的可断言证据。
...
机制:[A]–[C] 都是 `LlmAgent` tool-use loop → `DispatchToolset.dispatch_task({agentId})`,
派几个、派谁由 `planRoute(goal, policy)` 决定;路由脑是一个**情境感知的 `LlmProvider`**
