...
AipeHub の設計スタンス全体は **ハブは自らが LLM を実行しない / 状態はすべてディスクファイル / クレデンシャルはあなたのマシンに留まる / フェデレーションはピアツーピア** です。このスタンスを貫くと、**コミュニティインフラもサーバーを必要としません**:
...
- `templates.json` — 機械可読な `aipehub.site/v1` フィード（ストアフロントはデータでもあり、ファイルファースト）。
