...
3. **状态都是磁盘文件**。`.aipehub/` 里能看到全部；复制目录 = 搬走房间，重启透明。

宪章正文：[`CHARTER.md`](CHARTER.md)。与代码冲突时**宪章为源**。

## 二、依赖方向：箭头只能朝内

内核干净是 AipeHub 还能快速接新东西的根本原因，靠的就是依赖方向：

```
protocol (零依赖, wire root)  ←  core  ←  workflow / inbox / a2a  ←  … 叶子 …  ←  host / web (装配层)
```

- `protocol` 永远零 `@aipehub` 依赖；`core` 只看 `protocol`；`workflow` 不碰任何 `llm` 包；
  `inbox` / `a2a` 只到 `core`。
...
`AIPE_*` 环境旋钮已经 100+ 个——每一个都是系统能悄悄换行为的暗门。惯例：

- **能不加就不加**。先问「这能不能是默认值 / 探测出来 / 从已有旋钮推出来」。
- 真要加，**必须登记**进 [`../../scripts/aipe-env-registry.txt`](../../scripts/aipe-env-registry.txt)。
- 呈现视角这类「先展示什么」的开关，走 `AIPE_PROFILE` 那种**纯映射、不分叉行为**的形态，
  别做成第 N 个行为开关（见 [`DEPLOYMENT-PROFILE.md`](DEPLOYMENT-PROFILE.md)）。

→ `pnpm check:env-registry`：代码里出现没登记的 `AIPE_*` 就红，登记了却没用也红。
...
| `AIPE_*` 旋钮加一个登记一个 | `scripts/env-registry-gate.mjs` + `aipe-env-registry.txt` | `pnpm check:env-registry` |
