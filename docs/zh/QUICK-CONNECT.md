...
> 把主流编码 agent 一键接到正在运行的 Gotong Hub。
> 这是 [`AGENT-ADAPTER-CONTRACT.md`](AGENT-ADAPTER-CONTRACT.md) 「双向」里的
> **入站**那一半（agent → Gotong）；「出站」（hub 反过来驱动它们）是另一条
> 线，见该契约。
>
...
Gotong」对整个品类是**同一个动作**：把 agent 的 MCP 配置指向
`@gotong/mcp-server` —— 它本身是 Hub admin HTTP API 的一个瘦客户端
...
```
command: node
args:    [<packages/mcp-server/bin/gotong-mcp.js 的绝对路径>]
env:     GOTONG_HUB_URL, GOTONG_ADMIN_TOKEN
```

> **为什么用 `node` + 绝对路径，而不是 `npx -y @gotong/mcp-server`？**
> `@gotong/mcp-server` 还没发到 npm。发布前一律用绝对路径 spawn。
...
gotong connect claude-code --token="$GOTONG_ADMIN_TOKEN"
gotong connect cursor --name=my-hub --hub=http://127.0.0.1:3000
```

- `--hub=<url>`：Hub admin HTTP 基址（默认 `$GOTONG_HUB_URL` 或 `http://127.0.0.1:3000`）
- `--token=<token>`：要内联进配置的 admin token（默认占位符；**绝不**把 env 里的
  密钥自动写进终端输出——要内联请显式传 `--token`）
- `--name=<name>`：MCP server 在 agent 配置里的名字（默认 `gotong`）
- `--bin=<path>`：`gotong-mcp.js` 路径（monorepo 检出里自动探测，否则手动给）
...
```bash
claude mcp add gotong \
  -e GOTONG_HUB_URL=http://127.0.0.1:3000 \
  -e GOTONG_ADMIN_TOKEN=<你的 admin token> \
  -- node /abs/path/Gotong/packages/mcp-server/bin/gotong-mcp.js
```
...
> 各家配置路径 / 命令偶有变动，以官方文档为准；`gotong connect <id>` 输出的
...
- **admin token 是密钥**。`gotong connect` 默认填占位符 `<YOUR_ADMIN_TOKEN>`，
  不会自动把 `$GOTONG_ADMIN_TOKEN` 写进可被回滚缓存 / 录屏看到的终端输出。要内联
  请显式 `--token`，或直接在打印出的配置里手动替换。
- Gotong MCP server 用的是 **admin Bearer**，权限很大（能派任务、读账本）。给
...
本页只解决**入站**（agent 调 Gotong）。让 Hub **反过来驱动**这些 CLI agent
