...
跟 RAG 一样,**笔记库、搜索索引、向量库 一律不进 Gotong**。`identity.sqlite`
...
「知识库连接器」因此**不是 Gotong 的一个新子系统**,而是同一个 MCP 接入模式
的三种形态:
...
三者对 Gotong 是同构的:agent YAML 里一段 `mcpServers`,host 一个子进程,
...
- **凭证不进 audit log**;走漏面在 MCP server 一侧,不在 Gotong。
...
走通用 `mcp_calls` 指标,**不区分** RAG / 笔记搜索 / ES query —— Gotong 看到的
只是一次 MCP 工具调用,判不出 server 内部是哪种操作,不同 server 的「一次调用」
成本也天差地别。两层 quota(Gotong 侧 `mcp_calls` 上限挡死循环 + server 侧按
...
+ 访问控制 MCP 协议天然处理,Gotong 不在外面再封一层 shared_with 模型。

**第二层:Gotong per-link 的 KB allowlist(v5 C-M1)。** 这是 RAG-VIA-MCP
...
互不污染」。详见 `docs/zh/ledger/V5-C-FINAL.md`。
...
用 `gotong.template/v1` 模板搬走一整套 agent + workflow 时(v5 Stream B),
...
去灌)。这跟「Gotong 不存知识」一脉相承 —— 连模板这种「打包搬家」场景也不破例。
详见 `docs/zh/ledger/V5-B-FINAL.md`。
...
- `docs/zh/ledger/V5-C-FINAL.md` —— per-link KB allowlist(第六节第二层闸)+ 多组织
  隔离 E2E 验收门。
- `docs/zh/ledger/V5-B-FINAL.md` —— 模板 KB 槽位(第七节,带引用不带内容)。
