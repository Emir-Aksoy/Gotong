...
三种形态用的是**同一个 `gotong-host` 二进制、同一套 `GOTONG_*` 环境变量、同一个
...
|---|---|---|---|
| 二进制 | `gotong-host` | `gotong-host` | **无差异** |
| 数据目录 | `.gotong/`（整目录搬走=搬走房间） | `/srv/gotong-data` | 仅路径 |
| 配置 | `GOTONG_*` 环境变量 | `GOTONG_*` 环境变量 | **无差异** |
| IM 接入 | 填 `GOTONG_TELEGRAM_BOT_TOKEN` | 同左 | **无差异** |
| 成员绑定流程 | `/me` 出码 → `/bind` | 同左 | **无差异** |
| 绑定地址 | `127.0.0.1`（loopback） | `127.0.0.1`（Caddy 反代） | 都是 loopback |
| **反向代理 + TLS** | 不需要 | **Caddy + 域名 + 证书** | ← 周界 |
| **`GOTONG_COOKIE_SECURE`** | `0`（admin 只本机开） | **`1`**（cookie 过线） | ← 周界 |
| **`GOTONG_ALLOWED_HOSTS`** | 不需要 | **必设**（CSRF/DNS-rebinding） | ← 周界 |
| **`GOTONG_TRUST_PROXY`** | 不需要 | **`1`**（限速按真客户端 IP） | ← 周界 |
...
**非 loopback**（`0.0.0.0` / 域名）时才强制要求 `GOTONG_ALLOWED_HOSTS` + `GOTONG_COOKIE_SECURE=1`，
...
> 放开中（香港已证实）。**马来西亚号已真机证实在灰度内（WX-M3a）**：`gotong wechat-login`
> 全状态机走通（终端二维码 + 浏览器链接兜底均可用），`confirmed` 发回 bot token +
> `baseurl` 主端点。**消息 round-trip 实录待 WX-M3b 补录**（`/bind` 回执 → 管家对话 →
> 审批停靠在下次开口时补投）。
...
$EDITOR .env.local                      # 填 GOTONG_TELEGRAM_BOT_TOKEN=<你的 token>
...
成员发的自由文本会派给 `GOTONG_IM_CHAT_CAPABILITY`（默认 `chat`）这个 capability。
...
```bash
# 在 .env.local 里改：
GOTONG_HOST=0.0.0.0                       # 绑所有网卡
GOTONG_ALLOWED_HOSTS=192.168.1.42:3000    # 你的局域网 IP
GOTONG_ALLOW_INSECURE=1                   # ★ 必须 ★ 见下
```

> ⚠️ 绑 `0.0.0.0` 触发 boot 安全自检：它要求 `GOTONG_COOKIE_SECURE=1`，但 LAN 是明文
> HTTP、开了 Secure cookie 反而发不出去 → 死锁。可信 LAN 的正解是设 `GOTONG_ALLOW_INSECURE=1`
...
`平台 + 平台用户ID → Gotong userId` 对上，之后每条消息都按真实成员记账/审计。
...
它把「取码 → Node/pnpm → 建服务用户 → build → 落 `/etc/gotong.env`（就是下面这份模板）→
...
```bash
sudo cp deploy/.env.cloud /etc/gotong.env
sudo chown gotong:gotong /etc/gotong.env && sudo chmod 640 /etc/gotong.env
sudo $EDITOR /etc/gotong.env            # 改域名；master key 从 secret 注入（见下）
```

模板已设好过线防御三件套（`GOTONG_COOKIE_SECURE=1` / `GOTONG_ALLOWED_HOSTS` /
`GOTONG_TRUST_PROXY=1`）+ master key `env` provider + 保留策略。
- **T2（云 + IM）**：填 `GOTONG_TELEGRAM_BOT_TOKEN`，成员走 IM。Web 端口可以完全
...
> 的 `config-set`（owner 在网页 / CLI 改）把 `GOTONG_MODE` / `GOTONG_WEB_PORT` / `GOTONG_WS_PORT` /
> `GOTONG_OPEN_BROWSER` 这类**非密钥**旋钮写进 `<GOTONG_SPACE>/gotong.env`。让它生效 = host 启动前
> 有人 source 它；systemd 用 `EnvironmentFile=` 干这件事（host 自己仍只读 `process.env`，boot
> 读路径逐字节不变）。在 `[Service]` 段加一行（前缀 `-` = 文件缺失不报错）：
>
> ```ini
> EnvironmentFile=-/var/lib/gotong/.gotong/gotong.env   # 路径 = <GOTONG_SPACE>/gotong.env
> ```
>
> 它和上面那份 `/etc/gotong.env`（操作员手填的过线防御 + master key provider）是**两份不同的
> 文件**：`/etc/gotong.env` 是你部署时一次写好的基线，`<space>/gotong.env` 是 owner 之后经
> 控制台调过的旋钮。`setting` 白名单**按构造拒一切密钥键**，所以 `<space>/gotong.env` 里永远不会
> 有 token / master key —— 密钥仍走 §T2/T3.2 的 systemd secret。便携双击 launcher
> （`deploy/Gotong.command` / `.sh`）已自带同款 source，云端只需补这一行 systemd 配置。
...
- [`Dockerfile`](../../Dockerfile) —— 多阶段 build；运行时非 root（`node` 用户）、
  默认 `GOTONG_SPACE=/data`、内建 `/healthz` HEALTHCHECK。
- [`docker-compose.yml`](../../docker-compose.yml) —— 本地 / 内网档：数据 bind-mount 到
  `./data`，旋钮在 `environment:` 里加 `GOTONG_*`。端口**只发布到 loopback**
  （`127.0.0.1:3000:3000`），配 `GOTONG_ALLOW_INSECURE=1` 出厂即起（容器内必绑
...
`<GOTONG_SPACE>/runtime/admin-link.txt`（容器内 `/data/runtime/…` = 宿主 `./data/runtime/…`），
...
| 2 | 编辑 `/etc/gotong.env`：填 token + master key + **按拓扑调开关（见下）** | 1–2 分钟 | — |
| 3 | 跑 [`scripts/cloud-harden.sh`](../../scripts/cloud-harden.sh) | <1 分钟 | 无红项（纯 T2 档见下方注） |
| 4 | `sudo systemctl start gotong` | <1 分钟 | `curl -s 127.0.0.1:3000/healthz` 回 `ok`；journalctl 里出现 `IM bridge enabled`（`"platform":"telegram"`） |
...
- **T3（有域名 + Caddy）**：照模板 —— `GOTONG_ALLOWED_HOSTS` 填真域名，三件套全开。
- **T2（无域名，web 全封 loopback、只经 ssh 隧道进）**：`GOTONG_ALLOWED_HOSTS=127.0.0.1:3000`，
  `GOTONG_COOKIE_SECURE` / `GOTONG_TRUST_PROXY` 留空不设。为什么：CSRF 闸按 Host/Origin
...
`/etc/gotong.env` 明文、**别**提交 git）喂给 `GOTONG_MASTER_KEY`，并保持
...
| admin token 暴力破解 | 撞库进 admin | `GOTONG_ADMIN_RATE_MAX/SEC` 限速 + `GOTONG_TRUST_PROXY=1` 按真 IP 限 |
| CSRF / DNS-rebinding | 诱导你的浏览器发伪造 admin 请求 | `GOTONG_ALLOWED_HOSTS` 精确列白名单（不设 = 此防御关闭） |
| cookie 明文泄露 | 会话劫持 | `GOTONG_COOKIE_SECURE=1` + 全程 HTTPS |
| 数据盘/快照被盗 | 直接解开 vault | master key 走 `env` provider，不落数据盘（§T2/T3.2） |
| 开放注册被滥用 | 陌生 agent 涌入 | `GOTONG_GATING=admin-approval`（**绝不** `open`） |
...
逼你要么补好 TLS（正解），要么 `GOTONG_ALLOW_INSECURE=1` 强行放行（**别这么干**——
...
`X-Forwarded-For`** —— 哪怕开了 `GOTONG_TRUST_PROXY`：
...
sudo -u gotong -H GOTONG_SPACE=/srv/gotong-data \
  ...
  node /opt/gotong/packages/host/dist/main.js mint-admin-token "Operator"
```

它只 open 工作区、往 `admins.json` 加一个 admin、按 `GOTONG_HOST`/`GOTONG_WEB_PORT`/
`GOTONG_COOKIE_SECURE` 打印一次性 URL（公网域名直接对得上），然后退出。一次性 URL
写到 `<GOTONG_SPACE>/runtime/admin-link.txt`（权限 `0600`）而非 stdout，**防止 token
...
- [ ] `GOTONG_HOST=127.0.0.1`（或确认走了 §五 LAN 变体）
- [ ] `GOTONG_GATING=admin-approval`
- [ ] 测过一遍：自己用 Telegram 出码 → `/bind` → 发消息收到回复
- [ ] 机器人用户名 + 出码指引已发给成员

### T2/T3 云服务器（在 T1 清单基础上 +）
- [ ] `GOTONG_COOKIE_SECURE=1` + `GOTONG_ALLOWED_HOSTS=<所有域名>` + `GOTONG_TRUST_PROXY=1` 三件套
...
两道防线：**启动前**用 `gotong doctor` 预检环境；**真启动失败**时 host 把常见、可恢复
的失败翻成一句话 + 指向 `doctor` 的人话（`friendlyBootError`），而不是甩一坨 stack trace。

### 11.1 `gotong doctor` —— 启动前预检（不启动 host）

```bash
gotong doctor          # 逐项 ✓/⚠/✖ + 修复建议；退出码 0=无 blocker，1=有，2=用法错
gotong doctor --fix    # 先做「安全可逆」的自动修，再复检
```

预检 7 项：Node ≥ 20 / `@gotong/host` 可解析 / `GOTONG_WEB_PORT`、`GOTONG_WS_PORT` 真能 bind /
`GOTONG_SPACE` 可写（或首启可建）/ `provider=env` 时 `GOTONG_MASTER_KEY` 在 / env 里有没有 LLM
key。**只报告 env 变量名，绝不打印值**。

`--fix` **只自动建缺失的数据目录**（`mkdir -p GOTONG_SPACE`，可逆、host 首启本来也会建——
...
类，打出「`✖ Gotong could not start — …`」+ 该改哪个 env + `Run gotong doctor`：

| 症状（errno） | 人话提示讲什么 | 你要做什么 |
|---|---|---|
| 端口被占（`EADDRINUSE`） | 哪个口被占（admin UI 还是 agent WS）、对应哪个 env | 关掉占口的进程，或把 `GOTONG_WEB_PORT` / `GOTONG_WS_PORT` 改到空闲口 |
| 没权限绑端口（`EACCES`/`EPERM` on `listen`） | <1024 是特权端口（**不会**误导你去 chmod 数据目录） | `GOTONG_*_PORT` 改到 ≥1024，前面挂反代转 80/443 |
| master key 缺失 / 损坏 | vault 用 master key 加密 secrets；附上底层原因 | 默认 file：从备份恢复 `<space>/identity-master.key`（新 key 解不开旧 secrets）；env provider：设 `GOTONG_MASTER_KEY` + `GOTONG_MASTER_KEY_PROVIDER=env` |
| 数据目录不可写（`EACCES`/`EPERM`/`EROFS`） | 列出写不进的具体路径；只读挂载会显式说 | 修目录属主/权限，或把 `GOTONG_SPACE` 指到能写的目录（只读挂载则重挂 rw） |
| 磁盘满 / 超配额（`ENOSPC`/`EDQUOT`） | 写 `GOTONG_SPACE` 时盘满 / 超 quota | 清空间或提配额后重启 |
...
| 启动前预检 + `--fix`（`gotong doctor`） | `packages/cli/src/commands/doctor.ts` |
