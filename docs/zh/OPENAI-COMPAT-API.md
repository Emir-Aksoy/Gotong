...
#### M2 落地后的确切帧序（2026-09-02）

```
data: {... "delta":{"role":"assistant"}, "finish_reason":null}   ← 连接一建立就发
: keep-alive                                                     ← 每 15s，SSE 注释行
data: {... "delta":{"content":"<整段答案>"}, "finish_reason":null} ← **只此一帧**
data: {... "delta":{}, "finish_reason":"stop"}
data: [DONE]
```

- **内容只发一帧，不假装分片。** 把定稿后的答案切成小块再逐块吐，首 token
  延迟一点没变，却会让读日志的人以为这里有真流。宁可帧数难看，也不造这个像。
- **正文为空时那一帧根本不发**（不是一帧 `content:""`），累加仍然等于 `''`。
- **心跳走 SSE 注释行**（`:` 开头，规范规定解析器忽略）。派发可能要等到两分钟
  超时，中间隔着反代和客户端各自的空闲超时，没有它容易被掐。

#### 先验证、后开流

到「开流」那一行为止的所有形状类错误——`tools`、`n>1`、`stream_options.include_usage`、
`stream` 不是布尔、model 不存在或没权限、没 bearer、限流——**仍然是真 HTTP 状态码
+ JSON 错误信封**，一次派发都不发生。只有**派发之后**才可能出错的东西（agent 失败、
没人在线、超时）落在流里，因为那时 200 已经写出去了、改不了。

流里的失败发**一帧 `{"error":{...}}` 然后收掉连接，不发 `[DONE]`**：补上它等于
让客户端把一次截断读成正常收尾。那一帧里多带一个非标准的 `status` 键——
「这本该是 502 还是 503」在流里结构上无处可说，丢掉就真的丢了。

#### 两条路共用同一处派发

「delta 累加 === 非流式答案」这条判据的前提是**两边不是两份实现**。流式与非流式
共用 `runCompletion()` 一处派发 + 一处结果归一；分成两份，这把尺当天就变成两个
各自演化的东西在赛跑。判据本身是**三方对拍**（流式 / 非流式 / 写定的那段字）——
只比前两者，会在两边错成同一个样子时空洞地绿。
...
| `stream` 不是布尔 | **400** | 静默当成 falsy 降级 ⇒ 客户端等一个永远不来的 `[DONE]` |
| `stream_options.include_usage: true` | **400** | 同下面那条 `usage`——结构上给不出。它明确要了，就不能装作给了 |
...
3. **没有 `usage` 字段。** 真账在 hub 的用量账本里。`stream_options.include_usage`
   要了就是 400，不是静默不给。
4. **流是真 SSE，但首 token 延迟没有改善。** 连接立即建立并靠心跳保持，内容
   `delta` 只在结果定稿后**一帧**发出（§4.6）。要的是「delta 累加 === 最终答案」，
   不是像有流。派发之后才发生的失败落在流里（一帧 `error`，**没有 `[DONE]`**）。
5. **`system` 消息不生效**，`temperature` 一类参数被忽略（§4.7）。
6. **无状态**——历史以你发的 `messages` 为准（§边界②）。
