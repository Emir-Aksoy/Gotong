...
| 方向 | 含义 | 判据（适配器必须证明） |
|---|---|---|
| **入站** 外部 agent → 调 hub | 对方把 Gotong 当工具/对端 | 对方作为 **MCP 客户端**接 `@gotong/mcp-server`，或 **A2A 调用方**打 `/a2a`（per-peer bearer，fail-closed）。 |
...
> 接管发生在**各自机构内部**，Gotong 这侧的接管粒度由它们暴露的 HITL 决定（多为 T0–T2）。
...
1. ✅ **P0｜一个通用 CLI shell-out adapter**（`examples/coding-agent-bridge/`）——**已落地**
   （Stream E E2, commit `e5ebd51`→`57fe00d`）。`@gotong/cli-agent`（`CliParticipant` +
...
对比与排名文章、各厂商文档、awesome-ai-agents-2026 目录）。Tier 与接入路是 Gotong 侧的
