...
动作 hook，比 shell-out 的 spawn 前 regex 闸粒度更细（`@aipehub/acp-agent` + `examples/acp-coding-bridge`）。
...
1. ✅ **P0｜一个通用 CLI shell-out adapter**（`examples/coding-agent-bridge/`）——**已落地**
   ...
   M0-M8）。模仿 OpenClaw「从启动→hold session→分派」：`@aipehub/acp-agent` 把编码 agent 当
