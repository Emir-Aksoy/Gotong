...
1. **P0｜一个通用 CLI shell-out adapter**（`examples/coding-agent-bridge/`）——参数化命令模板，
   **一次覆盖 §6.1 整类**（Claude Code/Codex/OpenCode/Aider/Goose/Antigravity/Cursor CLI…）。
   先做到 T1（循环跑 + checkpoint + on-demand park），杠杆最高。
