...
> 这是项目的**单页地图**。看完你会知道：Gotong 是什么、它在"谁之下"、
...
**Gotong** 是一个 **自托管的协作工作空间，TypeScript / Python 通吃**：
...
框架（LangGraph、CrewAI、AutoGen）。Gotong **两者都不是**——它是它们
...
这个组合——而不是某个精巧的协议——才是 Gotong 的本体。它是第一个把
...
| **给家里装一个家庭 hub** | 模板画廊一键装：家人各有账号与角色、凭证进加密金库、对外动作停靠等家长批准 | [`FAMILY-HUB.md`](FAMILY-HUB.md) |
| **一个人的公司（OPC）** | 模板画廊一键装：三个专业分身（客户沟通 / 经营简报 / 新单研究）、AI 起草你签发、DeepSeek 性价比甜区 | [`SOLO-COMPANY-HUB.md`](SOLO-COMPANY-HUB.md) |
| **小团队 / 专业服务公司** | 模板画廊一键装：多人分角色、三分身接力起草、客户库/知识库走只读 MCP、负责人跨角色签发（OPC 向上兼容案例） | [`PRO-FIRM-HUB.md`](PRO-FIRM-HUB.md) |
...
"我能否把 Gotong 嵌进自己的闭源产品 / 我商用这些模板要不要注明 /
我能不能改 LICENSE 重新打包"。

而且它会**一直是** MIT：项目[承诺永不改证](GOVERNANCE.md#许可恒定)——
不转 BSL / SSPL，永远不。
...
Gotong 把"团队"做成了**一个 room** = 一个 `.gotong/` 目录。
角色三层：
...
所有人都在同一个 `.gotong/` 目录、同一个 hub 进程。这是默认情况。

### 多个 room 联起来（= 真·跨组织）

每个组织跑自己独立的 hub（自己的 `.gotong/`、自己的人和 agent、
...
**三大方向**（2026-08 钉死的聚焦域）：之后的每一个开发都往这三个方向走，
且每一个开发都要说明增强的是哪个方向（纪律与各方向路线：
[`DIRECTIONS.md`](DIRECTIONS.md)）：

- **工具调用能力** — 让 agent 把工具用对、用省、用安全：工具循环、工具面、执行分级与沙箱、调用韧性、效果回路。
- **记忆管理能力** — 让 agent 把记忆整理好、用得上、忘得掉：捕获与蒸馏、检索、分层、双时态与遗忘、投影、写侧可测性。
- **agent 间协作框架** — 让不同人的、不同宿主的、不同信任层级的 agent 低损耗互联：联邦与信任分级、互操作协议面、治理随协作旅行。

**两条方法论**（本项目对未来 agent 与 agent 生态的判断）：

- **智能体（阿同）：LLM 自适应** — 同一副骨架，接上弱、中、强任意一档模型，都在那一档拿到较好的实际效果；弱档骨架补位（步子更小、工具面更窄、勤复述、紧预算、早转派早问人），强档骨架让路，治理闸永不随档松。衡量靠实际效果（可用率、返工率），不靠榜单分数。
- **框架（Gotong）：形态兼容** — 到人和 agent 已经在的地方去，用他们已有的形态（技能包 SKILL.md、信封文件、MCP / A2A、IM）。判据：对方零改动，兼容工作可单方面完成。
- **统一原则：能力搬家** — 把能力从模型权重里搬进可携带的结构（技能、记忆、流程、治理）。搬得越多，能力越不随模型档位波动，也越能跨宿主旅行。

完整推理与证据链：[`STRATEGY-2026-08.md`](STRATEGY-2026-08.md) §十。
