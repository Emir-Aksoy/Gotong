...
**三句话守则**(贯穿四块):
1. **复用不重造** —— 接现成 `window.Gotong.describeError()`、现成 `testLlmKey` /
...
`✖ Gotong could not start — …` + 该改哪个 env + `Run gotong doctor`,纯函数零副作用
(调用点 `main.ts` 顶层 boot catch 已接线,零改):

| # | 触发(errno / 特征) | 提示 |
|---|---|---|
| 1 | `EADDRINUSE` | 哪个口被占(admin UI / agent WS)、对应 `GOTONG_WEB_PORT` / `GOTONG_WS_PORT`(body 与原 ⑥-M2 逐字节不变) |
| 2 | `EACCES`/`EPERM` **on `listen`** | <1024 特权端口提示,改 `GOTONG_*_PORT` ≥1024 或挂反代 |
| 3 | message 含 `"master key"` | vault key 缺失/损坏,默认 file 从备份恢复 `<space>/identity-master.key` / env provider 设 `GOTONG_MASTER_KEY` |
...
自动修**:数据目录(`GOTONG_SPACE`)缺失时 `mkdir -p`——可逆(`rmdir`)、host 首启本来也会建,
...
- 根 `README.md` Quick start 加「Won't start?」小节:`pnpm exec gotong doctor [--fix]` +
  指向 GO-LIVE §十一。
...
的 `wf-run-live-pulse` keyframe。tab 切走停、切回 `home` 重起(走既有 `gotong:tabchange`
...
形状,经**真**刷新按钮 + 真 `gotong:tabchange` 事件驱动真轮询链,断言:活跃 run → 亮
...
> `GOTONG_STRICT_DEFINITIONS=1` 才切「拒启」给 CI / 严格场景。
...
`@gotong/host/check` 子路径导出。它复用早就存在的确定性校验器,**零新分类器**:
...
1. **`gotong check`(CLI,按需)** —— tiny CLI 懒解析 host,动态 import 那个
   **不启动**的 `./check` 子路径跑 `runCheckCli`。host 不在场 → 给安装提示 + 返 1
   (VALID-M2,commit `6b4de6f`)。
2. **boot 横幅 + `GOTONG_STRICT_DEFINITIONS`(host main.ts)** —— 启动时
...
覆盖(doctor 面)。**带上 config 就会双重报告**。所以:`gotong check` 看全三域,
...
- **CLI 委托 host** —— `@gotong/host/check` 用**变量**(非字面量)动态 import,
...
- §七 的 VALID 校验(`@gotong/host/check`)就是 setting 的 `check` / `status` read-tier 复用进来;
...
- config-write **严格 grounded** 在 host 真读的文件上: `<space>/gotong.env`(非密钥白名单旋钮)+
...
8. 定义校验的**深度结构检查**(`checkWorkflowStructure`:unknown_agent / bad_ref /
   ...
   默认关;以及把 `gotong check` 接进 admin 「hub 体检」面板当一个常驻信号
