...
> Status: **M0 完(计划+三岔口拍板 2026-08-21) · M1 完(档案纯核 2026-08-21) · M2 完(分段执行器+接力驱动 2026-08-21) · M3 完(分解-回收 2026-08-21) · M4a 完(工种×模型配置面 2026-08-21) · M4b 完(槽解析+消费者 2026-08-21) · M6 完(capstone 2026-08-21) · M6.1 完(首跑三修:段里的钟+成本加权刻度 2026-08-22) · M6.2 完(待命语义:无事可做→睡到成员开口 2026-08-22) · **M6.3 完(段纪律禁未来日期承诺 2026-08-29)** · OBS-M2 完(进度不必开口问:观察者+面板卡 2026-08-28) · M5 随档刻度〔用户门〕待**。方向: **主 T 兼 M**
...
### M6.3 段纪律禁未来日期承诺(2026-08-29)✅

M6.2 收口时留了两条用户未选项,2026-08-29 用户批「几项都做」后其中提示层这条落地。
病根仍是 M6.2 记的那个:被追问的模型往 append-only journal 写**未来日期的承诺**,下一段
把它当历史读回去。此前的两道防线都是**条件性**的——钟卡的告诫只管「读」且只在有钟时
渲染(钟按合同是装饰性的),standby 唤醒句只在待命之后出现。这条管「写」,住在
`renderRelayPrompt` **无条件**的【本段纪律】块里:

> 日志只记已经发生的事:计划、约定、还没到的日期一律写进「下一步」,绝不写成已完成
> ——下一段把日志当历史读,把未来写成过去,后面每一段都会当真。

收尾段(`renderWindDownPrompt`)**刻意不带**:那里不教 `progress`(没有日志写),它自己的
「不要把部分完成说成完成」盖住同一病根的收尾形态,再塞一条会稀释「只做一件事」。
测试钉两头:无钟、无待命的裸 relay 里 bullet 仍在场(无条件性是靶心)+ wind-down 缺席;
变异(摘 bullet)恰 1 例红。另一条未选项「段用强模型」仍开着,等用户点。

### OBS-M2 长任务的进度不必开口问(2026-08-28)✅

治的是一件用起来才发现的事:长任务的全部意义就是「你不在的时候它在跑」,而在这一刀之前,
**唯一的进度面是去问阿同**。dossier 与 journal 老老实实躺在盘上,成员却看不见——对照
Manus 那种把 todo.md 摆在侧栏一直开着的做法,我们缺的不是数据是**面**。

修法是一只观察者加一张卡,**不新开权威点**:

- **`readLongRunSnapshot(dir, opts)`**(personal-butler 纯核,与 `readTaskNotesSnapshot`
  同族):**读者永不隔离**——坏档、目录名与内嵌 `taskId` 对不上、`readdir` 本身失败,
  一律跳过或诚实空,证据原地留着,**盘上一个字节不写**。**刻意不复用
  `openLongRunDossierStore`**,两条理由都写进了 docblock:①store 的每次读都排在驱动器的
  `enqueue` 串行链上(刷一次面板会排在某一段的写后面);②它的 loader 遇到坏档**会改名
  隔离**,而 rename 是一次写——**一个会给写者的文件改名的读者不是读者**。
- **排序是正确性不是展示偏好**:没完的在前、其余按 `updatedAt` 倒序,这样 `maxFinished`
  截尾才安全——一堆已结束的任务永远挤不掉一个还在跑的。`isUnfinishedLongRun` **刻意比
  `maxActiveTasks` 那道闸更宽**(闸只数 `active`/`winding_down`,而 `blocked` 恰恰是成员
  最需要看见的那一个)。
- **投影 `longRunForUser`**(host `me-panel-data.ts`,三态诚实同族)两处折叠承重:
  ①**sticky 的 `waitingForChildren` 必须配 `pending > 0`**——那面旗是刻意 sticky 的
  (M1 的判断),直接信它会让卡片永远写着「在等」;而 `standby` 按构造非 sticky(在武装
  下一段的同一次 mutate 里清),两者**刻意相反**,故投影只对前者加第二道守卫。
  ②**no silent caps**:计划/子活/日志三处截断各自带着 `planTotal`/`childrenTotal`/`more`,
  截了就说。自由文本按**码点**截断,增补平面的字不会被劈成两半。
- **路由 `GET /api/me/panel/data/longrun`**:`longRunForUser` 是 `MePanelDataSurface` 上的
  **可选**方法——老 host 身上根本没有它,这条路答 `{available:false}` 而不是 500(与
  `readContent` 同姿态),且鉴权/方法两道门排在派发**之前**。`?limit=` 是 1..10 的展示
  参数(上下界逐字镜像面板 schema 的 `limit`),写坏了退回观察者自己的缺省——**一个写坏的
  展示参数不该把整张卡弄空**(`?range=EVIL` 判例);`userId` 永远由会话钉死。
- **组件 `longrun-list`** 进闭集(12 → 13)、`longrun.mine` 进数据源白名单,并**进
  `DEFAULT_PANEL`**——默认就在,不必先去配一遍面板才看得见。
- 顺手把 dossier 根目录抽成一个有名字的叶子 `butler-space-dirs.ts`
  (`butlerLongRunRoot(memoryRoot)` = 记忆树的**兄弟**目录,MU-M5 的 git 快照因此不被搅动):
  factory 与投影现在都要推这条路径,两处各自 `join(dirname(memoryRoot),'longrun')` 迟早会
  不一样,而那天没有人会被通知。

**五道变异五次全红且只红该红那些**:①live 不再豁免截尾⇒恰 2 例[两条排序/截尾测] /
②观察者改名隔离坏档⇒恰 1 例[「坏档只是跳过——不隔离、不改名、一个字节不写」] /
③`more` 恒 0⇒**跨两包共红 3 例**(personal-butler 2 + host 1,重建 dist 后)=**这条缝
端到端接通的硬证据**,不是只在纯核里自娱自乐 / ④sticky 旗不配 `pending>0`⇒恰 1 例 /
⑤`?limit=` 不做白名单⇒恰 1 例;复原一律 python 精确替换 + `shasum` 对拍,三个源文件
收工后与基线逐字节相同。

**排错记**:host 夹具把 `memoryRoot` 直接设成 `mkdtemp` 的结果,而 `butlerLongRunRoot` 取
的是它的**兄弟**目录⇒所有测试共享同一个 `$TMPDIR/longrun`,跨测污染(5 failed)且跑完
还漏在盘上。修=夹具镜像生产布局,给 `memoryRoot` 一个自己的父目录。**派生路径的夹具,必须
把它派生出来的那一头也圈进沙箱。**

验收:personal-butler **337**(+7)、host **3292**+5skip(+8)、web **1690**(+4)、全仓
`pnpm -r typecheck` 净、四门 PASS(**旋钮 114 零新增**——一张卡不是开关;那个数字自
`34219a4` 删掉出站 ACP 适配器后从 116 降到 114,本刀未动它;main.ts 2742/2760 未动,接线
是既有 `panelData` 那只鸭子)。sw CACHE v22→**v23**。

**显式不做**:逐步回放(Manus 的 replay 要记录每一次工具调用的时序,而 journal 记的是段级
事实——做回放得先改 journal 的粒度,那是另一个里程碑);成员侧「取消这项任务」的按钮
(`cancel_longrun_task` 已是一件工具,再开一条零 LLM 的命令面是同一道闸的第二个执法点);
推送「这一段跑完了」(段是内部节律不是成员关心的事件,真要播报该按里程碑不按段)。
