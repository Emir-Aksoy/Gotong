...
- [`STOREFRONT-OPS.md`](STOREFRONT-OPS.md) — **门店运营 hub 一页纸**（奶茶店 `cafe-ops` + 酒吧 `bar-ops` 两样板：每岗位知识 / 排班请假 / 临时工薪 / 库存 / 合规五件事全落既有机制；工资「LLM 建议、人定钱」+ 酒吧年龄核查合规 `when` 门控）
...
- [`WORKFLOW-SCHEDULES.md`](WORKFLOW-SCHEDULES.md) — 定时工作流（零 LLM 调度：到点跑 + 跑完 IM 播报）
