...
> 给完全没有技术背景的小白：拿到一个文件夹、双击一个文件、AipeHub 就跑起来了，
...
```
pnpm --filter @aipehub/host deploy --prod <bundle>/app
```

这条命令产出一个**去符号链接、真实**的 prod `node_modules`：原生 addon
（含预编译 `better_sqlite3.node`）+ 全部 `@aipehub/*` workspace 包 + 动态按名加载
...
产物落在 `dist-portable/AipeHub-<平台>-<架构>/`（本机 macOS arm64 →
`AipeHub-macos-arm64/`）。脚本末尾会**用包内 pinned node 真机启动一遍**已部署的
host，断言 `/healthz` 返回 200、identity 已 bootstrap、SQLite 数据存储插件已就绪
——跑不过就直接报错、不产出一个坏包。

把整个 `AipeHub-macos-arm64/` 文件夹（或 `.tar.gz`）发给任何人即可。

### 小白：双击即跑

1. 拿到 `AipeHub-macos-arm64` 文件夹（解压如果是 .tar.gz）。
2. 双击里面的 **`AipeHub.command`**。
   - 首次双击 macOS 可能拦截（「未识别的开发者」）→ 右键点它 → 打开 → 打开。
     一次性信任提示，之后双击就行。我们**故意不做签名 .app**（签名重，数据路径
     完全一样）。
3. 浏览器自动打开 `http://127.0.0.1:3000` 的 5 分钟设置向导。完事。

数据存在 `~/.aipehub`（**在包外**）——删掉 / 换一个新版本的包，数据不丢。
...
```
AipeHub-macos-arm64/                  ← 发给小白的就是这个文件夹
├── AipeHub.command                   ← 双击它（来自 deploy/AipeHub.command，含 tier-0 分支）
...
└── app/                              ← pnpm deploy --prod 的输出 = host 包根
    ...
    ├── bin/aipehub-host.js
    └── node_modules/                 ← 完整 prod 闭包：@aipehub/*、better-sqlite3(预编译)、ws、im-*
```
...
便携包不另起一个 launcher——给**现有**的 `deploy/AipeHub.command`（+ 双胞
`deploy/AipeHub.sh`）**加性前置**一个最高优先级的「tier-0 自包含」分支：
...
- **不在便携包里**（比如从源码 checkout 里双击）→ 这两个文件不存在 → 分支不命中
  → fall-through 到既有的 repo-checkout / `aipehub` CLI / `npx` 逻辑，**对源码
  用户逐字节不变**。

一份 launcher 同时服务两种场景，免分叉，复用既有的健壮 `SCRIPT_DIR` 解析
（Finder 双击时工作目录是 `$HOME`，这段也能正确定位文件自身位置）、
`AIPE_OPEN_BROWSER=always` 默认（host 监听后自己开浏览器，无竞态）、以及
`launch()` 里的 `AIPE_LAUNCH_DRY_RUN` 可测缝。
...
| Linux x64 | 在 Linux 上跑（launcher 用 `AipeHub.sh`） | 推迟 |
...
- **不做签名 `.app` / notarization。** Gatekeeper 首次右键「打开」一次即可，
  数据路径与签名版完全相同（沿 `deploy/AipeHub.command` 既定立场）。
- **跨平台 / 体积优化 / 自动更新**：见上表与第五节，均显式推迟。便携包升级 =
  整包替换（数据在 `~/.aipehub` 包外，不受影响）。
...
1. 用**包内 pinned node** 跑**已 deploy 的 host**，临时 `AIPE_SPACE` + 随机端口 +
   `AIPE_OPEN_BROWSER=0`。
2. 轮询 `/healthz` 断言 **200**。
...
launcher 那一层另有 dry-run smoke（`AIPE_LAUNCH_DRY_RUN=1`）覆盖四例：
...
- 双击 launcher：[`deploy/AipeHub.command`](../../deploy/AipeHub.command) ·
  [`deploy/AipeHub.sh`](../../deploy/AipeHub.sh)
