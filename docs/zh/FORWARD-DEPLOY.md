...
> §五的**连接器槽位（`requires.connectors`）已落地**（FDE-M1a/M1b）、
> **验收用例（`acceptance[]`）已落地**（FDE-M2，见 §五「已实现」小节）——
> 落地形态比草案更收敛：都直接作为 `gotong.template/v1` 的可选块，**没有**
> 新 schema id；调度建议块仍是草案（M3+）。
>
> Last updated: 2026-07-04 · FDE-M2
...
## 五、方案包 schema：已实现的槽位块 + 验收块 + 剩余草案

> **状态：`requires.connectors` 已实现（FDE-M1a/M1b）、`acceptance[]` 已
> 实现（FDE-M2）**；`schedules` 一块仍是草案（M3+ 目标，字段名允许改）。
> 落地时对草案做了一个关键收窄：**没有** `gotong.solution-pack/v1` 这个新
> schema id——`requires` / `acceptance` 直接是 `gotong.template/v1` 的可选
> 块（与 `provenance` 同级同性质），旧 host 认不出就整块忽略，优雅降级是
> 结构自带的而不是约定的。
...
### 剩余草案（M3+ 目标：`schedules` 块）

沿用同一落地方式：作为 `gotong.template/v1` 的可选块加上去，旧 host 整块
忽略。一条不动的原则：**不带人员**——调度建议里没有 `userId`（晨报先例
延伸）：装完在定时卡把人补上，语义 = 该成员自己点了「运行」。

```yaml
# 与 requires / acceptance 同级的未来可选块（字段名收口前允许改）：
