...
> §五的三个可选块**全部落地**：连接器槽位 `requires.connectors`（FDE-M1a/
> M1b）、验收用例 `acceptance[]`（FDE-M2）、调度建议 `schedules[]`（FDE-M3，
> 连同「开荒一条命令」`gotong provision`）。落地形态比草案更收敛：都直接
> 作为 `gotong.template/v1` 的可选块，**没有**新 schema id。
>
> Last updated: 2026-07-06 · FDE-M3
...
| 用什么 | 干什么 |
|---|---|
| 模板 `acceptance[]` 黄金用例 + admin「跑验收」（FDE-M2，§五「已实现」） | 装完一键真跑：逐条走**同一道成员闸**派发（以点按钮的 admin 身份）、await 到 run 完成、`@gotong/evals` 零 LLM 判卷，出绿/红清单（红行带逐条 violation） |
...
| 真实用例 3～5 条 | 让流程主人给**真输入**（真的今天的报价单 / 真的 focus 主题），逐条跑，对着输出确认——模板带的黄金用例是底线，不是替代 |
| `@gotong/evals` 结构检查 | 输出的结构性断言（必要小节 / 必要短语在不在、禁语出没出现、超没超长），零 LLM、确定性、CI 可挂 |
...
**过关判据**：黄金用例全绿 + 真实用例全绿 + run 归属正确成员（审计里可查）
+ 结果真的到了该到的人（IM / inbox）。
...
| ~~行为验收靠人肉：evals 是独立包，publish 前只有结构校验，没有「在附带用例上跑一遍」~~ | ~~模板可附带 golden cases，装完 admin 工作流页一键真跑（复用试跑成员闸 + evals 检查器，零 LLM 判卷）~~ | **FDE-M2 ✅ 已收口**（§五「已实现」） |
| ~~「按方案开荒」未一条命令化：起 hub 之后装模板、建调度、跑验收全是手工续段~~ | ~~`gotong provision <pack> --user <成员>` 一条命令：装包 → 按模板 `schedules[]` 建议补人建真调度 → 跑黄金验收 → 绿/黄/红开荒报告~~ | **FDE-M3 ✅ 已收口**（§五「已实现」） |
...
## 五、方案包 schema：三个可选块全部已实现

> **状态：`requires.connectors`（FDE-M1a/M1b）、`acceptance[]`（FDE-M2）、
> `schedules[]`（FDE-M3）全部已实现**。落地时对草案做了一个关键收窄：
> **没有** `gotong.solution-pack/v1` 这个新 schema id——三块直接是
> `gotong.template/v1` 的可选块（与 `provenance` 同级同性质），旧 host
> 认不出就整块忽略，优雅降级是结构自带的而不是约定的。
...
### 已实现（FDE-M2）：验收用例 `acceptance[]`

在任何 `gotong.template/v1` 模板上加（与 `requires` 同级）：

```yaml
acceptance:                     # 验收用例（golden cases，零 LLM 判卷）
  - id: smoke-brief             # 用例名（name-identity 同款字符集，包内去重）
    workflowId: morning-brief   # 必须指向模板自带的 workflow（typo 装时就报，
                                #   否则会变成一颗永远红着怪 hub 的用例）
    trigger: { focus: 高效开始这一天 }   # 真实派发的 payload（scope key 除外）
    assert:                     # 至少一条真断言（零断言=永远绿=假验证，装时拒）
      contains: [今日重点, 提醒, 今日一学]   # 必要短语（evals requiredPhrases）
      forbid: [作为一个AI, 我无法访问]        # 禁语；另有 sections / maxBytes
    note: 开箱无连接器状态下也必须绿（诚实模式即合格线）
```

四段链路（与槽位块 M1a/M1b 完全同构，每段的守门测试都在）：

1. **解析**（`web/src/template-manifest.ts`）——大声校验：id 合法且去重、
   `workflowId` 必须在模板自带 workflow 里、`trigger` 必须是对象、`assert`
   至少一条真断言；纯验收包仍按空模板拒绝。词汇直接映射 `@gotong/evals`
   结构检查器（`sections`→requiredSections / `contains`→requiredPhrases /
   `forbid`→forbiddenPhrases / `maxBytes`），M2 顺手给 evals 补了
   `requiredPhrases`（晨报用编号小节而非 `##` 标题——改检查器，不倒着改
   模板输出格式）。
2. **装时报告**（`templates/import` 响应）——`postInstallChecklist.
   acceptanceCases[]`（id + workflowId），画廊卡装前可见「这包会自证」。
3. **装后持久化**（host `template-acceptance.json`）——姿态与槽位文件
   逐字同款：只存意图、按模板名 last-install-wins、重装掉了 `acceptance`
   的模板自动清条目、损坏 → 警告 + 当空，绝不拦装。
4. **跑验收**（admin 工作流页「验收」卡 / `POST /api/admin/templates/
   acceptance/:pack/run`）——逐条用例真跑：走 **`evaluateRunnable` 同一道
   成员闸**（published + 成员面开 + 角色 + scope key 强制 = 点按钮的人），
   与 /me「运行」、管家 `run_my_workflow`、定时 sweeper 完全同闸——验收
   做不出点按钮的人自己做不到的事；身份永远来自 session，绝不来自请求体。
   与其他派发路径都是 fire-and-forget 不同，验收 **await 到 run 完成**
   （默认 120s 超时，超时红牌但 run 继续后台跑），拿 finalOutput 抽文本 →
   evals 判卷 → 绿/红清单，红行带逐条 violation（缺哪个短语、犯哪条禁语）。

判定映射五种红：`unrunnable`（成员闸没过 / workflow 没装）、
`dispatch_failed`（run 本身失败）、`timeout`、`suspended`（黄金用例必须
无人值守——会停下等审批的用例不该当验收）、`assert_failed`（跑成了但断言
不过）。模板真身示例照旧：`examples/morning-brief-hub`（其 smoke-brief
的合格线 = 零连接器的诚实模式输出）。

### 已实现（FDE-M3）：调度建议 `schedules[]` + 开荒一条命令 `gotong provision`

在任何 `gotong.template/v1` 模板上加（与 `requires` / `acceptance` 同级）：

```yaml
schedules:                      # 调度建议（只带节奏，不带人员）
  - workflowId: morning-brief   # 必须指向模板自带的 workflow（typo 装时就报）
    cadence: { kind: daily, hour: 8 }   # 与定时卡同一词汇：daily/weekly/interval，
                                #   过的是 host sweeper 同一个归一化器（半解析=拒）
    note: 装完在「定时」卡补人启用, 每早 8 点自动跑   # 可选；另有可选 inputs
```

一条不动的原则落成了解析器的硬拒绝：**模板带 `userId` 直接拒装**——
templates bring structure, never people；人从定时卡「补人启用」按钮或
`gotong provision --user` 进来，语义 = 该成员自己点了「运行」。

四段链路（与前两块完全同构，每段的守门测试都在）：

1. **解析**（`web/src/template-manifest.ts`）——大声校验：`workflowId` 必须
   在模板自带 workflow 里、`cadence` 必须过 **host sweeper 同一个归一化器**
   （`@gotong/workflow` 的 `normalizeScheduleCadence`，为此把 cadence 纯核
   从 host 挪进 workflow 包——web 不得依赖 host，复制校验器就是第二份会漂
   的真相）、带 `userId` 拒、同 workflow 同 cadence 的完全重复拒；纯建议包
   仍按空模板拒绝。
2. **装时报告**（`templates/import` 响应）——`postInstallChecklist.
   scheduleSuggestions[]`，同姿态：报告，绝不自动建行。
3. **装后持久化**（host `template-schedule-suggestions.json`）——姿态与前
   两个文件逐字同款：只存意图、按模板名 last-install-wins、重装掉了
   `schedules` 的模板自动清条目、损坏 → 警告 + 当空；载入时 cadence 重跑
   同一个归一化器，手编辑坏了的条目掉行不猜。
4. **定时卡建议行 + 一键补人**（admin 工作流页「定时」卡 /
   `GET /api/admin/workflow-schedules/suggestions`）——建议行显示「模板 X
   建议:morning-brief 每天 8:00」，点「补人启用」填成员 id 后走**同一个
   POST upsert**落成真调度行（成员闸照常）；已有同 workflow 同 cadence 真
   行的建议显示「已有同频定时」徽章。建议永远不会自己开跑。

**开荒一条命令**（把「起 hub 之后装模板、建调度、跑验收全是手工续段」压平）：

```bash
gotong provision <pack.yaml> --url http://127.0.0.1:3000 --token "$ADMIN_TOKEN" \
  --user u-alice          # 可选：把模板的调度建议补人启用
  # --skip-acceptance     # 可选：不跑黄金用例（省 token，报告黄牌记一笔）
```

全走 admin HTTP API（远程 hub 与本机一视同仁）：装包 → （`--user` 时）逐条
建议 POST 成真调度 → 跑 pack 自带黄金验收（await 到 run 完成，零 LLM 判卷）
→ 绿/黄/红开荒报告。出码脚本可依赖：0 绿或仅黄 / 1 用法 / 2 装失败 /
3 装上了但没到位（工作流落地失败、建调度失败、验收红，红行逐条 violation）。
云上一步到位：`deploy/cloud-quickstart.sh --pack <pack.yaml>` 起完 hub 顺手
开荒（见 [`GO-LIVE.md`](GO-LIVE.md)）。模板真身示例照旧：
[`examples/morning-brief-hub`](../../examples/morning-brief-hub)（带全部三块）。

明确**非目标**（三块共守）：不是包管理器（无版本依赖解析、无自动升级）、
不装人、不带 key、不引入新权限面。
