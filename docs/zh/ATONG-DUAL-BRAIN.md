...
- **M4 生产部署 ✅(2026-07-18 落地)**:生产编排 = 阿同(接待)保持
  LongCat-2.0 + `thinking:'disabled'`,新建兄弟专家 agent 同 LongCat +
  thinking 默认/enabled,`escalateTo` 指专家,两行共用 `apiKeyEnv:
  LONGCAT_API_KEY`(**不再需要 DeepSeek key**);persona 贴下节文案(落在
  `managed.system`)。**部署时抓到的真缺口**:`escalate_to_expert` 的
  fail-closed 校验走 `roster.listOwned(userId)` = identity `resource_grants`
  表的 owner 行——org 级 agents.json 行**没有** grant,不补当场拒转派;已按
  operator-agent-service `seedOwner` 同款 INSERT 补上(活库是
  **identity.sqlite** 不是 identity.db,备份先行)。
- **M4 后诊断实录(2026-07-18,用户报「没看到两段式」)**:全链路逐环核
  (persona 在 prompt 根部[`managed.system`→`agentOpts.system`→butler 只追加
  不替换]/工具一等位[butler-tool-tiers 名单]/grant 在库/expert 已 spawn)全对,
  transcript 里 escalate **零触发** = 任务判定,不是链路——用户测的新闻热点类,
  阿同用 WSE 自带 Tavily 就地搜了答(日志 `kind=tool_use` 为证),符合纪律
  「拿不准先直接回答」。**LongCat-2.0 直探针三发全中**(服务器真 key,单工具+
  同款纪律):清晰重活(3000 字多来源调研)在 thinking **disabled** 下自动
  「回执文本 + 调 escalate_to_expert」同轮出手,流式 `delta.tool_calls`
  形状与 llm-openai 解析器逐字吻合;显式指令与 enabled 对照同过。**用户拍板
  保持现状**:搜索能答的就地答(快),两段式留给真重活;「搜索汇总类划进重活」
  选项被否,别再提议。触发验证语句=「帮我写一份 3000 字的深度调研报告:…
  要多来源交叉验证」。
