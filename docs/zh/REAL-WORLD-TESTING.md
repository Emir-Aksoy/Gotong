...
> ⚠️ 没记住 token 也别慌：删掉 `.aipehub/` 目录重启即可（会丢空间数据）。
> 或者用 `AIPE_RESET_TOKEN=1 pnpm host` 重置 admin（保留其它数据）。
...
如果以上**有任何一项不在**，说明 `.aipehub/` 目录被外部清了或者
启动参数指向了别的 space —— 排查重点是 `AIPE_SPACE` 环境变量。
...
```bash
AIPE_HOST=0.0.0.0 \
AIPE_ALLOWED_HOSTS="192.168.1.42:3000,localhost:3000" \
ANTHROPIC_API_KEY=sk-ant-… \
pnpm host
```

> `AIPE_ALLOWED_HOSTS` 是 CSRF 防护：派发 / 创建 agent 这类
...
|页面打开但点 + 创建 agent 报 403|`AIPE_ALLOWED_HOSTS` 没把客户端用的域名 / IP 加上|
...
- [ ] `AIPE_HOST=127.0.0.1`（让 Caddy / nginx 反代而不是直接暴露 3000）
- [ ] `AIPE_ALLOWED_HOSTS=hub.example.com` —— 把对外域名写进来
- [ ] `AIPE_COOKIE_SECURE=1` —— 配合 HTTPS，cookie 只走 https
- [ ] `AIPE_GATING=admin-approval` —— **不要**改成 `open`（生产没必要）
- [ ] `AIPE_ADMIN_RATE_MAX=10`（默认值就行，别关）
- [ ] **TLS 走 Caddy 自动 Let's Encrypt** 或 nginx + certbot
- [ ] `AIPE_SECRET_KEY` 主密钥**离机备份**了
  （丢了就解不开 `secrets.enc.json` 里的 API key —— 看 `SECURITY.md`）
- [ ] `.aipehub/` 整个目录在 systemd 配的 `WorkingDirectory` 之下，
...
sudo journalctl -u aipehub -n 200 | grep -iE 'sk-(ant|proj)|token=|api[_-]?key'
...
- 看 `.aipehub/transcript.jsonl` 大小增速是否合理（一个普通 room
  通常每天 < 5 MB；如果某天突然 200 MB+，要查是不是有 agent 死循环）
- 看 `journalctl -u aipehub` 里有没有 `[supervisor] respawn` 反复出现
...
报告 → 归档 → 改口径"全套，AipeHub 把里头**最体力活**的部分
...
接入**路线 —— 用 `@aipehub/sdk-node` 或 `aipehub` (python) 自己写
...
拿到一个新 hex → 改 `.aipehub/admins.json` 里那一行的 `token` 字段 →
