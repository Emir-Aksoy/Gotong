...
连接器的 **benign 读工具**(天气/日历/新闻),把真实数据自然融进问候。MCP 的 WRITE 半是
会 park 的 governed 闸,**绝不**交给无人值守的 sweep —— 对外部世界,早报能看不能动。
连接器挂了 / 解析不到 / 开关关 全部退回历史单次无工具路径,逐字节一致;工具抛错变
isError 结果模型自愈,SKIP 仍表示保持沉默。

**晨报写面板卡小刀(2026-08-02 追加)**:enrich 循环额外获准一个**内容-only** 面板工具面
(`list/read/write_panel_content` 三件,`buildButlerPanelContentToolset`)——查到的真实
天气/新闻可以顺手整理进本成员自己的面板中转卡(如 `connector.weather`),卡上自动标注
「阿同整理 · 更新于 X」。边界:写的只是**仅本人可见的展示文本**(与会话面同一套 benign
三段式论证,userId 构造时闭包钉死),**布局两件(`get_my_panel`/`set_panel_layout`)结构性
不进无人值守循环**——不只不 advertise,按名点也拒(重排有快照/横幅/撤销的会话语境,sweep
没有);MCP WRITE 半照旧不进。带面板面的循环上限 5 轮 / 800 token(只读档仍 3 轮 / 300);
面板店解析失败或缺席 ⇒ 降级回历史只读循环。零新旋钮:骑 enrich 既有 per-member opt-in,
面板 surface 在不在就是开关。
