...
> 信任契约可编辑并真正生效（入站 ACL + 出站审批走 Phase 16 inbox）；③ 两个 AipeHub
...
| **C A2A 闭环** | R1/R3 只到「声明 auth scheme + agent card」，card 故意 `skills:[]`，没有真正的 message API | agent card skills opt-in 翻 honest + 新包 `@aipehub/a2a`（wire 类型 + client）+ 入站 `message/send`→`hub.dispatch` + 出站 `A2aRemoteParticipant` + 双 hub 验收门 |

合计 13 个里程碑 commit（A-M1→A-M3、B-M1→B-M3b、C-M1→C-M5）+ 本文档。新增第 29 个
workspace 包 `@aipehub/a2a`。
...
- **人是 Participant，不是审批 tool**：出站跨组织 task 命中 `requireApprovalOutbound`
  的 peer → 抛 `SuspendTaskError` 停在成员 inbox（cap `aipehub.human/v1` 同源的复用），
  人在 `/me` 批准后才真正发出。**零新挂起设施**——全靠 Phase 11 suspend/resume + Phase
  16 inbox 两步恢复拼出来。
- **mesh 留高信任自有通路，A2A 拿生态可达性**：manifest 走 mesh RPC（AipeHub↔AipeHub，
...
`main.ts` 从 `buildLocalManifest`（A-M1）派生 skills，但 `AIPE_A2A_ADVERTISE_SKILLS`
默认关。capabilities 三 flag **仍全 false**——我们只做 blocking `message/send`，不做
streaming / pushNotifications / stateTransitionHistory，诚实不吹。

### 5.2 `@aipehub/a2a` 共享包（C-M2，`e1a9d18`）

第 29 个 workspace 包，仿 `@aipehub/inbox` 的小聚焦包：
...
- **认证（自有 bearer 域，不走浏览器 admin session/CSRF）**：caller 带
  `X-Aipe-Peer-Id` + `Authorization: Bearer <pre-shared token>`，host 侧
...
- web 路由 `/a2a` + `/a2a/message` 在 agent-card 之后、**CSRF 闸之前**（自有 bearer 域），
  鸭子 `A2aServerSurface` 注入（web 零 identity dep）。`AIPE_A2A_INBOUND_ENABLED` 默认关。
...
无 bespoke error plumbing。main.ts `registerOutboundA2aAgents` 从 `AIPE_A2A_AGENTS` JSON
...
Hub A: dispatch capability `translate`
  → A2aRemoteParticipant（出站）
    → POST http://127.0.0.1:<port>/a2a  （真 socket）
      → Hub B A2aServer（入站）—— bearer + X-Aipe-Peer-Id 验证
...
| 包 | 总数 | 本 phase 新增 |
|---|---|---|
| `@aipehub/a2a`（新） | 15 | 10 client + 5 participant |
| `@aipehub/host` | 419 | peer-manifest / outbound-approval(+e2e) / a2a-server / a2a-double-hub |
| `@aipehub/web` | 483 | peer-routes / identity-routes-peers policy / a2a route |
| `@aipehub/identity` | — | peer-store policy round-trip + v11→v12 迁移 |
| `@aipehub/core` | 294 | wrapOutbound hook |
...
| env | 默认 | 作用 |
|---|---|---|
| `AIPE_A2A_INBOUND_ENABLED` | `false` | 开入站 A2A `message/send` 端点（暴露 hub 给外部 A2A caller，**默认关**） |
| `AIPE_A2A_INBOUND_CAPABILITY` | （无） | 入站消息无 `metadata.skill` 时的 fallback 派发 capability |
| `AIPE_A2A_ADVERTISE_SKILLS` | `false` | agent card 是否枚举本地能力为 skills（public 端点，默认不枚举） |
| `AIPE_A2A_AGENTS` | （无） | 出站 A2A agents 的 JSON 配置，token 从各 entry 的 `tokenEnv` 指向的环境变量读 |
...
- per-link quota budget；对真实外部（非 AipeHub）A2A agent 的 wire 级互操作测试。
...
| C-M2 | `e1a9d18` | 新包 `@aipehub/a2a` —— message/send wire 类型 + client |
| C-M3 | `87b21e0` | 入站 `message/send` HTTP 端点 → hub.dispatch（fail-closed 认证） |
| C-M4 | `31430c9` | 出站 A2aRemoteParticipant + AIPE_A2A_AGENTS 接线 |
