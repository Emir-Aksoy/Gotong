...
> **状态**：**全部里程碑收口（2026-09-02，M0 → M5）**。可跑的证据：`pnpm demo:memory-economy`（[`examples/atong-memory-economy`](../../examples/atong-memory-economy)）；四道记忆门 `check:memory-recall` / `-write` / `-integration` / `-eviction`。
...
**⑥ 写侧折叠尺**（M4，住 `check:memory-write`）
两面，缺一面就假：**该折的折了**（`foldRate`）与**不该折的一条没少**（`keepRate`）。一个把什么都折掉的门在前者 100%、后者 0；今天（没有门）恰好相反。所以 `keepRate` 的地板从第一天起就是 1.0 —— 省字节永远排在不丢记录后面，尺子的形状先把这句话钉死。

两条与逐出尺不同的选择，各有理由：①**跨用例累计计数，不逐用例求平均** —— 夹具里有几条用例是故意一条都不该折的（短回合陷阱、只差一个词的两件事），逐用例平均会让它们的 `foldRate` 记成「空洞地满分」，于是**基线凭空得到一堆 1.0**；逐出尺那边可以用平均，是因为那份夹具每条用例的 `shouldKeep`/`shouldDrop` 都非空。②**夹具用 `buildTurnCapture` 真会写出来的格式**（`User: …\nButler: …`）—— 拿裸句子当夹具会漏掉最要紧的一件事：正是**管家的回复一起被写进去**，才让「明天的会」和「后天的会」分得开。

已知局限，钉成界碑而不是修：**「周三/周四 + 同一个答案」双向覆盖 0.889，会被折**。要分清哪个词面差异承重需要模型，而新颖门的立身之本正是零模型。代价的边界是量得出来的 —— 误折要求**两半都**近乎一样；答案一变（0.563）当场掉到阈值以下。丢的是「那天也问过」，不是「答案是什么」。
...
| M4 | 新颖门（写侧零 LLM 折叠） | 1 天 | ✅ 2026-09-02 `070f8e0f`——**该折的折掉 0% → 100%**、**该留的留住 100% → 100%（守住）**；判据必须**双向**（`relevanceScore` 是不对称的查询覆盖率，单向会让任何短文本被任何长文本吞掉），阈值仍与 `DEFAULT_FACT_DEDUP_THRESHOLD` 同源——变严的是比法不是那个数；只挂 turn capture 一处，profile/digest·atomic-facts·模型自己的 `remember` 刻意不挂；fail-**open**（与 M3c 相反：丢一轮对话比少回收字节严重）；字节账查出折叠付**一次性常量**、省**每次复述的线性项**，极短条目恰好复述 2 次时反亏 13 字节，且增长是**对数**不是恒定（计数器占位）；变异十三发全中，其中 N8-B / N8-K **首轮没红**且都是真问题（阈值没被夹具咬住 · 断言量错了守卫买到的东西） |
| M5 | capstone `examples/atong-memory-economy`：万轮不膨胀 / 跨店召回 / 用则存不用则忘 | 1 天 | ✅ 2026-09-02 `f8efdcae`——一万轮、四十件事、六成复述，两条路同种子同剧本。**先声明哪条不算 M4 的功劳**：「字节 ≤ 预算」「条数常量级」两边都成立，那是 `enforceBudget` 早有的；M4 挣到的是密度——写入 10000 → 2001（5 倍）、盘上零重复（13 条正好 13 件事，今天 25 条只有 10 件）、召回 20 格够得着 9 → 13 件、逐出 9975 → 1988。**正题在第二幕，且是阳性对照逼出来的**：原本只灌经济那边量出「不降」，那可能只是夹具伤不到；补上今天那边之后——今天沉淀的 25 条把唯一那条过敏记忆挤出前五，**单店召回 100% → 0%**，经济的没有。第三幕显著性 2.166 对 0.750。一个数**如实报但不当成绩**：记忆账在这个剧本里分不出两条路（都常年第 ④ 级），原写的「顶格 100% → 98%」是空洞的胜利，删掉换成逐出量 |

一个里程碑一个 commit，每刀配变异测试，数字一律量出来的。

**收口时的一句总账**：这条 track 一共立了四把尺（跨店整合 / 逐出 / 降温 / 写侧折叠），动了四把刀（网 · 账 · 门 · 钟），**一个删除执法点都没新增**——压力驱动的自动逐出仍然只有 `enforceBudget` 那一处。全程热路径零 LLM。变异一共打了六十余发，其中**六发首轮没红**（N-D / N-F / N2-B / N4-F / N5-D / N8-B / N8-K），每一发查下去都是真问题：不是夹具空洞，就是断言量错了东西。这个比例本身就是一条结论——**没红的那几发比全中的那几十发有用得多**。
