...
| M3c | ③ 降温：最低显著性的**散装事实**翻篇（不硬删）+ 剪死链 | 半天 | ✅ 2026-09-02 `9e4a34f`——**该逐的逐掉 25.0% → 100.0%**、该留的留住 89.1% → 100.0%（基线是**M3b 通电后的今天**，不是「什么都不开」，否则量到的是上一刀的功劳）；只翻 `levelRank === 1` 那层，借用逐出那处的判断不另写一份——翻 digest/profile 会把最受保护的送进过期带即队列最前面，**倒置分层保护**；四道守卫（钉住/已翻篇/未来事实/保护期）各配用例；变异十发全中；顺带查出 `pin-never-cools` 那条尺子**量不到**钉住守卫（显著性排序已经挡在前面），如实改写用例说明 |
| M3d | ③ 降温·知识库那一面：文件归档（缓解**货架数**，不减字节）+ 冷度信号 | 半天 | ✅ 2026-09-02 `face551`——判据是**后果**：货架撞顶时 `write` 抛 `knowledge_limit`，跑一次维护**同一个 `write` 就成功了**；目标瞄**降级线之下一档**而不是上限——`write` 在第 200 份就当场抛，上架数永远 ≤ `maxFiles`，照抄记忆那一面的「超出多少」会得到一段**永远为零、每道门都绿**的死码；冷度用 `mtime`（walk 本来就在 stat，零额外系统调用），如实写明它**跟踪写不跟踪读**，敢用是因为猜错只赔一个 `archive/` 前缀；共用 `rungFor` 但自算货架压力（字节制的 `LedgerLine` 装不下货架数）；变异十二发全中。**顺带纠正 M3c 的一处事实错误**：「全仓恰好一把剪刀」是错的（`grep '\.forget('` 十几处），准确说法是「压力驱动的自动逐出只有 `enforceBudget` 一处，记忆经济这一层一处都不加」；并查明字节那一面**早有人管**——`space-retention.ts` 的 `memory_archive_days` 剪的恰好就是 `knowledge/archive/**`，M3d 与它是**接力**不是缺口 |
