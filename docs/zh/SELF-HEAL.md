...
>
> Last updated: 2026-08-01 · 状态:**全完**(M1 hub 侧 + M2 deploy 三件套 + M3 生产安装,SIGSTOP 卡死演练 2026-08-01 生产实测通过:冻结→1/3→2/3→3/3→自动拉起全程 2 分 40 秒,台账三行齐 + M4 自愈事件 IM 事后播报)
...
- **IM 主动播报(M4)**:巡检 sweep(CARE-M3,10 分钟节律)顺带读台账,
  发现**新的**看门狗重启 / 限流 / 非正常停止行,就给开了「运行播报」的成员
  推一条**事后**通知(「🛠 08-01 13:42 hub 曾卡死…看门狗已自动重启救回」),
  同一份同意面(`set_run_broadcast`,默认关)、同一条投递管道(outbox 补投)。
  三条边界:①**事后叙述**——hub 卡死期间自己什么都发不出(看门狗刻意零 IM
  凭证),消息落在恢复后的第一轮巡检,文案全部过去式;②**高水位去重**
  (`patrol-state.json` 记 `selfHealAnnouncedThrough`),事件是过去的事,
  状态文件损坏时**宁漏不刷**(重新基线不倒灌历史)——与巡检卡「宁重不漏」
  刻意相反;③首次接入只立标不播,历史行留在面板里,IM 只报此后新发生的;
  `journalTail` 永不进 IM(低信息纪律,细节在面板和 `restart_history`)。
