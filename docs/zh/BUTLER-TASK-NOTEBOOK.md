...
> 能力** —— 全部落在管家层,内核零改动。
>
> Last updated: 2026-07-08 · **track 全完**(M0 计划 / M1 纯核+工具+复述缝 /
> M2 卡壳提醒 / M3 capstone)。
...
| **TN-M3** | capstone `examples/butler-task-notebook`:「失忆」provider 每轮只做一步、全靠 digest 知道进度,证 5 步使命跨 5 轮独立 task 完成 + 重启不丢 + governed 步照 park;self-assert exit 0 + 文档收尾 | **完**(`74d513d`) |
...
### TN-M3 —— capstone 弱模型演示 + 收官(2026-07-08,`74d513d`)

- **`examples/butler-task-notebook`**(host-free,零 key 零前置,自断言 exit 0):
  故意失忆的脚本 provider 驱动**真** `PersonalButlerAgent` —— 每个成员轮
  **全新构造** provider / 笔记本 store / agent(= 每轮都是重启)、
  `captureTurns: false` 无记忆拐棍、**结构性断言**每个成员发起的新轮首次模型
  调用只带 1 条消息(没有任何跨轮会话被携带);进度唯一来源 = TN-M1 复述缝
  注进 system prompt 的【任务笔记本】摘要。**5 步使命跨 6 个独立轮完成**。
- **四幕**:① 开任务落盘(开任务前的轮 system 无卡 —— 空笔记本 = prompt
  字节不变)② 失忆推进 3 轮 ×(读摘要 → 做一步 → 勾掉),逐轮断言摘要
  「下一步」正确 ③ **笔记本≠授权**:第 4 步「给宾客发邀请短信」对外发送,
  park 进审批;批准前断言**未发送、未勾步**;批准后同轮续跑先执行后勾步
  ④ 收尾归档,摘要消失,无任务轮管家如实说闲。
- **排错记**(值得留):digest 模板的括号是 ASCII `(`,demo 初版正则写
  `((\d+)…)` 被当作分组吞掉没转义 → 逐字节 hexdump 定位,显式 `\(` `\)` 修正。
  教训:对模板文本写解析器,先 hexdump 对齐标点的实际码位。
- **注册**:根 `pnpm demo:butler-task-notebook` + `EXAMPLES.md` ②区一行 +
  「记忆/管家」横查条目;README 附 demo↔生产对照表(demo 手写的三样 —— 笔记本
  接线 / provider / 内联审批 —— 生产里分别由 factory / 真 provider /
  HostInboxService 提供)。
- **验收**:demo 全幕 PASS exit 0;四门 PASS(旋钮仍 106,预算零动)。

**TN track 至此全完**(M0 计划 `0d1c507` / M1 纯核+复述缝 `7169d13` /
M2 卡壳提醒 `fb177ba` / M3 capstone `74d513d`)。三条边界全程守住:管家层
不进内核(core/workflow/protocol/identity 零改动)、笔记本≠授权(两处
会红的测试钉死)、笔记本≠第二工作流引擎(无 DAG 无调度,工具描述指路
create_workflow / set_reminder);全程**零新 env 旋钮**(仍 106)。
