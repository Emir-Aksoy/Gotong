...
> 给**已经上线**的 AipeHub 实例补齐 go-live 检查里剩下的红灯 + 黄灯。
> 这些都是**机器上的运维动作**——在你自己的服务器里执行，不是改代码。
> 本文用占位符（`<your-server>` / `~/aipehub/...`），不写任何 IP / 密钥。
>
...
5. 用到的占位符：
   ...
   | `~/aipehub/data` | `AIPE_SPACE` 工作区目录 |
   | `~/aipehub/aipehub.env` | systemd `EnvironmentFile`（0600） |
   | `~/aipehub/app/packages/host/dist/main.js` | host 入口 |
   | `NODE` | 机器上的 node 二进制（如 `/usr/local/bin/node`） |
   | `aipehub.service` | systemd 服务名 |

   下面所有命令假定你已 `ssh ubuntu@<your-server>` 进了盒子。先把入口固定成变量，
   后续命令直接复用（**按你的真实路径替换一次**）：

   ```bash
   export AIPE_SPACE="$HOME/aipehub/data"
   export NODE="/usr/local/bin/node"              # 你的 node 路径
   export HOST_MAIN="$HOME/aipehub/app/packages/host/dist/main.js"
   set -a; . "$HOME/aipehub/aipehub.env"; set +a  # 载入与 host 同款 env
   ```

   > 最后一行很关键：`mint-admin-token` / `rotate` 这类子命令必须读到和**运行中的
   > host 完全一致**的 `AIPE_SPACE` / `AIPE_HOST` / `AIPE_WEB_PORT` / `AIPE_COOKIE_SECURE`，
...
```
  New admin 'Recovery Operator' (adm_xxx) added to ~/aipehub/data/admins.json.
  Admin URL saved to (mode 0o600 — read once and delete):
    ~/aipehub/data/runtime/admin-link.txt
```

取出链接（**只在 SSH 会话里看，别截图**）：

```bash
cat "$AIPE_SPACE/runtime/admin-link.txt"
```
...
把这条命令记进你的运维笔记（或盒子上一个 `~/aipehub/RECOVERY.md`，**只在盒子里**）：
...
```bash
shred -u "$AIPE_SPACE/runtime/admin-link.txt" 2>/dev/null || rm -f "$AIPE_SPACE/runtime/admin-link.txt"
```

### 验证

- `admins.json` 里多了一条记录：`grep -c '"id"' "$AIPE_SPACE/admins.json"`（数变多）。
- 用新链接 + 隧道能进 admin 控制台。
...
拷贝」——一次手抖的 `tar czf data.tgz ~/aipehub/data`、一条 `scp -r data/`、一个将来
...
```bash
KEYHEX="$("$NODE" -e 'process.stdout.write(require("fs").readFileSync(process.argv[1]).toString("hex"))' "$AIPE_SPACE/identity-master.key")"
...
{
  printf 'AIPE_MASTER_KEY_PROVIDER=env\n'
  printf 'AIPE_MASTER_KEY=%s\n' "$KEYHEX"
} >> "$HOME/aipehub/aipehub.env"
unset KEYHEX                      # 立刻从当前 shell 抹掉
chmod 600 "$HOME/aipehub/aipehub.env"
```
...
journalctl -u aipehub -n 30 --no-pager | grep -i 'master key provider'
# 期望看到 source 为 env（而非 local-file）
curl -fsS http://127.0.0.1:${AIPE_WEB_PORT:-3000}/healthz   # 期望 200 ok
```
...
```bash
shred -u "$AIPE_SPACE/identity-master.key" 2>/dev/null || rm -f "$AIPE_SPACE/identity-master.key"
```
...
sed -i '/^AIPE_MASTER_KEY_PROVIDER=env$/d; /^AIPE_MASTER_KEY=/d' "$HOME/aipehub/aipehub.env"
sudo systemctl restart aipehub
```
...
- 这一步的**确定收益**：工作区目录的任何临时拷贝（`tar`/`scp -r data/`/忘加排除的备份）
  从此只含密文，不再连 KEK 一起泄。
- 单盘 VPS 上，如果 KEK 就放在同盘的 `aipehub.env`，一个**整盘快照**仍会同时抓到
...
不想让 KEK 以明文躺在 `aipehub.env` 里，可改用 systemd 加密凭证（用主机/TPM key 加密落盘，
启动时解进 `$CREDENTIALS_DIRECTORY` tmpfs）：

```bash
# 把 hex key 存成加密凭证（同样别回显值）
printf '%s' "$KEYHEX" | sudo systemd-creds encrypt --name=aipe_master_key - /etc/aipehub/aipe_master_key.cred
```

然后在 service 的 drop-in 里 `LoadCredentialEncrypted=aipe_master_key:/etc/aipehub/aipe_master_key.cred`，
并让 env 文件改成从凭证文件读（启动脚本里 `export AIPE_MASTER_KEY="$(cat "$CREDENTIALS_DIRECTORY/aipe_master_key")"`）。
...
3. **在飞书里**，给你的 AipeHub bot 发送绑定指令（按 [`IM-BRIDGES.md`](IM-BRIDGES.md)
...
1. 脚本随 repo 已经在盒子上：`~/aipehub/app/scripts/monitor/healthcheck.sh`。
2. 建一个飞书**自定义机器人**（群里加），拿 webhook URL，存进 0600 文件：
   ...
   ```
3. 薄包装 `~/aipehub/healthcheck-cron.sh`（设 `FEISHU_WEBHOOK_FILE` + `AIPE_WEB_PORT`，
   exec `healthcheck.sh ~/aipehub/monitor-state`），`chmod +x`。
4. crontab 每 3 分钟：
   ```cron
...
```bash
~/aipehub/healthcheck-cron.sh                 # host 在跑时：静默、exit 0
sudo systemctl stop aipehub
~/aipehub/healthcheck-cron.sh                 # 应收到飞书「DOWN」+ exit 2
sudo systemctl start aipehub
~/aipehub/healthcheck-cron.sh                 # 应收到飞书「RECOVERED」+ exit 0
```
