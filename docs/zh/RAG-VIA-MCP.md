...
Gotong **不**自己造 ingest / embedding / 向量存储 这套轮子。RAG 生态
里成熟方案太多 —— Chroma / Qdrant / Pinecone / pgvector / Milvus
—— 每家都有自己的运维姿势、模型选型、计费模型。Gotong 把这一层完全
甩给社区,通过 [Model Context Protocol](https://modelcontextprotocol.io)
把它们接进来。

具体说:

- **Gotong host 不存向量,不调 embedding API**。`identity.sqlite` 里
...
- 代价: Gotong 不知道你检索了什么内容、用了什么 embedding 模型;
  admin UI 没"知识库管理"页面 —— 那是 RAG 服务自己的事。
- 收益: 锁定零、迁移零;RAG 生态进化跟 Gotong 解耦;
  ...
  "mcpServers": [
    {
      "name": "knowledge",
      "command": "uvx",
      "args": ["chroma-mcp", "--persist-dir", ".gotong/knowledge/rfp"],
      "env": {
...
2. **MCP server 侧**: 让 server 自己根据 API key 做配额(Pinecone /
   OpenAI 都内建这套)。Gotong 把 OOM 留给 server 报错,然后 agent
...
MCP server 未来支持把 ingest 字节数 / query 次数回调给 Gotong
...
hub_B 的 agent 凭据。这是个独立项目,**不在 gotong 主仓**。

### 跟 D2 (跨 hub HITL) 的区别

**HITL 走 gotong**(D2 已实现):因为 HITL 涉及 task 跨 hub 路由 +
admin 决策 + 审计,这些都是 gotong 已有的核心抽象(hub.dispatch /
task.origin / audit_log)。让 gotong 在外面绕一圈反而复杂。

**knowledge 不走 gotong**(D3 跳过):因为 knowledge 检索是个纯数据
平面操作,跟 task scheduling / approval / audit 无关。MCP 协议天然
处理客户端-服务端拓扑,gotong 再封一层只是冗余。
...
| 担心 RAG 调用走漏数据 | env var 注入的 API key 不进 audit log;但 `mcp_calls` 计数会上 audit。需要细粒度内容审计的,在 MCP server 侧自己加 hook —— Gotong 看不到 RAG 内容是设计取舍,不是 bug。 |
