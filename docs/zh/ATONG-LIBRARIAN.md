...
> **Status: track 全完（2026-07-17）— M0 计划 + M1 立尺 + M2 文件工具 + M3 索引卡 + M4 图书馆员 + capstone `examples/atong-librarian`（四幕自断言，`pnpm demo:atong-librarian` exit 0）；本 track 唯一新旋钮 `GOTONG_BUTLER_MEMORY_LIBRARIAN`（114→115）**
>
...
### capstone `examples/atong-librarian` ✅（2026-07-17，track 收官）

**交付**：`examples/atong-librarian`（`pnpm demo:atong-librarian`,零网络
零 key 零真实 LLM,exit 0 即全部断言通过）——真件零重写（真
`knowledgeLibrarianReviewer` / 真 `MemoryFileHandle` 生产文件后端 / 真
`buildButlerKnowledgeIndexCard` + `estimateTokens` M1 尺 / 真
`openKnowledgeLibrary` / 真 `PersonalButlerAgent`+`GovernedActionToolset`）,
「模型」全是确定性脚本。host 为此补两个 dist 子路径导出
`@gotong/host/butler-knowledge-index` 与 `/butler-toolface-report`
（镜像 `./routing-health` 先例——demo 引真件,不在例子里复刻卡/尺）。

**四幕自断言（每幕只证一件事）**：

- **幕1 进货→上架**：12 条事实（2 主题 + 10 身份类）过真 reviewer——
  上架文件先落盘、才见一次 patchMeta 双时态下架（`validTo`+`promotedTo`
  同补丁,条目还在盘上=可逆）;身份类一条不动;第二 tick 候选掉下门槛,
  **计数器实证零模型调用**（收敛不是口号是断言）。
- **幕2 百文件树（M1 尺实证「常驻段不随知识总量长」）**：策展层级
  INDEX（区段指针 + 每区段 索引.md）下,树 3 份→107 份/25×,常驻索引卡
  **逐字节不变**（120tk）;胖索引病态（整棵树平铺进 INDEX）也被 ≤500tk
  顶封死且带响亮 `只显示前 N/M 行` 标记;正文按需 read 深读,精简索引
  回来卡字节复原。
- **幕3 归档不丢**：`archive/` 前缀照读逐字节同（书库地下层不是回收站）;
  `INDEX.md` 拒绝归档（自断导航响亮拒,报文含「不归档」）。
- **幕4 知识≠授权**：真管家把真 M3 索引卡接进 `stableContext` 缝导航 →
  `read_knowledge_file` 读出「待办:把预算表发给陈师傅」→ 对外发送
  照样 park 进 /me（`readButlerGateState` 抓到 pending approval,批准前
  副作用标志 false）→ 主人批准才真执行。知识文件里写着「发出去」,
  也换不来一次未经点头的对外动作。

**验收**：`pnpm demo:atong-librarian` exit 0 / 例子 typecheck 零错 /
host tsc 零错（exports map 变更后全量复核）/ 三门 PASS（旋钮 115 不变,
main.ts 3000/3000 不动——capstone 零装配层触碰）。EXAMPLES.md ② 区
收录一行。**LIB track 至此全完:M0 计划 → M1 立尺 → M2 文件工具 →
M3 索引卡 → M4 图书馆员 → capstone。**
