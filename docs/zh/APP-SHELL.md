...
| **M1 设备配对签发** | 抄 IM 6 位码形状,成员自助兑换出 `aipk_`;补 expiresAt + 设备维度 + Bearer 限速 | 成员自助全程走通;码单次消费;过期拒;限速生效 |
| **M2 base URL 层** | 一处咽喉收编 52+9 处 fetch、EventSource、15 个 bundle 路径、SW 同源锁 | 默认空 ⇒ 与今天**逐字节一致**(防腐门);设了 ⇒ 全部请求指向远端 |
...
| **M4.5 骨架配置化** | tabbar 与首屏纳入配置(范围见岔口 B) | 两个成员打开 app 看到**不同的导航结构**,不只是不同的面板内容 |
| **M5 壳工程** | `capacitor.config.ts`(`webDir` + `CapacitorHttp.enabled`)+ static 导出脚本 + PNG 图标集 | 真机装上、打得开、能连本机 hub |
| **M6 原生推送** | `@capacitor/push-notifications` 接 FCM/APNs;复用订阅存储形状与 fold 决策 | 真机锁屏收到低信息 tap;绑 IM 成员仍零双发 |
...
FCM 在大陆结构性不可用,不是配置问题。

- **A1 大陆版先不做推送(推荐)** — 只轮询,打开 app 才看到新消息。全球/马来
  西亚这条腿先做通;大陆推送作为独立后续 track。工作量:零。
- **A2 接厂商推送** — 小米 / 华为 HMS / OPPO / vivo,**每家一套 SDK 与审核**。
  工作量:大且持续(厂商 SDK 会腐)。
- **A3 UnifiedPush 自托管** — 与 PUSH-M2 刻意不做厂商域白名单的设计天然兼容
  (自托管 UnifiedPush 是合法生态),但要求用户额外装一个分发器 app。

推荐 **A1**:先证明壳本身成立,推送是第二战场,而且主用户在马来西亚。

### B. 骨架配置化画到哪(M4.5 的范围)

- **B1 只 member 可见的三页签(推荐)** — `home` / `panel` / `settings` 纳入
  配置,admin 的 14 个页签保持硬编码。admin 面是运维台,配置化几乎无价值,
  而爆炸半径小得多。
- **B2 全部 17 个页签** — 彻底,但要动 app.html 骨架与 15 个 admin bundle 的
  加载链,风险与收益不成比例。

### C. 设备配对的交互形状(M1)

- **C1 6 位码** — 直接抄 IM 绑定,web 上生成、app 里手输。最省。
- **C2 二维码(推荐)** — web 显示,app 扫码;二维码里编码 **hub 地址 + 一次性
  码**,一步解决「输地址」与「拿凭证」两件事。C1 作为扫不了时的兜底。
- **C3 手输 URL + 粘 token** — 最原始,但对 operator 自己装机最直接。

推荐 **C2 + C1 兜底**——它顺带把第一档「app 内输入服务器地址」的动线一起解决了。
