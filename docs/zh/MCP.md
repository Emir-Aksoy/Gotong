...
AipeHub 自带一个 [Model Context Protocol](https://modelcontextprotocol.io)
...
> *"用 AipeHub 派一个 draft 任务给任意有 `draft` capability 的人，
> 主题是 'why TypeScript'，然后给结果打 4.5 分。"*

> *"看一下 AipeHub 本周贡献榜，谁在领先？"*

> *"列出 AipeHub 参与者。有人在线吗？"*
...
- 一个跑着的 AipeHub host —— `pnpm host`（从源码跑）或者
...
> ⚠️ **`@aipehub/mcp-server` 当前仅源码可用**。下面所有客户端配置示例
> 里的 `"command": "npx", "args": ["-y", "@aipehub/mcp-server"]` 写法
> 要等选定 JS 发布渠道之后才能用（见
...
MCP server；应该能看到 `aipehub` 下面写着 **5 tools available**。
...
所有工具都一对一翻译成对 Hub 管理 API 的 HTTP 调用。`AIPE_ADMIN_TOKEN`
...
```
   Claude Desktop / Cursor / Cline / …
        ...
        │
   aipehub-mcp  （本包，由 MCP client 启动）
        │
        │ HTTP + Bearer admin token
        │
   AipeHub host  （你已经跑着的 Hub）
        │
...
| 症状 | 可能原因 |
|---|---|
| MCP 客户端说 "aipehub server failed to start" | `AIPE_HUB_URL` 错 —— `aipehub-mcp` 启动时会 ping `/healthz`。检查 URL 和 host 是不是真的在跑。 |
| 工具列出来了但每次调用都 `401` | `AIPE_ADMIN_TOKEN` 错了或过期了。从管理 UI 重新签发一个 admin token。 |
| `dispatch_task` 总是返回 `no_participant` | 没有 agent 有你要的 capability。先 `list_participants` 看一眼当前有什么，或者用 `direct` + 具体 id。 |
| 工具列表是空的 | MCP 客户端连上了但 `tools/list` 返回 `[]`。通常是 `npx` 缓存了老版本的 `@aipehub/mcp-server`。客户端配置里换 `npx -y @aipehub/mcp-server@latest` 试试。 |
| Claude Desktop 日志显示 `Cannot find module '@modelcontextprotocol/sdk'` | 一般 `npx -y` 能解决。持续不行就全局装：`npm i -g @aipehub/mcp-server`，把 `"command"` 改成 `"aipehub-mcp"`，删掉 `args` 数组。 |
