...
- **`renderAlertText(payload)`**: im/email 的一行人类可读告警, **只**从 counts-only 载荷渲染
  (`[aipehub] alert {firing|resolved}: {label (ruleId)|ruleId} — {metric} {comparator} {threshold} (observed {value}) on source {source}`)——
...
- **`main.ts` env 旋钮**(`ed4f87b`): `AIPE_PEER_SUMMARY_ALERT_RETRY_ATTEMPTS`(clamp [1,6]) /
