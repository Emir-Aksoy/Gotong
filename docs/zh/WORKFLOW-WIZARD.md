...
跑 live 评测：`OPENAI_API_KEY=… OPENAI_BASE_URL=… GOTONG_LIVE_OPENAI_MODEL=… npx vitest run packages/host/tests/live-wizard-eval.test.ts`
...
无新增 `GOTONG_*`。向导的开关就是 assist 面本身（workflowAssist 在则向导在），修复
