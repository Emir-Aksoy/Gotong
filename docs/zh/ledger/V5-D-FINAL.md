...
到 v4 收官，Gotong 的 agent 都是**被动**的:有人派 task 才动。但「人-智能体-机构」
...
resume sweep tick（host 既有 setInterval，默认 30s——心跳不加自己的 timer）
  │  发现 resume_at <= now
  ▼  Hub.resumeTask(gotong:heartbeat, task, state)
       │
...
2. **broker 是单例，capability 空**。它从不被 capability dispatch 选中（cap=`[]`），
   只被 sweep 按固定 id `gotong:heartbeat` resume。一个 broker 管所有 agent 的心跳，
