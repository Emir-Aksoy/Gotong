...
- web 层用鸭子类型 `UsageLedgerSurface`，**零** `@gotong/identity` 运行时依赖。
...
`<GOTONG_SPACE>/pricing.json` 覆盖（解析失败 → boot 期抛错，不静默吞）。
...
- `@gotong/llm`：`LlmAgentOptions.usageSink?(task, usage, meta)` +
...
+ 真 WorkflowController + 真 `estimateCostMicros` + 真 `@gotong/web` 导出列。两道闸：
...
- **价目覆盖**：`<GOTONG_SPACE>/pricing.json`（缺省用内置 `DEFAULT_PRICING`；malformed →
