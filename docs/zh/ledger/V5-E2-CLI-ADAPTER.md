...
回来，全程带齐 AGENT-ADAPTER-CONTRACT 的五条控制缝。这是 `gotong connect <agent>`
...
Stream E 是把 Gotong 从「能自洽运转」推到「装得下真实工作」的五条交付缺口，按杠杆
排序。E2 是用户点名的那一条：

- 现状只有 **入站** 方向（`gotong connect` 把外部 agent 接成 hub 的 MCP client）。
...
### 新包 `@gotong/cli-agent`（第 31 包，core-only 叶包）

跟 `@gotong/inbox` / `@gotong/a2a` 同源——可复用的 participant 住在只依赖 core 的
叶包里，example 导入叶包而不是 host（`@gotong/host` 一被 import 就会跑起来）。
