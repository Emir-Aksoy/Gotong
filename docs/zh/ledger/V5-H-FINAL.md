...
Stream G 把「一个 hub 的工作流编排**另一个 mesh hub**（Gotong↔Gotong）的能力」收口。
...
本地步。只依赖 `@gotong/core` + `@gotong/workflow` + `@gotong/a2a`。
...
- H-M4 示例用**注入的 `fetchImpl`** 扮外部 A2A agent（不起 socket，同 `@gotong/a2a` 单测
...
`@gotong/core` + `@gotong/workflow` + `@gotong/a2a`。
...
- **X-M1**（`f7a772a`，`@gotong/a2a`）`A2aRemoteParticipant` 构造加
...
- **X-M4**（`4c944f3`，host）`A2aOutboundManager` 持
  ...
  `GOTONG_A2A_OUTBOUND_QUOTA_WINDOW_MS`（镜像 `GOTONG_PEER_LINK_QUOTA_WINDOW_MS`）。
