...
| 里程碑 | commit | 内容 |
|---|---|---|
| **M8a** | `72f60bd` | `@gotong/a2a` **wire 类型 + tasks/get client**。`A2ATask`/`A2ATaskStatus`/`A2ATaskState`（`working`/`completed`/`failed` + `submitted`/`input-required`/`canceled` 前向兼容）+ `A2A_TERMINAL_TASK_STATES` + `isTerminalTaskState`/`isA2ATask` 判别器；`A2A_METHOD_TASKS_GET` + `tasks/get` 请求类型；`A2AResponse.result` 拓宽 `A2AMessage \| A2ATask`；构造器 `workingTask`/`completedTask`/`failedTask`/`buildTasksGetRequest`；`A2A_ERROR.TASK_NOT_FOUND=-32001`。client 抽共享 `postA2a`；新 `a2aSendRaw`（回原始 `Message\|Task`）+ `a2aGetTask`（单次 `tasks/get`）；`a2aSend` 遇 Task 抛 `A2aClientError` 带 `.taskId`。+12 测试。 |
...
a2aSendRaw(url, token, text,                   POST /a2a  {message/send}
  {peerId, metadata:{skill}})  ───────────▶    auth: X-Gotong-Peer-Id + Bearer
...
| 包 | 数 | 关键 |
|---|---|---|
| `@gotong/a2a` | 27 | tasks.test.ts +12（构造器 / 判别器 / `a2aGetTask` / `a2aSendRaw`）；client/participant 零回归 |
...
- 入站 A2A 仍由 `GOTONG_A2A_INBOUND_ENABLED`（默认关）+ identity 在位才挂载。
