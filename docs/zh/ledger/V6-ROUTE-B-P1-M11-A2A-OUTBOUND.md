...
> `GOTONG_A2A_AGENTS` 环境变量 JSON blob 配, 无 UI、无持久化、改一个要重启
...
- **状态都是磁盘文件**: 出站 agent 配置从「进程环境」变成 SQLite 行, 跟 transcript /
  agents / vault 一样 —— 复制 `.gotong/` = 搬走房间, 重启透明。环境变量 blob 做不到
...
照「删旧代码优先于 deprecation shim」: `GOTONG_A2A_AGENTS` 整个删掉, 不做「库为空时
...
模式 (web 零 `@gotong/identity` 运行时依赖)。与 OIDC 的 write-only `client_secret`
...
- **设好 env 后让它上线**: 把该行**停用→启用** (一次 PATCH) ⇒ host 重读环境变量并
  注册, 无需重启进程。
- **`GOTONG_A2A_AGENTS` 已删**: 旧的环境变量 JSON blob 不再读。迁移 = 把每条目在 UI
  里重新登记一遍 (token 仍走各自的环境变量, 照旧 `export`)。
- **入站 A2A 不受影响**: M11 只动**出站** (本 hub 调别人)。入站 (`A2aServer`, 别人调
  本 hub) 仍由 `GOTONG_A2A_INBOUND_ENABLED` 开关控制, 与本 milestone 无关。
...
- 出站 participant 本体: `@gotong/a2a` `A2aRemoteParticipant` (Phase 18 C-M4)
- 入站对侧: host `a2a-server.ts` (`GOTONG_A2A_INBOUND_ENABLED`) + Route B P1-M8 task lifecycle
