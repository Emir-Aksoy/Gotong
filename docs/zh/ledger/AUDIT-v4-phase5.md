...
| **C1.1/C1.2/C1.3** SPA | XSS / role 绕过 | server-side meta 注入 role + ALLOWED_ROLES Set 验白名单;`<meta name="x-gotong-role">` 注入前 escapeHTML | web/tests/c1-app-shell.test.ts 10 项 | client 端 role 显示仍是 hint —— 真正的 owner-only API 在后端 reauth |
...
| **D3** 跨 hub knowledge | — | 取消;走共享 MCP server,不在 gotong 攻击面 | — | MCP server 拓扑安全外置 |
...
cap,例如 5 个用户每人 100/day → 实际 org cap = 500/day。gotong 不再
做"软上限自动转硬上限",由操作员显式配。
...
- [x] 所有跨包 import 走 `@gotong/*` 包名,不走相对路径(monorepo workspace)
