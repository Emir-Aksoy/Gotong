...
> Phase 12 把 Gotong 从「浏览器 admin UI + Node SDK」单一入口扩到「人在
> 哪儿，agent 就在哪儿」。6 个 `@gotong/im-*` bridge 让 Telegram /
> Matrix / 飞书 / Discord / Slack / QQ 用户用熟悉的 IM 客户端跟 hub 对话；
> `gotong repl` 把 stdin/stdout 当本地 IM bridge，终端里 `:help` `:agents`
...
|---|---|---|
| M1 | `b0af21e` | `@gotong/im-adapter` 基础包（`ImBridge` / `ImUser` / `ImMessage` / `ImCommand` 类型 + `parseImCommand` + `ImBindingResolver`）+ identity migration v=10（`im_bindings` / `im_binding_codes` 表 + 7 个 store API + 2 个新 IdentityError code） |
| M2 | `a09640d` | `@gotong/im-telegram` —— bot API long-poll (`getUpdates`)，text + attachments dual-write，bot mention strip，anti-loop via `from.id`，`TelegramApiError` |
| M3 | `056d612` | `@gotong/im-matrix` —— Client-Server `/sync` 长轮询 + `since` 缓存 + room timeline 派发 + `PUT m.room.message` outbound + `MatrixApiError`（含 `retryAfterMs` / `errcode`） |
| M4 | `f254100` | `@gotong/im-lark` —— verification + 加密 webhook + `tenant_access_token` 2h cache + coalesced refresh + `LarkBridge` 内置 `node:http` listener（默认端口 9090）+ `im.message.receive_v1` 事件 |
| M5 | `19588c8` | `@gotong/im-discord` —— Gateway WSS (op 10/11 heartbeat + identify + RESUME), MESSAGE_CREATE 派发, `POST /channels/.../messages` outbound, intents bitfield, fatal close code (4004/4014) 不重连 |
| M6 | `6e4e185` | `@gotong/im-slack` —— Events API webhook (HMAC SHA256, 5-min replay window), `event_id` dedup (512-entry FIFO), `chat.postMessage` outbound, slack-file URI scheme，built-in HTTP server 端口 9091 |
| M7 | `4be4f56` | `@gotong/im-qq` —— OneBot v11 forward WS + echo-paired action multiplex + risk gate (`GOTONG_QQ_BRIDGE_ACK_RISK=true`) + `encodeQqChatId('private:<qq>' \| 'group:<gid>')` + array / string-form message 双兼容 |
...
| M12 | `355e5cf` | `gotong repl` 子命令 —— in-memory hub + 默认 `ReplEchoAgent`，`:`-prefix 元命令 (`:help` / `:agents` / `:transcript` / `:dispatch` / `:quit`)，`parse.ts` + `bootstrap.ts` + `loop.ts` + `commands/repl.ts`，readline line-event queue（修 pipe 模式 hang） |
...
agent 帮我干」。Phase 12 给的范式是 **「IM 客户端 = 远程终端，Gotong 是
后端」**。每个 bridge 干同一件事：
...
`gotong repl`（M12）把同一套 router 模型搬到本地终端。stdin 进来一行 =
...
| 决策点 | 选择 | 为什么 |
|---|---|---|
| `ImBridge` 抽象在哪 | 单独包 `@gotong/im-adapter`，只导出 type + `parseImCommand` | bridge 包之间互不依赖；host 只依赖 adapter 拿 type，**不**强依赖 6 个 bridge 包 |
| 每个 bridge 一个包 vs monorepo `@gotong/im` | 每平台独立包 | 用户只装他要用的（Telegram bot 不需要 Discord WS lib）；bin binary 不被 6 个 bridge 拖大；测试边界清晰 |
...
| QQ 协议风险 | 默认拒绝启动，需 `GOTONG_QQ_BRIDGE_ACK_RISK=true` env | OneBot v11 是逆向协议，账号有封禁风险；故意的运维摩擦让 ops 读 docs |
| IM identity 绑 Gotong user 怎么做 | 6 位数字 code + 短 TTL（默认 10 分钟） | UX：用户在 admin UI 点 "issue code" → 抄到 IM 里发 `/bind <code>` → bridge 自动解析。code 一次性 + rotate-on-reissue，比对话式 captcha 简单 |
...
| `Task.from` 形态 | `'im:<platform>:<platformUserId>'`，Gotong userId 走 `task.origin.userId` | transcript 里看着是 `from='im:telegram:123456789'`，可读 + audit 友好；quota gate 看 `origin.userId` |
...
| `@gotong/core` 加进 `@gotong/cli` runtime dep | 加 | 验过 `@gotong/core` 零 runtime dep（`better-sqlite3` 只是 devDep），不会拖垮 `npx @gotong/cli` 安装 |
...
`gotong repl` 走完全一样的链路，只是 step 1+8 换成 stdin / stdout，step 2-7
...
| 包 | 文件 | 测试数 | 主要场景 |
|---|---|---|---|
| `@gotong/im-adapter` | `command-parser.test.ts` | 15 | `/help` / `/bind <code>` / `/unbind` / `/agents` / `/workflow <name> args` / free-text；多语言空白容错；trailing-newline；非 `/` 前缀 → free |
| `@gotong/identity` | `im-bindings.test.ts` | 27 | 7 store API CRUD + 错误 code (invalid/expired)；code rotate-on-reissue；INSERT OR REPLACE 行为；ON DELETE CASCADE；sweep 过期 code；listImBindings filter；index sanity |
| `@gotong/im-telegram` | 3 files | 31 | bridge start/stop 幂等 / poll offset cursor / sendMessage formdata / 错误重试 / TelegramApiError；client fetch wrapper；message mapper 含 attachment + mention strip + anti-loop |
| `@gotong/im-matrix` | 3 files | 49 | sync long-poll + since 缓存 / autoJoin invite / event_id dedup ring / cold-start backlog skip / retry-after honour / MatrixApiError retryAfterMs；fetch + Bearer；message mapper m.room.message + edit/reply |
| `@gotong/im-lark` | 3 files | 69 | verification challenge + 加密 webhook decrypt / tenant_access_token coalesced refresh / 2h cache / im.message.receive_v1 / receipt fallback / 内置 http listener；client；message mapper 含 attachment file_key |
| `@gotong/im-discord` | 3 files | 60 | gateway state machine (HELLO/IDENTIFY/READY/HEARTBEAT/RECONNECT/INVALID_SESSION) / zombie detection / 4004/4014 不重连 / exponential reconnect / intent bitfield；client；message mapper 含 mention strip (`<@!id>` legacy) |
| `@gotong/im-slack` | 3 files | 85 | HMAC SHA256 verify + 5min replay window + constant-time / event_id dedup 512 FIFO / file_share 4 layers anti-loop / built-in http server；client xoxb auth；message mapper 含 `slack-file:` URI + Slack mention strip |
| `@gotong/im-qq` | 3 files | 80 | risk gate env + 默认拒启 / echo-paired action multiplex / 12-byte correlation id / connecting/open/closed state dedup / disposed on stop / private/group chatId 编码；client；message mapper array+string-form 双兼容 + 3-layer anti-loop |
| `@gotong/cli` | `repl-parse.test.ts` | 26 | noop / help / quit / agents / transcript (clamp [1,200] + default 5 + floor) / dispatch (full + alias `:d` `:send`) / free / unknown verb；`:`-prefix；case-insensitive |
| `@gotong/cli` | `repl-loop.test.ts` | 18 | `:quit` reason+turns / EOF / `:help` render / `:agents` list / 自由文本→default cap / `:dispatch tester` 显式 / no-participant / `:transcript` last N / 空行 noop / agent throw 不挂掉 / AbortError 退出 / `ReplEchoAgent` defaults；4 个 `createReplHub` test |
...
`printf ':help\n:agents\nhello world\n:transcript 4\n:quit\n' | node packages/cli/bin/gotong.js repl --no-banner`
...
- 每个 bridge 是独立包：`pnpm add @gotong/im-<platform>`。
...
1. **Caddy / Nginx 反代** —— 推荐。Caddy 自动签 TLS，配 ~5 行：
   ...
   ```
2. **`webhookPort: 0`** —— bridge 不开自己的 listener，host HTTP 层（@gotong/web）
...
- 必须 `GOTONG_QQ_BRIDGE_ACK_RISK=true` env 才启动（否则 `QqBridgeRiskNotAck` 错）
...
- `task` entry：`from='im:<platform>:<platformUserId>'`，origin.userId 是 Gotong userId
...
- pipe 模式自动检测（`process.stdout.isTTY`）—— `printf '...' | gotong repl`
...
- **`gotong repl --connect=<ws-url>`** —— 远程 hub REPL。
