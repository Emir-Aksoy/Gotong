...
- **框架不跑 LLM**:管家是一个 `Participant`(`LlmAgent` 子类),决策在它(模型)手里;
  Hub 只路由 / 记 transcript。记忆是**文件**(`.gotong/` 里看得见,复制目录=搬走管家的
  「大脑」),不是云端黑盒。
...
| M | 做什么 | 关键产物 | commit |
|---|---|---|---|
| **M1** | 记忆增强 agent:冻结块注入 + `remember/recall/forget` 当 LLM 工具 | `@gotong/personal-memory` — `MemorySession` + `renderFrozenBlock`(同会话稳定护缓存)+ `MemoryToolset` | `5cc0d96` |
...
| **M4** | 灵活调用接线 + 治理统一 + 检索后端 | `@gotong/personal-butler` — 有界 `PersonalButlerAgent` tool-loop + `GovernedActionToolset`(敏感→审批门控)+ 可换 `MemoryRetriever` | `c13dab2` |
...
| 包 | 测试 | 数 |
|---|---|---|
| `@gotong/personal-memory` | session / frozen-block(同会话字节相同护缓存)/ toolset 往返 / capture / consolidate 蒸馏 + prune / review heartbeat / retriever / agent + agent-capture | **75** |
| `@gotong/personal-butler` | agent(良性内联 / 治理 park / resume 批准·拒绝·无裁决 / mixed-round 原子 / bounded)9 + governed-toolset 12 + **sensitive-memory-write 5**(M6d) | **26** |
...
- **fold 进 host main.ts 当一等公民** — ✅ **已做(BF-M1–M6)**,详见
  [`docs/zh/ledger/PERSONAL-BUTLER-FOLD-IN-FINAL.md`](./PERSONAL-BUTLER-FOLD-IN-FINAL.md)。生产
  `gotong start` 注册的 `chat` agent 现在是一个 per-user `ButlerRouter`(按 `task.origin.userId`
  路由到每个成员各自的记忆命名空间),接管 IM 通道,**默认开**(`GOTONG_BUTLER` 关),GitHub
  代码库默认如此;`/me` 隐私视图读同一 `<space>/butler/memory` 子树。当前是**纯记忆**形态
  (跨会话记忆经 `frozenRefreshPerTask` 修复),**仍推迟**:① BF-M7 完整 governed 动作集(管家
  替成员做敏感动作 → 收件箱审批,sink 已就位休眠中);② BF-M8 per-user heartbeat 蒸馏 / 6h
  维护在生产 host 里跑。
...
- 现有管家:[`docs/zh/ledger/V5-STEWARD-FINAL.md`](./V5-STEWARD-FINAL.md)(`HubStewardAgent` + 四级分级 + 收件箱审批)
