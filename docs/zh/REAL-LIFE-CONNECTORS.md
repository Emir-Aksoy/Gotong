...
>
> Last updated: 2026-07-08 · C-M1 done · C-M2 出站 OAuth:M1 纯核 + M2 存储层 + M3 连接流 + M4a 活令牌注入 + M4b 自动刷新 + M5a admin CRUD 后端 done(M5b 目录预设 / M5c UI / M5d capstone 收尾)。
...
| **C-M2** | 出站 OAuth 连接器接入:日历 / 邮件 / 记账(Google / Microsoft / Notion-OAuth / Todoist-hosted)—— 「用 X 登录」按钮,令牌进 vault + 自动刷新,注入远程 MCP 的 bearer header | OAuth 2.1(出站) | M1–M4 ✅ · M5 在途 |
...
| **C-M2-M5b** | OAuth 连接器目录预设(Google 日历 / Notion 托管:端点/scope/`mcpServerName` 内置,admin 只填 client_id/secret/redirect) | 计划 |
| **C-M2-M5c** | admin 面板「用 X 登录」UI(列连接器 + 目录 + 连接/断开按钮打 M3b start + M5a 路由) | 计划 |
| **C-M2-M5d** | capstone + 文档收尾 | 计划 |
