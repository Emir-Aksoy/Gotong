...
> Status: **track 全完（M0→M5，2026-07-16 收口）**——八缺口收六（①②③④⑥⑦），
> ⑤ recent_hub_events + ⑧ 行动史显式推迟（见文末收口块）
...
> 收口时点各项归宿：② ✅ SEN-M4 / ④ ✅ SEN-M5（岔口 A）/ ⑤⑧ 显式推迟
> （见文末收口块）。
...
## ✅ SEN track 收口（2026-07-16，用户拍板「到此收口」）

**八缺口账本**：收六迟二。

| # | 缺口 | 归宿 |
|---|---|---|
| ① hub 健康按需工具 | ✅ M1b `hub_health`（`bd8a13f`） |
| ⑦ 红灯进每轮意识 | ✅ M1a 尾卡探针（同刀） |
| ③ peer 信任分档不可见 | ✅ M2 trustTier 转述 + pinnedKid 折布尔（`9dffb77`） |
| ⑥ 「我的状态」无汇总 | ✅ M3 `my_status` 六块折一（`405a95e`） |
| ② 定时工作流不可见 | ✅ M4 `list_schedules` admin 同源（`bfafd96`） |
| ④ 成员/角色不可见 | ✅ M5 `list_members` 岔口 A（`c0765e1`） |
| ⑤ 事件盲区 | ⏸ 显式推迟（下） |
| ⑧ 行动史 | ⏸ 显式推迟（与 ⑤ 共享披露设计） |

**⑤⑧ 显式推迟（不预造）**：两者共享同一道未决披露岔口（成员只见自己相关
事件 vs hub 级全见），且数据源是 transcript——有界投影要先定「多久算近期 /
多少条封顶 / 哪些事件类别入选」三个尺度，是真设计不是小刀。状态类感知
（「现在怎么样」）已全收口，事件回溯类（「昨天/这周发生了什么」）等真实
使用里出现高频问法再按 M0 惯例摆岔口重启;重启时读 §四 方向注记即可，
三管道格局不变、无需重新侦察。

**全 track 总账**：六 commit `a678f5a`（M0 docs）→ `bd8a13f`（M1）→
`9dffb77`（M2）→ `405a95e`（M3）→ `bfafd96`（M4）→ `c0765e1`（M5）;
四个新投影模块（hub-sense / self-status / schedules / members）+ 一次旧账
补字段（peers）;新/扩单测 17·8·13·11·12，host 全套 2134→**2183**;每刀
四门 PASS，**旋钮 114 全程冻结**，main.ts 3000 预算 M1–M3 零触碰、M4/M5
靠压历史注释净零守住;每个新 builder 过 AFR 注册三件套（tiers 名单 /
tripwire / 防腐门），M4 时名单双向核对门在 fixture 补 stub 前如期变红一次
——门活着。五边界零破例;唯一一次披露扩面（M5）走用户岔口人点头后落地。
