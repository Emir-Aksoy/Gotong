...
> 面向把 AipeHub 当生产后端跑在一台云主机上的运维。回答「现在吃多少、
> 跑起来要多少、该配多大的机」。
>
...
**纯「IM 聊天 → LLM 回复」用法下，AipeHub 自身极轻：稳态约 110–160 MiB
...
```bash
# 服务级资源账本（最准）
systemctl show aipehub.service -p MemoryCurrent -p MemoryPeak -p CPUUsageNSec -p TasksCurrent

# 系统内存/swap + load
free -h; uptime

# 数据增长（重点盯 transcript / sqlite）
du -sh ~/aipehub/data ~/aipehub/data/transcript.jsonl ~/aipehub/data/identity.sqlite

# 业务指标（Prometheus 文本，含 LLM 调用/token/成本/run 计数）
curl -s http://127.0.0.1:3000/metrics | grep aipehub_
```

需要关注的早期信号：`free` 的 `available` 持续走低、`du` 的 transcript
快速膨胀、`/metrics` 里 `aipehub_llm_*` 增速远超预期（可能是回环或滥用）。
...
- 运行方式：systemd `aipehub.service`（`Restart=always`，开机自启），
