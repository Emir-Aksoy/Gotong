...
**官方预置音色目录(2026-07-18 两独立客户端源码交叉核准,`mimo-v2.5-tts` 内置,
音色 id 即名字,换音色=改 `GOTONG_BUTLER_VOICE_VOICE` 一个 env + 重启)**:

| 音色 id | 描述 | 语言 |
|---|---|---|
| 冰糖 | 清澈甜美的年轻女声 | 中文 |
| 茉莉 | 温柔知性的成熟女声(当前生产在用) | 中文 |
| 苏打 | 阳光活力的年轻男声 | 中文 |
| 白桦 | 沉稳磁性的成熟男声 | 中文 |
| Mia | 明亮年轻女声 | 英文 |
| Chloe | 温暖优雅女声 | 英文 |
...
系列另有 `mimo-v2.5-tts-voicedesign`(一句话文本描述生成**合成**新音色,非真人,
法律性质与克隆不同,要用需另核 wire)与 `mimo-v2.5-tts-voiceclone`
(**结构性拒绝**——`buildButlerVoice` 构造当场拒,民法典 1023 声音权,永不接)。
写错音色 id 不炸:synthesize 折 `failed` → 回落纯文本(fail-soft 合同)。
...
- **生产现实(2026-07-18 用户真机发现,拍板「先留着」)**:用户的飞书语音走的是
  **语音输入法**——客户端就地转文字、消息到 hub 已是纯文本,ASR 腿根本不触发;
  只有按住说话发出的**语音条消息**(`message_type:'audio'`)才走下载+转写链。
  故本腿在该生产形态下近乎零触发=零成本,保留当兜底(别家成员/别的用法真发
  语音条时依然听得见),**不作为缺口重提、不主动催真机验证**。
