...
工作流 step:  human: { assignee, kind: approval|choice|edit, prompt }
                 │  desugar（M6，import 期）
                 ▼
            dispatch → capability aipehub.human/v1
                 │
                 ▼
...
| 层 | 物 | 依赖 |
|---|---|---|
| `@aipehub/inbox`（新包） | `InboxStore` 接口 + `FileInboxStore` + 类型 + `HumanInboxParticipant` broker | 只依赖 `@aipehub/core` |
| `@aipehub/host` | `HostInboxService`（两步恢复编排）+ boot 注册 broker | concrete `Hub` + `IdentityStore` + `@aipehub/inbox` |
| `@aipehub/web` | 鸭子 `InboxSurface`（`me-routes.ts`）+ `/me/inbox` 路由 + 前端面板 | 零 `@aipehub/inbox` 依赖 |
| `@aipehub/workflow` | `human:` YAML 糖（schema 脱糖） | 零 `@aipehub/inbox` 依赖（共享纯字符串常量 `aipehub.human/v1`） |
...
`dispatch → capability aipehub.human/v1`，payload 就是 broker 认的
...
- **runner / resolver / deepCheck 零改**：它们只见普通 capability dispatch。deepCheck 不
  报 `unknown_capability`，因为 broker 在 boot 注册了 `aipehub.human/v1`，inventory 自带它。

---

## 五、broker：`HumanInboxParticipant`（M2，`packages/inbox/`）

`extends AgentParticipant`，固定 id `aipehub:human-inbox`，cap `aipehub.human/v1`：
...
`serveWeb({inbox})`。启动日志：`JOIN aipehub:human-inbox (agent) caps=[aipehub.human/v1]`
...
- **鸭子 `InboxSurface`**（M3，`me-routes.ts`）：`listPending(userId)` /
  ...
  → `server.ts` 的 `WebServerOptions` / `HandlerCtx`。web 零 `@aipehub/inbox` 依赖。
- **路由**（M4）：
...
| 包 | 文件 | 数 | 测什么 |
|---|---|---|---|
| `@aipehub/inbox` | `file-inbox-store.test.ts` | 8 | 原子写 / `listPending` 过滤 / `get` 缺失返 null / `markResolved` race 守卫 |
| `@aipehub/inbox` | `human-inbox-participant.test.ts` | 8 | 合法 payload 写 item + 抛 `SuspendTaskError(永不)` / 缺 assignee 抛 / `onResume({answer})` / parent/parentKind |
| `@aipehub/web` | `me-inbox-routes.test.ts` | 8 | 401 无 session / 不能列别人 / 404 / 409 已处理 / happy resolve 传参 |
| `@aipehub/host` | `inbox-service.test.ts` | 6 | 子先父后 / 二次 resolve 被 markResolved 挡（不二次 resumeTask）/ 父完成才删行 |
| `@aipehub/host` | `inbox-e2e.test.ts` | 2 | **验收门**：happy + output 契约 + sweep 取不到 + 无漂移 |
| `@aipehub/workflow` | `human-step.test.ts` | 7 | `human:` → dispatch 到 `aipehub.human/v1` / 坏块 import 期抛 |
...
| M6 | `37ffb3f` | `human:` step 糖 → dispatch 到 `aipehub.human/v1` |
| M7 | `57c9e68` | 无漂移验收门 |
