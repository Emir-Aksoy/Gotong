...
```
        ...
        │   └───┬──────────────┬──────────────┬───┘                │
        │       │              │              │                    │
        │   🤖 host-managed   🤖 SDK agent   🪢 TeamBridge           │
        │      LLM agent       (Node/Py)       (another Hub)        │
        │   (templates/      (你自己的代码)     (递归)               │
        │    community/)                                            │
        └──────────────────────────────────────────────────────────┘
                                  ↑
...
| 你是 | 第一步 | 深入阅读 |
|---|---|---|
| **个人开发者 / 想 5 分钟跑起来** | `pnpm install && pnpm host` → 浏览器打开首次 admin URL | [`README.md` Quick start](../README.md#quick-start--pick-a-demo) |
...
| 路径 A · Host-managed | 路径 B · 外部 SDK 接入 |
|---|---|
...
4  Alice 派任务："写一个关于坚持的儿童故事"，strategy = capability:[story]
   → host-managed 的 storyteller 抢到任务 → 30s 后产出 600 字故事。
...
| 我想… | 读这 |
|---|---|
| 五分钟跑起来 | [`README.md` Quick start](../README.md#quick-start--pick-a-demo) |
