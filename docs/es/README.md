...
- **Tus claves y datos se quedan en tu disco.** Las credenciales viven cifradas en tu propio directorio `.gotong/`. Federar con otro hub comparte una capacidad, no tu vault.
...
| 🤖 **Escribiendo un agente para conectar** | [`docs/AGENT.md`](../../docs/AGENT.md) | `@gotong/sdk-node` o Python `gotong`. Subclasifica `AgentParticipant`. |
...
| 🔌 **Conduciendo un Hub desde Claude Desktop / Cursor / Cline** | [`docs/MCP.md`](../../docs/MCP.md) | `@gotong/mcp-server` es un puente MCP — 5 herramientas (list / dispatch / evaluate / leaderboard / tasks). Añade 5 líneas a tu configuración de cliente MCP. |
| 🧰 **Dando a tus agentes el ecosistema de herramientas MCP** | [`docs/MCP.md`](../../docs/MCP.md#6-outbound--using-third-party-mcp-tools-from-your-agent) | `@gotong/mcp-client` permite que tus agentes Gotong adjunten Filesystem / GitHub / Slack / Postgres / cualquier servidor MCP. `LlmAgent` ejecuta un bucle de uso de herramientas de múltiples turnos de serie (v0.3+) — simplemente pasa `tools: toolset` y Claude / GPT deciden cuándo llamar a qué herramienta. |
...
| **La clave API vive** | Cifrada en `.gotong/secrets.enc.json` (por agente o predeterminada del espacio de trabajo) | Donde tu código la lea |
...
construyen *y difunden* Gotong** → [`CONTRIBUTORS.md`](../../CONTRIBUTORS.md) — porque
...
Luego entrega la carpeta `Gotong-macos-arm64/` completa a cualquiera. Ellos **hacen doble clic en `Gotong.command`** → el navegador abre el asistente de configuración de 5 minutos. El paquete incluye su propio runtime de Node fijado + el host compilado + un `node_modules` real en disco (incluyendo el binding nativo de SQLite), por lo que ejecuta el host **completo** respaldado por identidad en una máquina sin nada instalado. Los datos viven en `~/.gotong` (fuera de la carpeta), así que reemplazar el paquete nunca pierde datos.
...
`GOTONG_OPEN_BROWSER` controla la apertura automática: sin definir = `auto` (solo el primer arranque local), `1`/`always` = en cada arranque, `0`/`never` = desactivado. También se fuerza a desactivado cuando el host está expuesto en red — un servidor sin cabeza nunca abre un navegador, y el asistente no es accesible allí de todos modos (esa ruta usa el archivo del token de administración). El banner en sí siempre se imprime.
...
```bash
pnpm exec gotong-host --help       # referencia completa de variables de entorno
pnpm exec gotong-host --version    # versión actual del host
```

Después de arrancar, sigue [`docs/OVERVIEW.md`](../../docs/OVERVIEW.md) para el recorrido "qué hacer ahora".

**¿No arranca?** Ejecuta una verificación previa antes de arrancar — inspecciona el env `GOTONG_*` exacto que lee el host (versión de Node, puertos realmente libres para enlazar, directorio de datos con permisos de escritura, clave maestra) e imprime, por cada verificación, ✓ / ⚠ / ✖ con una corrección en una línea:

```bash
pnpm exec gotong doctor          # solo informe
pnpm exec gotong doctor --fix    # también crea automáticamente un directorio de datos faltante (la única reparación segura y reversible)
```

Y si un arranque *falla*, el host convierte los fallos comunes y recuperables (puerto ya en uso, sin permiso para enlazar un puerto, clave maestra faltante/inválida, directorio de datos sin permisos de escritura, disco lleno) en un mensaje de una línea nombrando qué variable `GOTONG_*` cambiar — no un rastreo de pila. Ver la sección de solución de problemas en [`docs/zh/GO-LIVE.md`](../../docs/zh/GO-LIVE.md) §十一.
...
> 🙏 **Sobre privacidad / datos**: tus 4 secciones de autodescripción se enviarán a DeepSeek (servidores en China continental) para inferencia. Cuando el flujo de trabajo termina, todos los resultados caen en el directorio `.gotong-*/services/` de tu propio ordenador, no se sube nada a ninguna nube. Cada coach está diseñado como un acompañante con límites — el coach de cuerpo, al encontrar señales de alerta (dolor de pecho persistente / sangrado inexplicable, etc.), te indicará que consultes un médico; el coach de mente, al encontrar señales de riesgo, proporcionará líneas de crisis de 24h (nacional 400-161-9995 / Malasia Befrienders 03-7956 8144). **Esto no es un sustituto de un médico / psicólogo / asesor financiero / terapeuta de relaciones.**
...
```bash
GOTONG_LOG_LEVEL=info       # silent | trace | debug | info (predeterminado) | warn | error | fatal
GOTONG_LOG_FORMAT=json      # json | pretty (predeterminado: auto por TTY)
GOTONG_LOG_DISABLED=1       # escape hatch de desactivación total
```
...
|---|---|
| `@gotong/services-sdk` | Contrato `ServicePlugin`, registro, cargador. La costura que implementan los autores de plugins. |
| `@gotong/service-memory-file` | `memory:file` de primera parte — episódico / semántico / de trabajo como JSONL. |
| `@gotong/service-artifact-file` | `artifact:file` de primera parte — directorios por propietario de archivos con guardas de MIME + tamaño. |
| `@gotong/service-datastore-sqlite` | `datastore:sqlite` de primera parte — KV + SQL bruto en un `.sqlite` por nombre declarado. |
...
> **Nota de despliegue**: el host resuelve los paquetes de plugins desde su propio `node_modules/`, por lo que los plugins de terceros necesitan estar instalados donde el host pueda verlos — `pnpm add my-org/gotong-redis-memory` en el espacio de trabajo del host, o una dependencia `package.json` en la imagen de despliegue. Poner el nombre del paquete en `plugins.json` solo no es suficiente si el paquete en sí no está en disco.

## Paquetes

| Paquete | Propósito |
|---|---|
| `@gotong/core` | Hub, registro, planificador, transcript, almacenamiento, clases base de Participant |
| `@gotong/web` | UI de referencia embebible (HTTP + SSE + SPA vainilla) |
| `@gotong/host` | Binario de producción — controlado por env, sin estado de demo, incluye `gotong-host` |
| `@gotong/protocol` | Tipos de protocolo wire + codec (sin runtime) |
| `@gotong/transport-ws` | Transporte WebSocket del lado del Hub |
| `@gotong/sdk-node` | SDK de Node para agentes remotos (también exporta `TeamBridgeAgent`) |
| `@gotong/llm` | Clase base `LlmAgent` + interfaz `LlmProvider` + `MockLlmProvider` |
| `@gotong/llm-anthropic` | Proveedor Anthropic Claude (dep de par: `@anthropic-ai/sdk`) |
| `@gotong/llm-openai` | Proveedor OpenAI (dep de par: `openai`) |
| `@gotong/services-sdk` | Contrato de plugin de Servicios del Hub (v2.2) — ver la sección anterior |
| `@gotong/service-memory-file` | Plugin `memory:file` de primera parte (JSONL en disco) |
| `@gotong/service-artifact-file` | Plugin `artifact:file` de primera parte (dirs por propietario, con barrera de MIME) |
| `@gotong/service-datastore-sqlite` | Plugin `datastore:sqlite` de primera parte (KV + SQL) |
| `@gotong/mcp-server` | Puente MCP (Model Context Protocol) — permite que Claude Desktop / Cursor conduzca un Hub |
| `gotong` (PyPI, en `python-sdk/`) | SDK de Python — conecta agentes Python a un Hub sobre el mismo protocolo wire |
