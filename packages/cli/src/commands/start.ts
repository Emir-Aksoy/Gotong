/**
 * `gotong start` — convenience launcher that delegates to `@gotong/host`.
 *
 * The CLI deliberately does NOT depend on `@gotong/host`: the host drags in
 * the LLM SDKs, better-sqlite3, the web bundle, … which would bloat every
 * `npx @gotong/cli connect` / `repl`. So `start` resolves the host LAZILY at
 * runtime and hands the process over to it:
 *
 *   - host present  → import its `.` entry. That entry runs the server as a
 *                     top-level side-effect (exactly like the `gotong-host`
 ...
 *                     stdio unchanged: `gotong start` ≡ `npx @gotong/host`.
 *   - host absent   → print a one-line install hint and exit non-zero.
 *
 * The host is env-driven (GOTONG_SPACE / GOTONG_WEB_PORT / …), so `start` takes no
 ...
 * `@gotong/host` (like every package here) is ESM-only, its `exports` map has
 ...
 * that IS a CLI dep (e.g. `@gotong/core`) without needing the host installed.
 */
...
export async function start(
  ...
  // require `@gotong/host` to be a build-time dependency of the CLI.
  ...
  if (stray) {
    err(`[gotong start] unexpected argument: ${stray}`)
    err('  The host is configured via environment variables, not flags.')
    err('  Run `gotong start --help` for the supported GOTONG_* vars.')
    return 2
  }

  if (!resolveHost()) {
    err('[gotong start] @gotong/host is not installed.')
    err('')
    ...
    err('      npx @gotong/host')
    err('')
    err('  …or install it once so `gotong start` works thereafter:')
    err('')
    err('      npm i -g @gotong/host')
    err('')
    return 1
  }

  // Host present: importing its entry boots the server as a top-level
  // side-effect. The event loop stays alive (listeners on GOTONG_WS_PORT /
  // GOTONG_WEB_PORT), so we return 0 and the process keeps serving. A boot
  // failure is the host's own concern — it logs and `process.exit(1)`s.
  await importHost()
  return 0
}

const START_HELP = `gotong start
...
Run \`gotong init\` first if you want to pin team mode or name the admin;
otherwise just:
