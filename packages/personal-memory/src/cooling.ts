/**
 * cooling.ts — 阶梯第 ③ 级存储降温，不改变事实的有效期。
 *
 * # 降温不是删除,这一点是结构性的
 *
 ...
 *   - **已经盖过 `validTo` 的**:已在失效回收带，无须再降温。
 *   - **还没生效的未来事实**(`validFrom > now`):不能把尚未生效误判为长期未使用。
 ...
 * `enforceBudget`(执法回收),降温先标出冷条目，回收才看得见其存储优先级。
 *
