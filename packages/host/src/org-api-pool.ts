 ...
 */
/**
 * Per-call context for the gate — facts known only at dispatch time, not at
 * gate-construction time. Today only `modelUnpriced` (M8): the agent's model
 * has no entry in the pricing table, so its post-call cost records as $0.
 */
export interface QuotaCallContext {
  /** This call's model is unpriced — its cost can't be metered. */
  modelUnpriced?: boolean
}

export type QuotaGate = (
  subject: QuotaSubject | undefined,
  ctx?: QuotaCallContext,
) => void
...
export interface MakeLlmQuotaGateOpts {
   ...
   *
   * `denyIfModelUnpriced` (M8) marks the COST peek: an unpriced model
   * records cost=$0 post-call, so a configured cost cap could never
   * enforce against it (fail-OPEN). When this peek's quota is set AND the
   * call's model is unpriced, refuse up front — unless the gate was built
   * with `allowUnpricedModels`.
   */
  budgetPeeks?: Array<{
    metric: string
    period: UsagePeriod
    denyIfModelUnpriced?: boolean
  }>
  /**
...
/**
 * M8 — thrown by a `QuotaGate` when a configured cost cap can't meter the
 * call's model (the model is unpriced, so its cost would record as $0 and the
 * cap would never bite). Distinct from {@link QuotaExceededError}: nothing is
 * over budget — the budget is simply un-enforceable for this model.
 */
export class UnpricedModelDeniedError extends Error {
  readonly code = 'unpriced_model_denied' as const
  constructor(
    public readonly userId: string,
    public readonly metric: string,
    public readonly period: UsagePeriod,
...
/**
 * Write the `api_quota_denied` audit row (best-effort, tagged with the
 * `unpriced_model` reason) then throw {@link UnpricedModelDeniedError}.
 */
function denyUnpriced(
  identity: IdentityStore,
  orgIdForAudit: string | null,
  d: { userId: string; metric: string; period: UsagePeriod; quota: number },
): never {
  try {
    identity.writeAuditLog({
      action: 'api_quota_denied',
  ...
  throw new UnpricedModelDeniedError(d.userId, d.metric, d.period, d.quota)
}
...
export class OrgApiPool {
  ...
  /**
   * provider → resolved ORG key. Misses are cached as `null` so a "we know
   ...
   */
  private userCache = new Map<string, ResolvedLlmKey | null>()
  /**
  ...
  /**
   * v5 A-M3 — resolve a member's OWN ("bring your own") LLM key for
   * `provider`, or `null` when that member has none. Reads vault rows
   * with `kind='llm_provider' AND ownerKind='user' AND ownerId=userId`.
   *
   * This is the org pool's USER-scope fallback: the spawn path consults
   * {@link resolveLlmKey} (org-owned) FIRST, then this for an agent that
   * has an owning member — so an org-provided key stays the shared
   * default (central billing) and a personal key only kicks in when the
   * org configured none for that provider. Cached in a map distinct from
   * the org cache; both flush on vault mutation / `invalidate()`.
   */
  resolveUserLlmKey(provider: string, userId: string): ResolvedLlmKey | null {
    if (typeof provider !== 'string' || provider.length === 0) {
      throw new TypeError(
        'OrgApiPool.resolveUserLlmKey: provider must be a non-empty string',
      )
    }
    ...
    const entries = this.identity
      ...
      .filter((e) => readProviderTag(e) === provider)
    // listVaultEntries already returns newest-first.
    const chosen = entries[0]
    if (!chosen) {
    ...
    this.userCache.set(cacheKey, resolved)
    return resolved
  }
  ...
  makeLlmQuotaGate(opts: MakeLlmQuotaGateOpts): QuotaGate {
    ...
    const allowUnpricedModels = opts.allowUnpricedModels ?? false
    const identity = this.identity
    const orgIdForAudit = this.orgId
    return function quotaGate(
      subject: QuotaSubject | undefined,
      ctx?: QuotaCallContext,
    ): void {
      ...
      for (const b of budgetPeeks) {
        ...
        // M8 — fail-closed on the cost cap for an unpriced model. Its post-call
        // cost records as $0, so this cap (q !== null) would never enforce; deny
        // up front rather than let it spend without limit. Skipped when there's
        // no cost cap (q === null) or the operator allowed unpriced models.
        if (
          b.denyIfModelUnpriced &&
          q !== null &&
          ctx?.modelUnpriced &&
          !allowUnpricedModels
        ) {
          denyUnpriced(identity, orgIdForAudit, {
            userId,
            metric: b.metric,
            period: b.period,
            quota: q,
