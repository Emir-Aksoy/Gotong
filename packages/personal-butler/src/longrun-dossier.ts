...
export function renderRelayPrompt(
  ...
  parts.push(
    [
      ...
      // LONG-M6.2 的孪生刀(用户 2026-08-29 批「几项都做」):生产病根是被追问的
      // 模型往 journal 写未来日期的承诺,下一段当历史读回去。钟卡的告诫只管「读」
      // 且只在有钟时出现;这条管「写」,住在无条件的纪律块里。
      '- 日志只记已经发生的事:计划、约定、还没到的日期一律写进「下一步」,绝不写成已完成——下一段把日志当历史读,把未来写成过去,后面每一段都会当真。',
