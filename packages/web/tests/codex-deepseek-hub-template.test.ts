/**
 * Anti-rot acceptance gate for the codex-deepseek-hub loadable template (DST1).
 *
 * It reads the SHIPPED
 * `examples/codex-deepseek-hub/template/codex-deepseek-hub.template.yaml` off
 ...
 * is the "runnable proof" that the codex-deepseek-hub config is a LOADABLE file
 * (not a built-in TS literal): import the one file → the methodology-aware
 * pairing-mentor agent + an addressable Obsidian KB slot (with a presetData
 * POINTER, never content) land.
 *
 * Sibling to personal-coding-hub-template.test.ts — the two examples share the
 * SAME template structure (1 mentor + 1 KB slot, no workflow, the two coders
 * wired at runtime as CliParticipants). This gate asserts the codex-deepseek
 * variant's distinct identity (pairing-mentor + coder_pairing_methodology).
 */
...
describe('examples/codex-deepseek-hub/template (DST1)', () => {
  it('parses as a valid aipehub.template/v1 manifest', () => {
    ...
    expect(t.agents.map((a) => a.id)).toEqual(['pairing-mentor'])
    expect(t.workflows).toEqual([])
    expect(t.knowledgeBases.map((k) => k.name)).toEqual(['coder_pairing_methodology'])
  })
  ...
  it('imports end-to-end: the mentor agent lands, KB slot is reported', async () => {
    ...
    try {
      ...
      expect((await space.agents()).map((a) => a.id)).toContain('pairing-mentor')
      expect(json.team.created.map((a: any) => a.id)).toEqual(['pairing-mentor'])
      ...
      expect(json.knowledgeBases).toEqual([
        {
          ...
          description:
            '配对编码方法论知识库(推理主理 × 快手实现, Obsidian vault via mcp-obsidian)',
          wiring: 'inline',
