...
(() => {
  const I18N = {
    zh: {
      ...
      reallifeDirTitle: '用 X 登录',
      reallifeDirIntro: '挑一个服务,填你自己在其开发者后台注册的 OAuth 应用三件套(客户端 ID / 密钥 / 回调 URL),点「连接」跳转授权。',
      reallifeDisabled: '此主机未启用身份存储,无法连接 OAuth 服务',
      reallifeEmpty: '尚未连接任何服务',
      reallifeColConnector: '连接器',
      reallifeColProvider: 'MCP server',
      reallifeColScope: '范围',
      reallifeColStatus: '状态',
      reallifeCat: { calendar: '日历', email: '邮件' },
      reallifeConnect: '连接',
      reallifeReconnect: '重新连接',
      reallifeDisconnect: '断开',
      reallifeRemove: '移除',
      reallifeAdded: '已添加',
      reallifeConnected: '✓ 已连接',
      reallifeNotConnected: '未连接',
      reallifeExpiryMins: (mins) => (mins > 0 ? `(令牌 ~${mins} 分钟)` : '(将自动刷新)'),
      reallifeClientId: '客户端 ID（client_id）',
      reallifeClientSecret: '客户端密钥（client_secret）',
      reallifeRedirect: '回调 URL（redirect_uri）',
      reallifeConnectBtn: (name) => `连接 ${name}`,
      reallifeConfirmRemove: (name) => `确定移除「${name}」？将删除该连接器及其关联的 MCP server。`,
      reallifeConnectedBanner: (id) => `✓ 已连接「${id}」。该服务的工具现已可用。`,
      reallifeErrorBanner: (code) => `连接失败:${code}。请检查 OAuth 应用配置(回调 URL / 客户端密钥)后重试。`,
      // --- services tab (v2.2) ---
      ...
      wfAcceptSummary: (n) => `${n} 个包`,
      wfAcceptRunBtn: '跑验收',
      wfAcceptRunningBtn: '验收中…',
      wfAcceptRunning: '验收中——真实跑工作流（烧真 token），每条最多等 2 分钟。',
      wfAcceptGreen: '通过',
      wfAcceptRed: '未过',
      wfAcceptAllGreen: '全部通过',
      wfAcceptHasRed: (n) => `${n} 条未过`,
      wfAcceptDoneGreen: (pack) => `「${pack}」验收全绿。`,
      wfAcceptDoneRed: (pack) => `「${pack}」有用例未过，逐条看下面的红行。`,
      wfAcceptReason: (r) =>
        ({
          unrunnable: '不可跑（要已发布 + surface.me 开着 + 角色允许）',
          dispatch_failed: '运行失败',
          timeout: '超时（run 仍在后台跑，结果看「历史」）',
          suspended: '挂起——工作流里有人工步，黄金用例必须能无人值守跑完',
          assert_failed: '断言未过',
        })[r] || r,
      ...
      // WX-M3b — IM binding card (the /me face of GO-LIVE §六).
      meImBindTitle: '绑定 IM',
      meImBindHint: '在微信 / Telegram / 飞书等聊天里直接用你的 AI:点下面生成 6 位绑定码,然后在聊天里发给机器人:<code>/bind 你的码</code>。码 10 分钟有效;重新生成会作废旧码。',
      meImBindBtn: '生成绑定码',
      meImBindMinting: '生成中…',
      meImBindFailed: (e) => `生成失败: ${e}`,
      meImBindCodeMsg: (mins) => `在 IM 里发给机器人:<code>/bind</code> + 这个码(${mins} 分钟内有效)`,
      meImBindListTitle: '已绑定:',
      meImBindNone: '还没有绑定任何 IM。',
      meImBindUnbind: '解绑',
      meImBindUnbindFailed: (e) => `解绑失败: ${e}`,
      mePushTitle: '浏览器通知',
      mePushHint: '没绑 IM 也能被叫到:阿同有话找你时,浏览器弹一条「有新消息」提醒(不含内容),点开进这里看。绑了 IM 的成员走 IM,不会重复打扰。',
      mePushEnable: '开启通知',
      mePushDisable: '关闭通知',
      mePushOn: (n) => `已开启(本账号 ${n} 台设备)。`,
      mePushDenied: '浏览器已拒绝通知权限——要开启需先在浏览器设置里允许本站通知。',
      mePushFailed: (e) => `操作失败: ${e}`,
      meDeviceTitle: '连接手机 app',
      meDeviceHint: '在手机 app 里扫下面的二维码,就能连上这台 hub。二维码里同时带了地址和一次性配对码,10 分钟内有效;重新生成会作废旧的。扫不了就手输下面那串码。',
      meDeviceMint: '生成配对码',
      meDeviceMinting: '生成中…',
      meDeviceMintFailed: (e) => `生成失败: ${e}`,
      meDeviceQrAlt: '配对二维码',
      meDeviceCodeMsg: (m) => `扫码或手输上面这串码 · ${m} 分钟内有效,用过即失效`,
      meDeviceListTitle: '已连接的设备:',
      meDeviceNone: '还没有连接过设备。',
      meDeviceUntil: (d) => `有效至 ${d}`,
      meDeviceExpired: '已过期(需重新配对)',
      meDeviceNoExpiry: '长期密钥(不会过期)',
      meDeviceRevoke: '断开',
      meDeviceRevokeFailed: (e) => `断开失败: ${e}`,
      meExchangeTitle: '导入交付物文件',
      meExchangeHint: '朋友的本机 AI(pi / WorkBuddy 等)按标准生成的交付物 .json 文件,聊天里收到后在这里导入:先预览核对,再选一个工作流处理。处理完把结果文件下载下来,聊天里发回给对方就算送达。',
      meExchangeTarget: '用哪个工作流处理',
      meExchangeImportBtn: '确认导入',
      meExchangePreviewing: '正在解析文件…',
      meExchangeReadFailed: '读不了这个文件。',
      meExchangeFailed: (e) => `失败: ${e}`,
      meExchangeInvalid: '这不是一份有效的交付物信封:',
      meExchangeFrom: '来自',
      meExchangeTitleLabel: '标题',
      meExchangeCapability: '请求能力',
      meExchangeSigValid: '✓ 签名完整性通过(只证明文件没被改过,不证明发件人身份——发件人以聊天来源为准)',
      meExchangeSigInvalid: (r) => `✗ 签名校验失败: ${r}`,
      meExchangeSigUnsigned: '未签名(以聊天来源辨别发件人)',
      meExchangeOriginal: '原始请求（核验结果时选填）',
      meExchangeEvidence: (passed, failed, untested) => `验收：${passed} 项通过 / ${failed} 项失败 / ${untested} 项未测试`,
      meExchangeEvidenceConsistent: '证据与产物一致',
      meExchangeEvidenceInconsistent: '证据与产物不一致',
      meExchangeRequestMatched: '与原始请求一致',
      meExchangeRequestMismatch: '与原始请求不一致',
      meExchangeRequestMissing: '未提供原始请求，未确认验收要求',
      meExchangeReplayMine: (s) => `这份交付物你已经导入过了(状态: ${s})。`,
      meExchangeReplayTheirs: '这份交付物已被其他成员导入过。',
      meExchangeIsResult: '这是一份「结果」信封(对方发回的处理结果),不能当请求导入。',
      meExchangeNoTargets: '没有能处理这份交付物的工作流(能力不匹配,或没有对你开放)。',
      meExchangeImporting: '导入中…',
      meExchangeRunning: '处理中…稍等一下。',
      meExchangeSuspended: (n) => `停在需要人拍板的一步(${n})。批准后点「刷新结果」。`,
      meExchangeDone: '处理完成。下载结果文件,聊天里发回给对方。',
      meExchangeDownload: '下载结果文件',
      meExchangeRefresh: '刷新结果',
      meAdvancedTools: '更多工具（发起工作流 / 搭助手 / 密钥 / 记忆…）',
      meSubmitting: '提交中…',
      meSavingDots: '保存中…',
      meSaved: '已保存',
      meCreated: '已创建',
      ...
      meStewardTyping: '管家正在草拟回复…',
      meStewardStreamBroken: '连接中断,没有收到管家的完整回复。请重试。',
      meChatStreamBroken: '连接中断,没有收到完整回复。请重试。',
      ...
      // --- EFF-M3 — 效果信号卡(内部回路仪表;数字只与自己比) ---
      healthEffectTitle: (days) => `效果信号(近 ${days} 天)`,
      healthEffectParks: (a, r, c, pct) => `park 决定:批准 ${a} · 拒绝 ${r} · 打回 ${c}${pct !== null ? `(打回率 ${pct}%)` : ''}`,
      healthEffectParksUnreadable: 'park 决定:读不到(详见服务器日志)',
      healthEffectEscalations: (total, ok) => `转派:${total} 次,其中成功 ${ok}`,
      healthEffectEscalationsUnreadable: '转派:读不到(详见服务器日志)',
      healthEffectCalls: (n, parkPer, escPer) => `LLM 调用 ${n} 次${parkPer !== null || escPer !== null ? ` — 每百次:${[parkPer !== null ? `park ${parkPer}` : null, escPer !== null ? `转派 ${escPer}` : null].filter(Boolean).join(' · ')}` : ''}`,
      healthEffectNoDenominator: 'LLM 调用分母未接或读不到 — 不折比率',
      // --- STOR-M1 — 空间账本卡(只丈量不删除) ---
      healthSpaceTitle: '空间账本',
      healthSpaceNotYet: '还没有丈量记录。账本随 6 小时维护节律更新,开机也会先量一次。',
      healthSpaceTotal: (size, n) => `总占用 ${size},共 ${n} 个条目。只丈量,不删除任何东西。`,
      healthSpaceCat: (id, size, n) => `${id}:${size}(${n} 条)`,
      healthSpaceMore: (n) => `(还有 ${n} 个更小的类目未列出)`,
      healthSpaceTruncated: '丈量被截断:条目数超过上限,以上数字是下界,不是全量。',
      ...
      padmPolPinnedKid: '锚定签名公钥 (kid)',
      padmPolPinnedKidHint: '(带外记下的 RFC 7638 指纹, 43 字符; 留空=不锚定。仅提醒, 绝不挡联邦)',
      padmPolTrustTier: '信任档 (trust tier)',
      padmPolTrustTierHint: '(选定的信任级别: 只调审批摩擦, 不撤确认底线; 留空=未分级, 回落 T1 地板。仅提醒)',
      padmPolTrustTierNone: '未分级 (回落 T1)',
      padmPolTrustTierT0: '仅可发现',
      padmPolTrustTierT1: '令牌 (默认地板)',
      padmPolTrustTierT2: '已验证 (已锚定公钥)',
      padmPolTrustTierT3: '完全信任',
      padmPolSave: '保存策略',
      ...
      setupKeySubmit: '保存并继续',
      setupKeySkip: '跳过,以后再配',
      setupKeyNeed: '请填入 API Key,或点「跳过」。',
      setupKeySaving: '保存中…',
      setupKeySaved: 'Key 已保存,进入下一步…',
      // 部署摩擦 ⑤ — first-run agent step (wizard step 3).
      setupAgentTitle: '第三步 (可选) — 创建第一个智能体',
      setupAgentIntro: '给 hub 一个能对话的智能体(贴身管家「阿同」)。不建的话,成员在 IM 里说话没人接。人设以后随时可改。',
      setupAgentName: '名字',
      setupAgentProvider: '模型提供方',
      setupAgentModel: '模型名',
      setupAgentModelPh: '如 deepseek-chat',
      setupAgentModelHint: '选 DeepSeek 时已预填 deepseek-chat;其他提供方请填厂商当前的模型名(写错会导致对话报错,可随时在管理面板改)。',
      setupAgentSubmit: '创建并继续',
      setupAgentSkip: '跳过,以后再建',
      setupAgentNeedModel: '请填模型名,或点「跳过」。',
      setupAgentSaving: '创建中…',
      setupAgentLive: '阿同已上线!下一步接 IM。',
      setupAgentSavedRestart: '已保存,重启 host 后上线。下一步…',
      setupAgentExisting: '已有能对话的智能体,直接下一步…',
      // DEPLOY-B2 — first-run IM step (wizard step 4).
      setupImTitle: '第四步 (可选) — 接一个 IM 机器人',
      ...
      tabHome: '我的',
      tabPanel: '面板',
      // SDUI panel strings (sdui*) moved to sdui-ui.js in SHELL-M4 — the
      // renderer owns its own copy so it can ship without this file.
      tabUsers: '用户',
      tabQuotas: '配额',
      tabUsage: '用量',
      tabReputation: '信誉',
    ...
    en: {
      ...
      reallifeDirTitle: 'Sign in with X',
      reallifeDirIntro: 'Pick a service, fill in your own OAuth app’s three fields (client ID / secret / callback URL) registered in its developer console, and click Connect to authorize.',
      reallifeDisabled: 'No identity store on this host — cannot connect OAuth services',
      reallifeEmpty: 'No services connected yet',
      reallifeColConnector: 'Connector',
      reallifeColProvider: 'MCP server',
      reallifeColScope: 'Scope',
      reallifeColStatus: 'Status',
      reallifeCat: { calendar: 'Calendar', email: 'Email' },
      reallifeConnect: 'Connect',
      reallifeReconnect: 'Reconnect',
      reallifeDisconnect: 'Disconnect',
      reallifeRemove: 'Remove',
      reallifeAdded: 'Added',
      reallifeConnected: '✓ Connected',
      reallifeNotConnected: 'Not connected',
      reallifeExpiryMins: (mins) => (mins > 0 ? `(token ~${mins}m)` : '(will refresh)'),
      reallifeClientId: 'Client ID (client_id)',
      reallifeClientSecret: 'Client secret (client_secret)',
      reallifeRedirect: 'Callback URL (redirect_uri)',
      reallifeConnectBtn: (name) => `Connect ${name}`,
      reallifeConfirmRemove: (name) => `Remove “${name}”? This deletes the connector and its linked MCP server.`,
      reallifeConnectedBanner: (id) => `✓ Connected “${id}”. Its tools are now available.`,
      reallifeErrorBanner: (code) => `Connect failed: ${code}. Check the OAuth app config (callback URL / client secret) and retry.`,
      // --- services tab (v2.2) ---
      ...
      wfAcceptSummary: (n) => `${n} pack${n === 1 ? '' : 's'}`,
      wfAcceptRunBtn: 'Run acceptance',
      wfAcceptRunningBtn: 'Running…',
      wfAcceptRunning: 'Running — real workflow runs (real tokens); up to 2 minutes per case.',
      wfAcceptGreen: 'pass',
      wfAcceptRed: 'fail',
      wfAcceptAllGreen: 'all green',
      wfAcceptHasRed: (n) => `${n} failed`,
      wfAcceptDoneGreen: (pack) => `"${pack}" acceptance is all green.`,
      wfAcceptDoneRed: (pack) => `"${pack}" has failing cases — see the red rows below.`,
      wfAcceptReason: (r) =>
        ({
          unrunnable: 'not runnable (needs published + surface.me enabled + role allowed)',
          dispatch_failed: 'run failed',
          timeout: 'timed out (the run continues in the background — see "History")',
          suspended: 'suspended — the workflow has a human step; golden cases must run unattended',
          assert_failed: 'assertions failed',
        })[r] || r,
      ...
      // WX-M3b — IM binding card (the /me face of GO-LIVE §6).
      meImBindTitle: 'Link IM',
      meImBindHint: 'Use your AI right inside WeChat / Telegram / Lark: mint a 6-digit code below, then DM the bot <code>/bind &lt;code&gt;</code>. Codes last 10 minutes; minting again rotates the old one out.',
      meImBindBtn: 'Mint binding code',
      meImBindMinting: 'Minting…',
      meImBindFailed: (e) => `Minting failed: ${e}`,
      meImBindCodeMsg: (mins) => `DM the bot <code>/bind</code> + this code (valid ${mins} min)`,
      meImBindListTitle: 'Linked:',
      meImBindNone: 'No IM linked yet.',
      meImBindUnbind: 'Unlink',
      meImBindUnbindFailed: (e) => `Unlink failed: ${e}`,
      mePushTitle: 'Browser notifications',
      mePushHint: 'Reachable without an IM: when Atong has something for you, this browser shows a low-info "new message" alert (no content) — tap it to read here. Members with an IM linked are reached there instead, never both.',
      mePushEnable: 'Enable notifications',
      mePushDisable: 'Disable notifications',
      mePushOn: (n) => `Enabled (${n} device${n === 1 ? '' : 's'} on this account).`,
      mePushDenied: 'The browser has denied notification permission — allow notifications for this site in browser settings first.',
      mePushFailed: (e) => `Failed: ${e}`,
      meDeviceTitle: 'Connect the phone app',
      meDeviceHint: 'Scan this from the phone app to connect it to this hub. The QR carries both the address and a one-shot pairing code, good for 10 minutes; minting a new one retires the old. If the phone cannot scan, type the code below instead.',
      meDeviceMint: 'Create pairing code',
      meDeviceMinting: 'Creating…',
      meDeviceMintFailed: (e) => `Could not create a code: ${e}`,
      meDeviceQrAlt: 'Pairing QR code',
      meDeviceCodeMsg: (m) => `Scan, or type the code above · valid ${m} min, single use`,
      meDeviceListTitle: 'Connected devices:',
      meDeviceNone: 'No devices connected yet.',
      meDeviceUntil: (d) => `valid until ${d}`,
      meDeviceExpired: 'expired (pair again)',
      meDeviceNoExpiry: 'long-lived key (never expires)',
      meDeviceRevoke: 'Disconnect',
      meDeviceRevokeFailed: (e) => `Disconnect failed: ${e}`,
      meExchangeTitle: 'Import a deliverable file',
      meExchangeHint: "A friend's local AI (pi / WorkBuddy etc.) writes a standard deliverable .json file; when it reaches you in chat, import it here: preview first, then pick a workflow to process it. When it finishes, download the result file and send it back in chat — that's delivery.",
      meExchangeTarget: 'Process with which workflow',
      meExchangeImportBtn: 'Confirm import',
      meExchangePreviewing: 'Parsing the file…',
      meExchangeReadFailed: 'Could not read this file.',
      meExchangeFailed: (e) => `Failed: ${e}`,
      meExchangeInvalid: 'Not a valid deliverable envelope:',
      meExchangeFrom: 'From',
      meExchangeTitleLabel: 'Title',
      meExchangeCapability: 'Requested capability',
      meExchangeSigValid: '✓ Signature intact (proves the file was not altered — NOT who sent it; trust the chat sender)',
      meExchangeSigInvalid: (r) => `✗ Signature check failed: ${r}`,
      meExchangeSigUnsigned: 'Unsigned (judge by who sent it in chat)',
      meExchangeOriginal: 'Original request (optional when verifying a result)',
      meExchangeEvidence: (passed, failed, untested) => `Acceptance: ${passed} passed / ${failed} failed / ${untested} untested`,
      meExchangeEvidenceConsistent: 'Evidence matches the artifact',
      meExchangeEvidenceInconsistent: 'Evidence does not match the artifact',
      meExchangeRequestMatched: 'Original request matches',
      meExchangeRequestMismatch: 'Original request does not match',
      meExchangeRequestMissing: 'Original request absent; requirements not confirmed',
      meExchangeReplayMine: (s) => `You already imported this deliverable (status: ${s}).`,
      meExchangeReplayTheirs: 'This deliverable was already imported by another member.',
      meExchangeIsResult: 'This is a RESULT envelope (a reply someone sent back); it cannot be imported as a request.',
      meExchangeNoTargets: 'No workflow can take this deliverable (capability mismatch, or none is open to you).',
      meExchangeImporting: 'Importing…',
      meExchangeRunning: 'Processing… one moment.',
      meExchangeSuspended: (n) => `Waiting on a human step (${n}). Refresh after it is approved.`,
      meExchangeDone: 'Done. Download the result file and send it back in chat.',
      meExchangeDownload: 'Download result file',
      meExchangeRefresh: 'Refresh result',
      meAdvancedTools: 'More tools (start a workflow / build helpers / keys / memory…)',
      ...
      meStewardTyping: 'The steward is drafting a reply…',
      meStewardStreamBroken: 'Connection dropped before the full reply arrived. Please retry.',
      meChatStreamBroken: 'Connection dropped before the full reply arrived. Please retry.',
      ...
      // --- EFF-M3 — effect-signals card (internal-loop gauge; numbers compare only to ourselves) ---
      healthEffectTitle: (days) => `Effect signals (last ${days} days)`,
      healthEffectParks: (a, r, c, pct) => `Park decisions: approved ${a} · rejected ${r} · changes requested ${c}${pct !== null ? ` (rework ${pct}%)` : ''}`,
      healthEffectParksUnreadable: 'Park decisions: unreadable (see server logs)',
      healthEffectEscalations: (total, ok) => `Escalations: ${total}, of which ${ok} succeeded`,
      healthEffectEscalationsUnreadable: 'Escalations: unreadable (see server logs)',
      healthEffectCalls: (n, parkPer, escPer) => `LLM calls: ${n}${parkPer !== null || escPer !== null ? ` — per 100 calls: ${[parkPer !== null ? `park ${parkPer}` : null, escPer !== null ? `escalate ${escPer}` : null].filter(Boolean).join(' · ')}` : ''}`,
      healthEffectNoDenominator: 'LLM-call denominator unwired or unreadable — no ratios',
      // --- STOR-M1 — disk-ledger card (measures only; deletes nothing) ---
      healthSpaceTitle: 'Disk ledger',
      healthSpaceNotYet: 'No census yet. The ledger refreshes on the 6-hour maintenance cadence, and once at boot.',
      healthSpaceTotal: (size, n) => `Total ${size} across ${n} entries. Measures only — deletes nothing.`,
      healthSpaceCat: (id, size, n) => `${id}: ${size} (${n} entries)`,
      healthSpaceMore: (n) => `(${n} smaller categories not listed)`,
      healthSpaceTruncated: 'Census truncated: entry count hit the cap, so figures are lower bounds.',
      ...
      padmPolPinnedKid: 'Pinned signing key (kid)',
      padmPolPinnedKidHint: '(out-of-band RFC 7638 thumbprint, 43 chars; blank = no anchor. Advisory, never blocks federation)',
      padmPolTrustTier: 'Trust tier',
      padmPolTrustTierHint: '(the chosen trust level — only lowers approval friction, never removes a confirmation floor; blank = un-graded, falls back to the T1 floor. Advisory)',
      padmPolTrustTierNone: 'Un-graded (T1 floor)',
      padmPolTrustTierT0: 'Discoverable only',
      padmPolTrustTierT1: 'Token (default floor)',
      padmPolTrustTierT2: 'Verified (pinned key)',
      padmPolTrustTierT3: 'Trusted',
      ...
      usgColCacheRead: 'Cache read',
      usgColCacheHit: 'Cache hit',
      ...
      setupKeySubmit: 'Save and continue',
      ...
      // 部署摩擦 ⑤ — first-run agent step (wizard step 3).
      setupAgentTitle: 'Step 3 (optional) — create your first agent',
      setupAgentIntro: 'Give the hub an agent that can talk (the resident butler "Atong"). Without one, IM messages go unanswered. The persona can be changed any time.',
      setupAgentName: 'Name',
      setupAgentProvider: 'Model provider',
      setupAgentModel: 'Model id',
      setupAgentModelPh: 'e.g. deepseek-chat',
      setupAgentModelHint: 'DeepSeek comes prefilled with deepseek-chat; for other providers type the vendor\'s current model id (a wrong id breaks chat — fixable any time in the admin panel).',
      setupAgentSubmit: 'Create and continue',
      setupAgentSkip: 'Skip, create later',
      setupAgentNeedModel: 'Enter a model id, or click "Skip".',
      setupAgentSaving: 'Creating…',
      setupAgentLive: 'Atong is live! Next: connect an IM.',
      setupAgentSavedRestart: 'Saved — starts on the next host restart. Next…',
      setupAgentExisting: 'A chat-capable agent already exists — moving on…',
      // DEPLOY-B2 — first-run IM step (wizard step 4).
      setupImTitle: 'Step 4 (optional) — connect an IM bot',
