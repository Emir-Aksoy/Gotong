...
description: 与 Gotong hub 的用户互发任务/成果文件(gotong.envelope/v1 标准交付物信封)。当用户要「发任务给对方的 hub/阿同」「答复收到的 gotong 请求」「看收件箱/gotong-in 里的文件」,或把微信里收到的 .json 交付物信封拿来处理、提到 gotong-out/gotong-in 目录时使用。
---
...
- `gotong-in/` — 用户把收到的信封文件放进来(微信里收到的 `.json` 先保存到本地,
  再挪进这个目录),你按需解析。不监听、不轮询。

## 唯一入口:脚本(不要手写信封文件)

本技能自带零依赖脚本 `scripts/validate.py`(相对本技能目录解析;只用 Python 标准库,
需要 Python ≥ 3.9,命令统一走 `python3`)。
**信封只能经这个脚本产出/解析**——脚本内部做完整 schema 校验(fail-closed,
不合格会把全部错误一次报出来,修正后重试)。绝不绕过脚本直接往 `gotong-out/`
手写 JSON:手写的文件没过校验,对方 hub 会拒收。

下文用 `$SKILL` 代指本技能目录(即本 SKILL.md 所在目录;按宿主给出的技能路径解析)。

### 发信封(在项目目录里运行;草稿 JSON 走 stdin)

```bash
python3 "$SKILL/scripts/validate.py" emit <<'EOF'
{"kind": "request",
 "title": "请分析一下今天恒指的走势",
...
- `from_name` 是对方看到的署名,先问用户想署什么名(建议「真名 (WorkBuddy @ 设备)」)。
...
```bash
# 列出收件箱
python3 "$SKILL/scripts/validate.py" ingest
# 读一份(裸文件名,不带路径)
python3 "$SKILL/scripts/validate.py" ingest exg-xxxx.json
# 只校验任意一份本地信封文件(只读,不落任何副作用)
python3 "$SKILL/scripts/validate.py" validate ./某份信封.json
```
...
3. **签名只证完整性,不证发件人。** 本机(纯 Python)不做 ES256 数学验签,脚本最多报
   「◐ 结构完好、kid 绑定一致」——无论签没签、验没验,发件人是谁一律以用户聊天里的
   来源为准。未签名完全合法;报 `✗ 无效` 则文件可能被改动过,谨慎对待。
