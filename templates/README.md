...
> **正式公网模板库**会迁到独立仓 **`Gotong/gotong-templates`**。
...
3. 回到你的 Gotong admin 控制台，点 **导入**，把内容粘贴进去
...
teams/                     多智能体团队模板（项目原创，一次导入 N 个）
  ...
  traditional-industry-ai-enablement.yaml
                           ⭐ 传统行业 AI 赋能梳理（6 角色：
                           诊断 → 机会 → 工具 → 落地 → 顾虑 → 通俗教练）
  admin-task-orchestration.yaml
                           ⭐ 行政任务编排（8 角色：解析 → 拆解 →
                           联络 → 派发 → 跟踪 → 写报告 → 归档 → 改口径）

workflows/                 工作流模板（@gotong/workflow 配套，一键串联多 agent）
  editorial-flow.yaml      中文编辑流水线（writer → reviewer）
  admin-task-flow.yaml     ⭐ 行政任务全流程（解析→拆解→[并行: 联络+派发]→报告→归档）
  admin-report-restyle-flow.yaml
                           ⭐ 报告改写专用（旧报告 + 新口径 → 同事实新表达）
  industry-enablement-flow.yaml
                           ⭐ 传统行业 AI 赋能（诊断→机会→工具→落地→顾虑）
...
**单 agent（`schema: gotong.agent/v1`）**
