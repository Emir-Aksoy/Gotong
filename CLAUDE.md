...
- **NET agent 网络 A track 全完：管家出网 M0→M3（2026-07-06，`d105712`→`af48654` 共 4 commit）**
  ...
  **显式推迟**（①按 task.from 推回要开 inbox-service/im-bridge 间新缝还得辨任务类别 ②BE-M5 面
  ...
  requireApprovalOutbound，高敏边 owner 转达）+ PARTICIPANT/OVERVIEW 指针。验收：host 1861 全绿
  （15 单测+4 e2e 新增），四门 PASS（main 2973/2980 不动）。B track（M4 名片/M5 connect
  preflight）动工前先重侦察 A2A agent-card 等标准现状。
...
更早里程碑（FDE 前置部署 track M0→M2（playbook 五段流水线 + 连接器槽位 name-identity +
