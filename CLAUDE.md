...
| 控制面历史趋势 + 告警阈值 (peer.summary 快照/趋势 sparkline + 告警规则纯求值, counts-only, E5 day-2; **day-3** 告警投递: firing 边沿触发 v28 + webhook 通道无密钥 v29 + best-effort dispatcher + opt-in sweep + 触发历史/通知渠道 UI; **多通道** im[telegram/slack/discord/lark]/email 通道 v30 无状态平台 send + 重试退避 + 进程内去重窗) | `docs/zh/V5-F-FINAL.md` |
