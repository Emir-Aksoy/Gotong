...
| **下一步** | — | v5 Stream D + 0 + A 完. v5 路线推荐顺序 D→0+A→B→C: 接下来 **Stream B** (B-M1 模板 manifest 格式+parse+validate → B-M2 模板导出 [结构, 默认无内容/人员] → B-M3 内容/人员导出权限闸 [加密/省略+审计] → B-M4 模板导入 → B-M5 一键模板自带 KB 示例+文档) → **C** (联邦授权细化: C-M1 可调用知识库入 per-link 契约 → C-M2 workflow 节点级 I/O 授权 → C-M3 多组织隔离 E2E 扩展 + V5-C-FINAL). 锁定决策: v5 版本号 / 心跳复用 suspended_tasks / agent-owner 受限子集 / 单 `resource_grants` 表 / 导出结构明文·内容加密·人员默认省略. 按 /goal 全按推荐逐里程碑执行中 |
