...
| **v4 Phase 12 M9-M11** | **下一步（可选）** | PWA + mobile responsive + 移动简化 shell (跟主题正交, 等 IM bridges 实战反馈再做) |
| **Phase 13 M4-M5** | **下一步** | M4: `@aipehub/evals` 深度检查 (agent ref / dispatch 环) ; M5: real-LLM example + release notes |
...
| AI 辅助 workflow 编辑 | ✓ Phase 13 M1+M3 完 (assistant agent + admin UI 对话框 + status chip + 保存); M4-M5 加深度校验 + real-LLM demo | — |
...
| 优先 | 任务 | 工作量 |
|---|---|---|
| 现在 | Phase 13 M4: 复用 `@aipehub/evals` 做 generated workflow 深度检查 (agent 引用是否存在 / dispatch 链是否环 / capability 是否合法) | 1 天 |
| 现在 | Phase 13 M5: real-LLM example 项目 (Anthropic 或 DeepSeek end-to-end demo) + `docs/zh/AI-WORKFLOW-EDITOR.md` release notes | 半天 |
| 短期 | Phase 13 M3 的 templates/workflows few-shot 注入 (当前 examples 数组为空, 让 LLM 看 6 个内置模板生成更准) | 半天 |
| 短期 | 2026-05-27 audit P3: 拆 `packages/web/src/server.ts` (3563 行) 的 route groups, 沿用 `identity-routes.ts` / `me-routes.ts` 模式 | 2-3 天 |
| 短期 | 2026-05-27 audit P3: 拆 `packages/web/static/admin.js` (3265 行 + Phase 13 M3 又加了 ~120 行) 按 feature 成多个 static script | 1-2 天 |
| 中期 | Phase 12 M9-M11 PWA + mobile responsive + 移动简化 shell (跟主题正交, 等 IM bridges 实战反馈再做) | 1-2 周 |
| 中期 | 默认 RAG MCP server 推荐 + setup 文档 | 半周 |
