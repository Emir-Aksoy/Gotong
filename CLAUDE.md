...
| **下一步** | — | v5 Stream D + 0 + A + B 完. v5 路线推荐顺序 D→0+A→B→C, 只剩 **Stream C** (联邦授权细化, 接 B 的可寻址 KB 槽位): C-M1 可调用知识库维度入 per-link 契约 (B 的 KB 槽位 `name` 就是授权目标) → C-M2 workflow 节点级 I/O 授权 → C-M3 多组织隔离 E2E 扩展 + V5-C-FINAL. 锁定决策: v5 版本号 / 心跳复用 suspended_tasks / agent-owner 受限子集 / 单 `resource_grants` 表 / 导出结构明文·内容加密·人员默认省略. 按 /goal 全按推荐逐里程碑执行中 |
