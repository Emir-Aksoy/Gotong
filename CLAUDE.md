...
| **v4 Phase 6** | **进行中** | 已落 6 项 + P0+P1 audit 7 项, P2/P3 backlog 待清 |
...
**Phase 6 P0+P1 audit 已修(#141-147)**:
FixedWindowLimiter 内存膨胀 / 限速 IP 走 XFF / HELLO 状态机覆盖 /
IdentityError → HTTP status / OrgApiPool vault 感知 / revoke 失败死循环 /
audit metadata PII 清洗。

**Phase 6 P2/P3 待办(`TaskList` 里全有, 不在这里复制)**: #148-#157
共 10 项。约半小时清完 P2(#148-#152), P3 不影响 launch。
...
| 范式 | 现状 | 短期可补 |
|---|---|---|
| LLM streaming | `LlmProvider` 无 stream 接口 | 加 `LlmProvider.stream?(req): AsyncIterable<chunk>`, workflow runner 透传, SSE 吐到 admin UI |
| 多模态 content | `LlmMessage.content` 只装 text + tool blocks | 扩 image_blob / file_ref blocks, providers 翻译到各家 SDK |
| Agent → 子 agent | LlmAgent 只能调 tool, 不能 dispatch | 加 `LlmAgentToolset.dispatch` toolset, 让 agent 通过 tool-use 调 capability |
