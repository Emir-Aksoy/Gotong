...
| **下一步** | (Phase 13 完整完成) | 没有强制路线 — 备选: admin.js 转 ES module + bundler 后再拆 / PWA + mobile (Phase 12 M9-M11) / 微信小程序 |
...
| 优先 | 任务 | 工作量 |
|---|---|---|
| ~~短期~~ | ~~2026-05-27 audit P3: 拆 `packages/web/src/server.ts` (3563 行) 的 route groups~~ | **2026-05-28 两批完成** — batch 1 `workflow-routes.ts` (3701→3578); batch 2 `agents-routes.ts`/`services-routes.ts`/`uploads-routes.ts` (3578→2780, -798); 剩 setup 路由可选拆 |
