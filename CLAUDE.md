...
- **FDE 前置部署 track（M0→M2 收口）** — 用户战略定调：框架要
  ...
  **现算**永不过期；未挂一律**黄牌**——optional:false 也不红，红牌只留 host 亲证事实，第三方
  ...
  四门 PASS（main 3247/3250）。M3（按方案开荒一条命令）在途。
...
- **LIFE 定时工作流（L1 M1→M3 + L2① 晨报 bundle 全完，默认开零旋钮）** — 用户诉求「在工作和生活中产生 agent
  ...
  **模板只带结构不带人员**——userId 进不了模板，装完补一条调度）+ 单装
  ...
  详见 [`docs/zh/WORKFLOW-SCHEDULES.md`](docs/zh/WORKFLOW-SCHEDULES.md)。L4（生产实例
  dogfooding，用户操作项）在途；OC-0 OpenClaw 对接侦察等 LIFE 出货。
更早里程碑（DEPLOY 部署简易性 A→C 全完（cloud-quickstart 一条命令 + IM token 落 vault + 向导
