...
- **MR-M6 per-candidate env 凭证 + 生产 LongCat 一顺/MiMo 兜底（2026-07-17，本 commit）** — 用户「弄了一个美团
  LongCat 的 api,部署到服务端阿同并设为默认第一顺序模型」。侦察撞上 MR-M2 显式推迟的结构缺口:候选链共享**同一把
  per-agent key**(space.ts fallbacks 注释原话「需要 per-candidate 凭证时再扩」),LongCat 主 + MiMo 备两家
  openai-compatible 各自的 key 结构性配不出来(per-agent 一格、org pool 按 provider 字符串撞名);岔口摆给用户,
  拍板**「先补缝再换刀」**。**M6 = 那句「再扩」**:`ManagedAgentSpec.apiKeyEnv` + `FallbackCandidate.apiKeyEnv`
  (存 env **变量名**如 `LONGCAT_API_KEY`,key 值只活在 gotong.env——与 MCP `${NAME}`/TAVILY 同一凭证纪律,manifest
  校验按标识符形状拒 key 样值粘错当场 400);**排他语义**=设了只认 `process.env[名]`,缺失=无 key 响亮失败(主 spawn
  拒/候选 warn 跳过),**绝不静默回落**存量链——那里可能是另一家的 key,借用=拿 MiMo 钱包打 LongCat 端点得撒谎 401;
  未设=解析链逐字节不变。`resolveApiKey(agentId,provider,apiKeyEnv?)` 咽喉收口七调用点:spawn/路由候选/**M5 测试
  路由探针**/**CARE 断供探活**(两探针必须测路由真用的那把 key,否则按钮测错钱包、CARE 每轮误报)/管家 provider/
  key 有无判定。admin-only(me-routes 结构性无此面),面板 capture-echo 镜像 maintenanceModel 先例(`_editingApiKeyEnv`,
  fallback 级随 `_editingFallbacks` 整数组回显免费生存)。**生产落地**:LongCat 主走 `apiKeyEnv`(env 快路),MiMo 兜底
  **不带** apiKeyEnv=沿用既有加密 per-agent key **零迁移**;LongCat 官方 OpenAI 兼容端点 `https://api.longcat.chat/openai/v1`
  (2026-07-16 WebSearch+官方 docs 核准,模型名部署时 GET /models 实证)。验收:host routing 11(+5:双厂商 key 分离/
  死候选跳过不借 key/主 env 排他/探针同源/CARE 目标 env key),web manifest 107(+7)/agents-route 28(+5),host 2223/
  web 1387 全绿,四门 PASS(**旋钮 115 零新增**——env 名是 spec 数据、key 值是凭证非旋钮;main.ts 3000/3000 零触碰)。
  见 [`docs/zh/MODEL-ROUTING.md`](docs/zh/MODEL-ROUTING.md) §3.4。
- **阿同图书馆员 track（LIB）全完：M0 计划 + M1 立尺 + M2 文件工具 + M3 索引卡 + M4 图书馆员 + capstone
  （2026-07-16/17，`5bfa48d`→本 commit 共 6 commit）** — 用户战略定方向「atong 进一步进化的方向在于自主管理自己的
  大量知识文件,它需要自己编排层级,不要浪费上下文,同时又能管理好知识(包括我的状态、它的状态、它在做的事情、它手上
  的工具、它的知识)」。**M0 侦察**(`5bfa48d`,纯 docs `docs/zh/ATONG-LIBRARIAN.md`):五域盘点=四域有器官(我的状态
  =记忆蒸馏/它的状态=SEN my_status/在做的事=TN 笔记本/工具=AFR 目录)、**「它的知识」是真缺口**——semantic.jsonl 进货
  区每条活跃事实进冻结块=每轮都付,没有「按需付费的书架」;三岔口用户全拍板取推荐项(①索引卡进稳定缓存段 ②双层:
  进货+上架 ③后台图书馆员 opt-in 新旋钮)。**五条不可破边界**:①热路径零 LLM(索引卡纯渲染,图书馆员只在 6h 维护)
  ②缓存前缀稳定(索引卡在稳定段,内容不变字节不变)③知识≠授权(文件工具 benign 只读写自己书架,对外动作仍 governed)
  ④opt-in 未开字节不变 ⑤内核零改动(全在 personal-butler/host 层)。**M1 立尺**(`bcac07c`):`pnpm report:atong-context`
  每轮 prompt 段级 token 构成报告(冻结块/探针/工具面逐段量)+ 注入点 tripwire(composeContextProbes 调用点正则扫源
  ≡ 报告注册表,加注入点不登记就红)——先有尺后动刀,M3 的「≤500tk」「字节不变」全用它量。**M2 文件工具**(`6d6f1c6`):
  `knowledge-library.ts` 纯核上架区(`<ownerDir>/knowledge/` 每成员 .md 树,200 文件/32KB 文件/4MB 树/6 深硬顶响亮拒,
  tmp+rename,`archive/` 挪走不真删,INDEX.md 不可归档)+ 4 个 benign 文件工具进 AFR 目录层(list/read/write/archive,
  写只写自己书架)。**M3 索引卡**(`7e4a6c9`):`buildButlerKnowledgeIndexCard` 把自著 INDEX.md 渲染进 system prompt
  **稳定段**(岔口①:导航天天用,放 volatile 段=每轮重付缓存)——≤500tk 顶(超了截断+响亮 `只显示前 N/M 行` 标记),
  无书架/空 INDEX=null=prompt 字节不变(防腐门断言),树再大卡不长=「常驻段不随知识总量长」的结构性保证。**M4 图书馆员**
  (`af8c5b7`):第五个自门控 reviewer 进 6h 维护链(M-RECON 同款姿态,位序=reconcile 后 link 前),opt-in
  `GOTONG_BUTLER_MEMORY_LIBRARIAN`(**114→115,本 track 唯一新旋钮**)——把进货区主题类 ad-hoc 事实「上架」进 knowledge/
  并重写 INDEX;**一次模型调用的合同**:自门控(候选<12→`{}` 零 LLM,上架即关区间候选集单调收敛,maxBatch=40 是步频
  不是丢弃)/**写前退后**(文件先落盘才动记忆,崩溃留重影永不留失踪;下架=CLOSE+promotedTo **一次** patchMeta,无半态,
  可逆)/fail-soft 全家(坏 JSON 零操作、库层拒只跳那条事实绝不下架、幻觉 factIds 关不掉任何东西、纯幻觉 promotion
  不碰盘)/INDEX 兜底(动过的文件必须指得到,机械补指针;零上架 INDEX 零字节动)/双写者硬化(tmp 名唯一化,后 rename
  整篇赢)。**capstone**(本 commit)`examples/atong-librarian` 四幕自断言(真件零重写,零网络零 key 零 LLM):幕1 进货→
  上架(文件先落盘才双时态下架+可逆+出处,第二 tick 计数器实证零调用)/幕2 百文件树(策展层级 INDEX 下树 3→107 份 25×,
  常驻卡 120tk **逐字节不变**——M1 尺实证「常驻段不随知识总量长」;胖索引病态被 ≤500tk 顶封死)/幕3 归档不丢(archive/
  前缀照读逐字节同,INDEX 拒归档)/幕4 知识≠授权(真管家真 M3 卡接 stableContext 导航→读出「待办:发预算表」→对外发送
  照样 park,批准前零发送);`pnpm demo:atong-librarian` exit 0;host 补 `./butler-knowledge-index`+`./butler-toolface-report`
  两 dist 子路径导出(routing-health 先例,demo 引真件不复刻)。**显式推迟**:defrag 整文件重组(等真实使用信号)/prune
  archive(与「归档不真删」立场冲突,需 retention 岔口)/SKILL.md 写端(BF-M8 起推迟不借道复活)/index-only 重写(索引
  变更锚定真上架防幻觉)。验收:personal-butler 98/host 2218 全绿,demo exit 0,四门 PASS(**旋钮 115 全登记**,main.ts
  3000/3000——M4 接线压 CARE-M4/BF-M8 注释净零,capstone 零装配层触碰)。生产建议:开图书馆员同开 `GOTONG_BUTLER_MEMORY_GIT`
  当 undo 网。见 [`docs/zh/ATONG-LIBRARIAN.md`](docs/zh/ATONG-LIBRARIAN.md)。
- **LSA-M5 补刀（WSE）：web 搜索成为阿同「key 即用」能力（2026-07-16，本 commit）** — 用户问「工具包里有没有
  web-search?没有就整合成一类能力,给它加上 key 自己就能使用那种」。侦察答案=LSA-M2 只接了目录,「自己就能搜」还剩
  两断点,一刀收口(新 `host/src/butler-web-search.ts` + factory/pool/main.ts 三缝):**断点1 分级(真阻断点)**——
  `gh api` 逐字核两家官方 server 源码证实**都没标 readOnlyHint**(Tavily 零 annotations/Brave 只有 title+
  openWorldHint),工具名 `tavily_search` 又不以 read 动词开头 → defaultMcpToolClass fail-safe **全落 governed=
  每搜一次 park 一次**;修=`classifyButlerMcpTool` server 级只读兜底(搜索 server 工具面全是「读外部世界」不存在
  「写用户数据」对象),优先级钉死 **server 显式 annotations > read 动词启发 > 搜索名单 > fail-safe write**(官方
  未来真标 destructiveHint 照 govern),面板手挂路径同时修好。**断点2 装配**——env 快路:`TAVILY_API_KEY`/
  `BRAVE_API_KEY` 在 ⇒ detect 取**目录同一条 spec**(原样引用零复刻,key 只以 `${NAME}` 占位存在)⇒ pool 新构造项
  `butlerBonusMcpSpecs` 只对**管家行** spawn 时并入,**同名让位**(用户自己配的 server 永远赢);挂上走 S1-M2 MCP
  read 半边进 benign 面,B1 能力清单自动长。env 名不带 GOTONG_ 是被钉死的(探测名必须=spec 占位名 envSecretSource
  才解析得到;生态惯例名一份 key 面板/env 两路通用;MEM0_API_KEY 先例=连接器凭证非旋钮);授权论证=面板路径 key 进
  vault 不进 env,env 放 key 的唯一动机就是「让 hub 的 AI 能搜」=最粗粒度 opt-in,未设=[]=字节不变;搜索词离盒是
  目录 dataLeavesBox 早披露的既定事实,接入≠授权(拿结果对外发仍 governed)。**顺手抓真缺口**:`@gotong/web` dist
  陈旧(LSA-M2 两条连接器只在 src 从没 build,host 是第一个消费方),已重 build+新「目录同源」防腐测试兼任 dist
  新鲜度哨兵。验收:新单测 16(目录同源 2/detect 5 含 key 明文防腐/classify 3 含显式 write 赢名单/merge 3/pool
  注入 3 死命令零网络),host 2183→**2199** 全绿,tsc 零错,四门 PASS(**旋钮 114 零新增**,main.ts 3000/3000 压注释
  净零)。见 [`docs/zh/LLM-STEWARDSHIP.md`](docs/zh/LLM-STEWARDSHIP.md) M5 节。
- **阿同感知力 track（SEN）全完：M0 计划 + M1→M5 五刀，八缺口收六迟二（2026-07-16，`a678f5a`→本 commit 共 7 commit）** —
  用户问「怎么能增加 atong 的感知力,包括对自己和对它所在的 hub」。**M0 侦察先行**(`a678f5a`,纯 docs
  `docs/zh/ATONG-PERCEPTION.md`):三管道盘点(每轮尾卡 composeContextProbes volatile 段/按需 benign 工具两层/零 LLM
  阈值推送)结论=**分频格局已对,缺的是器官不是管道**;八缺口逐条 file:line 举证。**五条不可破边界**:①热路径零 LLM
  (红黄判定只认 `derivePatrolCards` 一个权威,巡检/面板/工具/探针四处永不各说各话)②缓存前缀稳定(尾卡走 volatile 段,
  **无信号=null=prompt 字节不变**)③披露结构性 ⊆ 既有面(越面必须 role 分级或摆岔口给用户)④知识≠授权(感知全 benign
  只读,修复仍走 governed 闸)⑤内核零改动零新旋钮(**114 全程冻结**,main.ts 3000 预算靠压历史注释净零)。**M1 hub 红灯
  感知**(`bd8a13f`)一刀双缺口:尾卡探针纯读巡检落盘的 `patrol-state.json`(mtime 超 30min=巡检不在班→null 诚实未知,
  绝不拿死巡检的旧牌面当现状)+ 目录 benign `hub_health`(骑 `deps.onboarding.health` 惰性面=main.ts 零改动;断供走面板
  CARE-M7 无阈值姿态;披露 ⊆ 巡检既有成员推送面=零新披露)。**M2 peers 信任补账**(`9dffb77`):ButlerPeerRow 补 trustTier
  转述(null 如实「未分级(按 T1 对待)」绝不发明档位;PIN 是事实后缀不是档位)+ **pinnedKid 折布尔**(43 字符指纹结构性
  不进投影);ask_peer 仍只认 outboundCaps=信任档只是转述不改授权。**M3 my_status**(`405a95e`)「对自己」主菜:六块既有
  投影(大脑候选链/断供/累计用量/记忆计数/开放任务/备份事实)折一次只读自检,**固定六行逐行降级绝不跳行**(缺席=
  「(未接)」,honest-unknown 也是状态),用量行首钉「累计」绝不冒充「今日」(账本聚合无日界),记忆窄切片只声明计数字段=
  条目内容结构性拿不到。**M4 list_schedules**(`bfafd96`):侦察坐实零披露岔口——`WorkflowScheduleDef.userId` 就是 sweeper
  派发走的归属,看自己名下的行=零新披露;包装 admin `list()` 同源不自读 intent 文件,main.ts 把 web 内联 surface 提前
  构造成**同一实例**;渲染按真实编码(daily/weekly 的 fireMark 就是本地日期直印、interval 折 UTC 分钟,解析不动如实
  「(记录无法解析)」绝不印假日期)。**M5 list_members**(`c0765e1`)track 唯一披露岔口:侦察证实 /me 零名单 API+家庭
  审批 assignee 靠场外手打,三选项摆给用户**拍板 A=全员见「名+角色+id」**;投影带 id 是侦察改的设计(`newId()` 随机串
  才是 workflow assignee 真正要填的,尾行钉「填 id 那串不是名字」);**email 结构性红线**(窄切片只声明 id/displayName,
  join 挑列,哨兵 email 进 fixture 断渲染无 @);membership 缺失「(角色未知)」绝不猜默认。**⑤ recent_hub_events + ⑧
  行动史显式推迟**:共享同一道未决披露岔口(自己相关 vs hub 级)+transcript 有界投影要先定三个尺度(多近/多少条/哪些
  类别),状态类感知已全收口,事件回溯类等真实高频问法出现再重启,不为完备而完备。验收:五刀新/扩单测 17·8·13·11·12,
  host 全套 2134→**2183** 全绿,每刀四门 PASS(**旋钮 114 全程零新增**,main.ts 3000/3000——M1-M3 零触碰、M4/M5 压注释
  净零);每个新 builder 过 AFR 注册三件套(tiers 名单/tripwire/防腐门),M4 时名单双向核对门在 fixture 补 stub 前如期
  变红一次证门活着。见 [`docs/zh/ATONG-PERCEPTION.md`](docs/zh/ATONG-PERCEPTION.md)。
- **阿同框架及恢复能力提升 track（AFR）全完：M0 计划 + 腿A 目录化 M1→M3 + 腿B 向导 M4→M5 + 腿C 恢复 M6→M8
  （2026-07-15/16，`96aad86`→本 commit 共 9 commit）** — 用户三诉求:①「工具模块整合好以目录方式留给它调用,降低
  system prompt 认知负担」②「随身向导+医生进预置知识,但不要做成每次都调用的认知负担」③「不建立任何中心化节点及
  恢复方式,由用户自行选择备份……档位可调整」。**M0 计划**先辩死中央身份锚(用户初问「邮箱为准的中央归属」,辩论
  结论=效率真高但撞北极星「零中央节点」,裁决不做并把「已裁决不重开」钉进文档:中央锚·邮箱恢复·M-of-N 社交恢复)。
  **五条不可破边界**:①热路径零 LLM(目录渲染/分诊/面包屑全静态)②能力零阉割+闸零绕过(governed 恒一等,目录只收
  benign 长尾)③缓存前缀稳定(目录内容不进 system)④零中央节点+授权动作人点头 ⑤内核零改动零新旋钮(**114 全程冻结**)。
  **腿A 工具面目录化**:M1(`96aad86`)度量先行——`pnpm report:atong-toolface` 基线 35 工具 ~6038tk + **tripwire**
  (工厂 builder 调用点正则扫源 ≡ 度量注册表,加 toolset 不登记就红,M7 新增两 builder 时如期变红证门活着);M2
  (`51a73cb`)`packages/llm` 纯核 `TwoTierToolset`(`list_tool_directory` 渲染全目录 + `use_tool` 查表转发,未知名
  如实拒并指路目录);M3(`aa39b43`)factory 接线**默认发**+四道防腐门(名单双向核对/「指路不指空」=一等 schema 不得
  点名目录工具/能力不减端到端=目录里工具经 use_tool 真执行真落盘/B1 能力清单两层单层**逐字节一致**),账本:单层
  35 工具~6506tk→两层 30 工具~4947tk(**省 24%**),`singleTierToolFace` 逃生阀保留。**腿B 随身向导+医生**:M4
  (`e10f6fc`)`gotong_guide(topic)` 一个 benign 工具按需取 9 张策展知识卡(建 agent/建流/接 IM/审批/诊断/备份…;
  每张 ≤500 token 承重门+反向扫描门,卡是策展物不搬运 docs/zh 原文;医生=诊断+指路,动手仍走既有闸);M5(`0235d58`)
  面包屑接线——三处零 LLM 播报(运行失败/断供/升级卡)尾附静态 topic 指针,**失败分支才附**(done 没有修法可指),
  断供播报诚实标「到时」(断供期大脑拉不了卡),防腐门断言问句=真实卡标题+永不含原始工具名。**腿C 零中央节点分档
  恢复**:M6(上一 commit)`gotong backup --tier=identity|relations` **白名单式过滤**(identity=签名钥+名片+space.json
  三件,金库密文/主钥/会话**结构性**进不来,新文件默认落保守侧;relations=+peers **非密投影**,`SELECT` 挑列不是滤列
  [vault_entry_id 连字段名都不出现],诚实边界印进**档案本体**「令牌在金库,恢复的是认识谁不是连得上,重连需对端
  re-mint」;与 --include-master-key 组合当场拒;读取阶梯 better-sqlite3→sqlite3 CLI→exit 3 响亮失败绝不静默降档);
  M7(`a78fe42`)阿同恢复层——「上次备份」事实文件(cli backup 成功先归档后写 `runtime/last-backup.json`,谁打的都
  刷新)+ benign `backup_status`(目录层,只数 peer 增量,agent 无 createdAt **不造基线**)+ governed `pack_backup`
  (只收子集档,**owner/admin 两端闸**:classify 在 park 前服务端拒不够格=绝不浪费审批,execute 批准后再查=降权批准
  救不回;身份档含 hub 签名钥=凭证级动作)+ 陈旧提醒 sweeper(TN-M2 镜像:6h 节律纯时间戳分诊,14d 陈旧线+每人 14d
  冷却,从没备份过=首 tick 即 due;同意面=巡检先例「开了播报才收」**再叠 privileged 过滤**,送达才记标记;文案带
  「打一份身份档备份」短语+M5 面包屑,永不出现原始工具名);host 直接 import cli 真 `backup()`(workspace 依赖早在,
  零 shell-out 零复刻)。M8(本 commit)capstone `examples/atong-recovery` 四幕 15 断言零 LLM 零 key:真 ES256 钥
  (kid=RFC 7638)+真金库(令牌真加密)造家→三档真打包→真 restore 进三个新目录→**幕1** kid 独立复算不变+全字节扫
  4 哨兵串全空(身份档结构性无密)/**幕2** 投影行还在(endpoint/pinned_kid/trust_tier)+诚实 note 在档案本体/**幕3**
  搬家档新家用恢复出的主钥**真开金库**解出令牌明文(boot 级证明)/**幕4** M7 事实闭环;`pnpm demo:atong-recovery`
  exit 0。**capstone 抓到 M7 真缺口并修**:首次备份「先归档后写」天然不进档,但**二次**全量备份会把上轮事实带进
  档案(恢复进新家抱旧台账装新鲜,压掉「新家该打一份」提醒)→修=事实文件与 backups/ 同罪 staging 全模式排除+cli
  防回归。验收:cli 282/host 2134/llm 245 全绿,demo exit 0,四门 PASS(**旋钮 114 全程零新增**,main.ts 2999/3000
  ——M7 接线 +15 行靠压三段注释腾出)。见 [`docs/zh/ATONG-FRAMEWORK-RECOVERY.md`](docs/zh/ATONG-FRAMEWORK-RECOVERY.md)。
- **管家 LLM 自省与多模型自治 track（LSA）全完：M0 计划 + M1 自省 + M2 web 搜索 + M3 发现引导 + M4 并行综合封顶
  ...
  **M3 发现引导**(`4b90135`,③④重设计落地;2026-07-14 补 Gemini 成第 6 家)benign 只读 `discover_llm_providers`:
  渲染**静态策展目录**——六家 provider(OpenRouter/Groq/Cerebras/Gemini 免费额度 + Together 试用 + DeepSeek 低价),
  每条含 OpenAI 兼容 base URL/免费额度真相/注册链接/拿 key 三步/环境变量名(2026-07-14 逐一核官方文档——成员照 base
  URL 配 agent 写错即害人,宁少列也核准;**Gemini 首发因官方 `docs/openai` URL WebFetch 连报错推迟,后改走 WebSearch
  交叉核准补入**——该 URL 已列进 CLAUDE.md §4.1 禁止 WebFetch;Gemini 的 costTruth 如实写「免费档输入/输出可能被
  Google 用于改进产品」的隐私真相);**角色分对**——`ButlerLlmProviderOption` 结构上无 key 字段,渲染卡
...
| 模型路由（MR 全完：M1 RoutingProvider 纯核=有序候选+首-chunk-前 failover+per-candidate 三态熔断 + M2 opt-in `fallbacks` 配置面+providerFactory 装配缝 + M3 per-provider **被动**健康 in-memory 投影上面板黄条 + M4 capstone + M5 **手动测试路由**=逐候选主动探针[面板按钮→真 spawn 链发最小 completion→复用 describeKeyTest 逐候选渲染]补被动健康的洞 + M6 **per-candidate env 凭证**=`apiKeyEnv` 存 env 名非 key 值、排他语义缺失响亮失败绝不借存量链、七调用点含 M5/CARE 两探针同源[首个真实用例=生产 LongCat 主+MiMo 兜底];四边界=热路径零 LLM 走确定性路由非内容感知/opt-in 字节不变/数据离盒 opt-in/内核零改动） | `docs/zh/MODEL-ROUTING.md` |
| 模型编排策略研究（什么时候多模型齐发 ensemble / 什么时候轻量便宜 / 什么时候上重量级；核心结论=「用哪种」是**策略表 + 确定性信号**非现场 LLM 选路[撞边界①]；三模式 field 数据[MoA 开源集成 65.1%>GPT-4o 57.5% / FrugalGPT 省 98% / RouteLLM 2× 省保 95%]+ 各自代价；**cascade 只能做「结构性失败即升」=routing failover 已覆盖,「质量不够即升」做不了**；喂给 LSA-M4 推迟的 ensemble 配置面设计） | `docs/zh/MODEL-ORCHESTRATION-STRATEGY.md` |
| 阿同框架及恢复能力提升（AFR **全完 M0→M8**：三腿=A 工具面目录化[35 工具两层化:governed+高频一等、低频 benign 长尾进目录 `list_tool_directory`+`use_tool` 查表转发,默认发,省 24% schema token,四防腐门=名单双向核对/指路不指空/能力不减端到端/B1 逐字节一致]+ B 随身向导/医生[`gotong_guide(topic)` 按需取 9 张策展知识卡 ≤500tk,零 LLM 播报尾附静态面包屑,失败分支才附]+ C 零中央节点分档恢复[`--tier=identity\|relations` 白名单过滤金库主钥结构性进不来+peers 非密投影诚实边界印进档案本体;阿同层=backup_status 只读/pack_backup owner-admin 两端闸 park/陈旧提醒 sweeper 送达才记;capstone `atong-recovery` kid 复算不变+搬家档新家真开金库]；五边界=热路径零 LLM/能力零阉割闸零绕过/缓存前缀稳定/零中央节点+授权动作人点/内核零改动零新旋钮(114 全程)；已裁决不重开=中央身份锚·邮箱恢复·M-of-N 社交恢复） | `docs/zh/ATONG-FRAMEWORK-RECOVERY.md` |
| 阿同感知力（SEN **全完 M0→M5**：八缺口收六迟二——M1 hub 红灯[尾卡探针纯读巡检牌面 30min 新鲜度门+benign `hub_health` 同源体检卡,判定只认 derivePatrolCards 一个权威] + M2 peers 信任档转述[null 回落 T1 不发明档位,pinnedKid 折布尔指纹不进投影] + M3 `my_status` 六块折一[固定六行逐行降级不跳行,累计不冒充今日] + M4 `list_schedules`[admin list 同源同实例,归属即披露零岔口,mark 按真实编码渲染] + M5 `list_members`[岔口 A 用户拍板全员见名+角色+id,email 结构性红线,id 是 assignee 真正要填的]；⑤事件投影/⑧行动史显式推迟[共享披露岔口,按需重启]；五边界=判定单权威/尾卡无信号字节不变/披露⊆既有面或摆岔口/benign 只读/内核零改动 114 冻结） | `docs/zh/ATONG-PERCEPTION.md` |
