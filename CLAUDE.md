...
- **模型路由 track（MR）全完：M0→M4（2026-07-09，`c4a3c63`→本 commit 共 5 commit）** — 用户拍板「管家参考
  ...
  `@gotong/host/routing-health` 子路径导出(避 main.js 副作用);37 单测,main.ts 压注释净零守 3000。**M4**(本 commit)
  ...
  12 自断言 + `pnpm demo:model-routing` exit 0。验收:llm 16 / host routing 40 / web 173 全绿,四门 PASS(旋钮
  **仍 107 零新增**,main.ts 3000/3000)。显式推迟:软失败内容校验 / 语义缓存层 / hub-steward·workflow-assist 各自
  new provider 的单独接线 / 主动逐候选探活(按需再加)。见 [`docs/zh/MODEL-ROUTING.md`](docs/zh/MODEL-ROUTING.md)。
...
| 模型路由（MR 全完：M1 RoutingProvider 纯核=有序候选+首-chunk-前 failover+per-candidate 三态熔断 + M2 opt-in `fallbacks` 配置面+providerFactory 装配缝 + M3 per-provider 健康 in-memory 投影上面板黄条 + capstone；四边界=热路径零 LLM 走确定性路由非内容感知/opt-in 字节不变/数据离盒 opt-in/内核零改动） | `docs/zh/MODEL-ROUTING.md` |
