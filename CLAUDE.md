...
│       ├── acp-outbound.ts            ACP-OUT-M2 — AcpOutboundManager: 从 acp_outbound_agents 开机物化 + 运行时 refresh/remove 出站 AcpParticipant (构造不 spawn, 首派发懒起长 ACP session; 挂 dangerousToolGate fail-closed; onChunk 复用 llm_stream_chunk transcript 事件 → admin 打字机零 core 改); 镜像 a2a-outbound 但无密钥/无 readEnv (ACP 走底层 agent 自己登录态)
│       └── ...
