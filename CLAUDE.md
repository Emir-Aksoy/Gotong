...
| 阿同框架及恢复能力提升（AFR **M0 计划**：三腿=A 工具面目录化[30+ 工具两层化:核心+governed 一等、低频 benign 长尾进目录 `list_tool_directory`+`use_tool` 查表转发,承接 NA 推迟的「工具面瘦身先度量后动」,M1 先度量]+ B 随身向导/医生[`gotong_guide(topic)` 一个 benign 工具按需取策展知识卡,零 LLM 播报尾附静态 topic 面包屑]+ C 零中央节点分档恢复[身份/身份+关系/搬家三档打包用户自持+阿同陈旧提醒 sweeper+恢复演练 capstone；关系档诚实边界=令牌在金库,恢复「认识谁」非「连得上」]；五边界=热路径零 LLM/能力零阉割闸零绕过/缓存前缀稳定/零中央节点+授权动作人点/内核零改动零新旋钮；已裁决不重开=中央身份锚·邮箱恢复·M-of-N 社交恢复） | `docs/zh/ATONG-FRAMEWORK-RECOVERY.md` |
