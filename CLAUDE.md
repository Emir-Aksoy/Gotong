...
| **下一步** | — | v5 Stream D + Stream 0 完. v5 路线推荐顺序 D→0+A→B→C: 接下来 **Stream A** (A-M1 `resource_grants` 通用表 identity v16 [吃 0-M1 `principalKey`] → A-M2 agent 归属 + `/me` agent CRUD → A-M3 API 凭证归属 per-user/agent key → A-M4 授权 UI) → **B** (模板 manifest 导入导出 + 内容/人员导出权限闸) → **C** (联邦授权细化到 workflow 节点 I/O + 可调用知识库). 锁定决策: v5 版本号 / 心跳复用 suspended_tasks / agent-owner 受限子集 / 单 `resource_grants` 表 / 导出结构明文·内容加密·人员默认省略. 按 /goal 全按推荐逐里程碑执行中 |
