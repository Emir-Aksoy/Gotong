...
- **接入现实生活 track（C）：C-M1 现实生活连接器目录首批（2026-07-07，`6cd0a17`）** — 用户在
  ...
  **做宽 + 做可信**,抬「深度辅助」天花板。开工前按「先查市面」核官方 MCP 注册站,抓到**改路线的
  ...
  8+1+10 单测 + host 1911 全绿)均已落,**opt-in 全程零新 env 旋钮仍 106**,M5(连接器目录+「用 X 登录」UI+
  capstone)收尾。见 [`docs/zh/REAL-LIFE-CONNECTORS.md`](docs/zh/REAL-LIFE-CONNECTORS.md)。
...
| 接入现实生活 track（C：把连接器目录伸向日历/邮件/消息/笔记/任务；C-M1 静态 token 首批 Notion/Todoist，C-M2 出站 OAuth 主菜；三边界=全走 MCP 不存数据、凭证纪律、接入≠授权行动） | `docs/zh/REAL-LIFE-CONNECTORS.md` |
