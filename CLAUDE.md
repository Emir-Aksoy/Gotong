...
- **WIZ 六段建流向导（WIZ-M1→M5 全完）** — 用户逐字诉求「先确定任务（可跳过）→ 查已有
  ...
  **「用了预置模板才有的能力」是缺口不是错误**→④出三补法由人批准）。三入口：admin/成员
  ...
  green=2/needs_user=2/error=0）。详见 [`docs/zh/WORKFLOW-WIZARD.md`](docs/zh/WORKFLOW-WIZARD.md)。
更早里程碑（BE 管家增强 M1→M6（眼睛三只读 + 诊断闭环 + 零 LLM 运行播报）见
...
- **GitHub 已公开 + push 已解冻 (repo 2026-06-28 转 PUBLIC)**: 仓库 `Emir-Aksoy/Gotong` 已公开, push 解冻。推送纪律: **只推 `main`**, fast-forward only, **绝不强推**; 推前 `git fetch` 校验 `git merge-base --is-ancestor origin/main main`。远端有 dependabot 分支 + PR, 不动它们。Actions 仍仓库级禁用 (公开后重新启用免费)。具体哪次该不该 push 仍按用户指令, 不擅自 push。
