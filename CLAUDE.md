...
- **LIFE-L1 定时工作流（M1→M3 全完，默认开零旋钮）** — 用户诉求「在工作和生活中产生 agent
  ...
  **零新代码**。前置小抽取 `armButlerSweeps`（`5e7ba51`）腾 main.ts 行数预算。**M3**（`5e159ee`）
  ...
  不可豁免，写 mark 防当日双跑；failure ladder 404/409/500）。详见
  [`docs/zh/WORKFLOW-SCHEDULES.md`](docs/zh/WORKFLOW-SCHEDULES.md)。LIFE-L2①（「我的晨报」
  bundle 一键装）/ L4（生产实例 dogfooding，用户操作项）在途；OC-0 OpenClaw 对接侦察等 LIFE 出货。
