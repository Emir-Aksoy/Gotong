...
- **STD 标准对齐 track：名片签名 M0→M2b-1（2026-07-07，`c6ceab4`→`8927f05` 共 5 commit）**
  ...
  **M0**（`c6ceab4`）计划 [`docs/zh/STD-STANDARDS-ALIGNMENT.md`](docs/zh/STD-STANDARDS-ALIGNMENT.md)
  ...
  四门 PASS（旋钮仍 106）。**M2b-2 下一步**:identity `peers` 表加可空 `pinned_kid` 列（additive
  迁移）+ owner 显式填 + server 对「已登记且带 kid」的 peer 复验 + web admin 联邦面板显示 匹配/不符
  ——**advisory 警告绝不硬挡联邦**（最终裁决仍 token 握手，没 PIN 的 peer 零影响）。
