...
| **下一步** | (Phase 13 完整完成) | 没有强制路线 — 可以做 P3 第 2 批 (admin.js 的 managed-agents / bundle import / 转 ES module) / RAG MCP server 推荐 / 微信小程序 |
...
| 优先 | 任务 | 工作量 |
|---|---|---|
| 短期 | Phase 13 follow-up: templates/workflows few-shot 注入 (assistant 当前 `examples` 数组为空, 读 6 个内置模板 + 头部 `assistant_hint` 注入让 LLM 生成更准) | 半天 |
| 短期 | Phase 13 follow-up: admin UI 接 Phase 8 streaming 让 LLM 打字实时可见 (现在 await 完整 response, 30s+ 体验差) | 1 天 |
| ~~短期~~ | ~~2026-05-27 audit P3: 拆 `packages/web/src/server.ts` (3563 行) 的 route groups~~ | **2026-05-28 第一批已完成** — `workflow-routes.ts` 拆出 6 个 workflow 路由 (server.ts 3701→3578); 还可继续拆 services / agents / uploads / setup |
| ~~短期~~ | ~~2026-05-27 audit P3: 拆 `packages/web/static/admin.js`~~ | **2026-05-28 第一批已完成** — `admin-wf-assist.js` factory pattern 拆出 assist 对话框 (admin.js 3641→3363); 进一步拆需要 ES module + bundler 才划算 |
