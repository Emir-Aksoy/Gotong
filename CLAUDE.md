...
- **常驻管家 fold 进 IM 通道（BF-M1→M8 全完，默认开）** — 生产 `aipehub start` 注册的
  ...
  per-user 蒸馏/6h 维护。详见 [`docs/zh/PERSONAL-BUTLER-FOLD-IN-FINAL.md`](docs/zh/PERSONAL-BUTLER-FOLD-IN-FINAL.md)。
- **呈现/打包（只读 DAG 可视化 + 模板画廊一键装）** — admin 工作流「流程图」按钮
  （`projectWorkflowGraph` 纯投影 → 手写 SVG 分层 DAG，只读不碰「YAML 是治理/版本化的根」）
  + admin「模板画廊」10 个开箱模板卡片一键安装（复用 `POST /api/admin/templates/import`，
  构建期生成器嵌入 + catalog 路由过真 parseTemplate）。详见
  [`docs/zh/WORKFLOW-DAG-VIZ.md`](docs/zh/WORKFLOW-DAG-VIZ.md) · [`docs/zh/TEMPLATE-GALLERY.md`](docs/zh/TEMPLATE-GALLERY.md)。
...
### 缺口 1：hub / 跨 hub 心智未在入口体现

hub 是节点单位：一个人 + 自己的 agent = **主权 hub**；多 agent 也能组成**非主权 hub**（主权
在外部）；工作流既可 hub 内也可跨 hub 完成。但目前 UX 入口 / 文档仍按「个人 vs 组织」老框架
讲，没把真正的分界「**hub 内 vs 跨 hub**」摆到首屏。→ PRO track（`AIPE_PROFILE=hub|federation`
两档，团队 / 组织都归「跨 hub」）+ DOC track 收口。
...
### 缺口 3：管家作为「用户 ↔ 框架」中间层还缺眼睛

常驻管家（`aipehub start` 的 `chat` agent = per-user `ButlerRouter`）能**写**（建 / 改 / 删自己
的 agent、改工作流，全走 `/me` 收件箱审批）但几乎**看不见** run 状态 / agent 健康 / 用量，也不能
诊断→修复（RES 目前只在 admin 面板）、大白话建工作流、把一次性任务直接派给自己的 agent。管家
正是我们架好的「用户到框架」中间链接，让它更好地管理其他智能体和工作流 = 给它补眼睛（读投影，
全复用既有成员 surface），写永远走既有审批闸。→ BE track（管家增强五缝：观察面 / 诊断闭环 /
`create_workflow` / `ask_my_agent` / 运行结果主动播报）收口。
...
| 主题 | 读哪 |
|---|---|
| v4 整体架构 + Phase 路线 · 跨 org federation 模型 | `docs/zh/V4-ARCH.md` · `docs/zh/V4-PHASE4.md` · `docs/zh/V4-PHASE5-FINAL.md` |
...
| 控制面历史趋势 + 告警阈值 + 跨 hub 聚合 | `docs/zh/V5-F-FINAL.md` |
| 跨 hub 工作流编排（北极星 第 2 层） · 两机操作员 runbook | `docs/zh/V5-G-FINAL.md` · `docs/zh/FEDERATION-RUNBOOK.md` |
| A2A 外部 agent 当工作流步（+ 会挂起的 H2） | `docs/zh/V5-H-FINAL.md` |
| 成员大白话改工作流（OpenClaw 式）+ 跨 hub 出入口锁 | `docs/zh/V5-WFEDIT-FINAL.md` |
| 成员大白话管理 hub 设置（管家 Stream SW） | `docs/zh/V5-STEWARD-FINAL.md` |
| 常驻个人管家（记忆 + 治理 tool-loop + fold 进 host；建之前设计见 DESIGN） | `docs/zh/PERSONAL-BUTLER-FOLD-IN-FINAL.md` · `docs/zh/PERSONAL-BUTLER-FINAL.md` · `docs/zh/PERSONAL-BUTLER-DESIGN.md` |
| 管家记忆增强（多级 / 重要性 / 召回索引 / dreaming / 技能 / 6h 维护） | `docs/zh/MEMORY-TIERS-FINAL.md` · `docs/zh/MEMORY-ADVANCED-FINAL.md` · `docs/zh/MEMORY-DREAMING-SKILLS-FINAL.md` |
...
| 企业 SSO（OIDC · SAML） · 联邦 peer onboarding | `docs/zh/V6-ROUTE-B-P1-M4-OIDC.md` · `M5-SAML.md` · `M7-PEER-ONBOARDING.md` |
...
| 出站 CLI shell-out adapter · 出站 ACP 长连接 adapter | `docs/zh/V5-E2-CLI-ADAPTER.md` · `docs/zh/V5-ACP-ADAPTER.md` |
| Services 插件 RFC 系列 | `docs/services-rfc.md` 及 `*-rfc.md` |
| 完整审计报告 · 全量审计 2026-06-10 | `docs/zh/AUDIT-v4-phase5.md` · `docs/zh/AUDIT-2026-06-10-FULL.md` |
...
docs/  docs/zh/                 双语文档 (docs/zh/PROGRESS-LEDGER.md = 全部里程碑逐字账本)
