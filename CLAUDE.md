...
| 成员用大白话改工作流 (OpenClaw 式 `/me` 自然语言编辑) + 跨 hub 出入口锁 (入口=trigger / 出口=跨 hub egress 步 + dataClasses 逐字节不变, 纯本地只锁 trigger=最小锁; 纯函数 `enforceEditBoundary` 复用 `crossHubStepsOf` 检测器防漂移; 编辑=同工作流出新修订非 fork; 真栈 E2E 证「重指向跨 hub 出口→boundary_locked 且零持久化」) | `docs/zh/V5-WFEDIT-FINAL.md` |
