...
- **DEPLOY 部署简易性（A→C 全完）** — 用户诉求「部署越简单越好 + 设置面板」。**A**（`12c47cd`）
  ...
  崩溃循环，静态审计发现不了）——dev compose 改 loopback-only 发布 + `AIPE_ALLOW_INSECURE=1`；
  ...
  同步进向导（稳态引导零额外 RTT；`AIPE_BOOTSTRAP` 是占位符非旋钮，按 `AIPE_ROLE` 先例登记）；
...
- **EXT track（EXT-M1→M2 全完）** ✅ — 扩展面 + 例子从「一堆」变「梯子」：
  ...
  `AgentParticipant` 基类两写法，每个片段的 import 符号对着 `@aipehub/core` 实导出核过）+
...
- **GUARD track（GUARD-M1→M2 全完，默认全绿）** ✅ — 防再膨胀护栏，四道会红的承重门
  ...
  workflow 零 LLM / web∌host / kernel↛装配层）+ `AIPE_*` 旋钮注册表（`scripts/env-registry-gate.mjs`
  + `scripts/aipe-env-registry.txt`，核出真实 **103** 个，加一个不登记就红）+ 装配层行数预算棘轮
