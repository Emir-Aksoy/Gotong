...
| **下一步** | — | 没有强制路线 — 备选: Sprint 4 后续 (Prometheus 业务指标 / per-user token 预算 UI / 成本软阈值告警, 见 PHASE17 第十三节) / 草稿列表 UI (list() 只返 live) / inbox 超时升级 + 多人审批 (backlog #21) / admin.js 深拆 / 微信小程序 |
...
├── identity/                   v4 — users/credentials/sessions/vault/quota/peers/im_bindings/suspended_tasks; Phase 17: usage_ledger (v=11) + ledger-store.ts (逐条账本) + quota-store.ts recordUsage (ungated 记账)
