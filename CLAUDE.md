...
| **Phase 13 M2+** | **下一步** | HTTP route `POST /api/workflows/validate` + admin UI 编辑器接入 + few-shot examples + real-LLM end-to-end demo |
...
| AI 辅助 workflow 编辑 | ✓ Phase 13 M1 完 (WorkflowAssistantAgent + draftStatus + parseWorkflow self-validate); M2+ 接 HTTP route + admin UI | — |
...
| 优先 | 任务 | 工作量 |
|---|---|---|
| 现在 | Phase 13 M2: `POST /api/workflows/validate` HTTP route (forward draftStatus/validationError) | 半天 |
| 现在 | Phase 13 M3: admin UI workflow 编辑器 — 对话框 + YAML 预览 + draftStatus chip | 1-2 天 |
| 短期 | Phase 13 M4: templates/workflows 注入 few-shot examples | 半天 |
| 短期 | Phase 13 M5: real-LLM end-to-end demo example (Anthropic / DeepSeek 跑通) | 半天 |
| 短期 | 2026-05-27 audit P3: 拆 `packages/web/src/server.ts` (3563 行) 的 route groups, 沿用 `identity-routes.ts` / `me-routes.ts` 模式 | 2-3 天 |
| 短期 | 2026-05-27 audit P3: 拆 `packages/web/static/admin.js` (3265 行) 按 feature 成多个 static script | 1-2 天 |
