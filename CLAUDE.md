...
- **REN 全仓改名 AipeHub → Gotong（2026-07-04 一次收口，`5645b9a`）** — 用户拍板传播友好定名。
  ...
  web 1231）+ 四门全绿 + TTFR 3.2s 首结果。**GitHub 仓库改名 + push + PyPI/npm/域名注册 =
  用户动作**（顺序：先 GitHub 改名再推，旧 URL 自动 redirect）；既有生产机迁移三件事：env
  前缀 `AIPE_*`→`GOTONG_*` / systemd 服务名 / 状态目录名（新部署零迁移）。
...
- **DEPLOY 部署简易性（A→C 全完）** — 用户诉求「部署越简单越好 + 设置面板」。**A**（`12c47cd`）
  ...
  浮出 + **部署 TTFR 分钟账**入 [`docs/zh/GO-LIVE.md`](docs/zh/GO-LIVE.md) §T2/T3.1a/b（裸
  ...
  待用户拍板。
更早里程碑（WIZ 六段建流向导 M1→M5 见 [`docs/zh/WORKFLOW-WIZARD.md`](docs/zh/WORKFLOW-WIZARD.md)
...
- **GitHub 已公开 + push 已解冻 (repo 2026-06-28 转 PUBLIC)**: 仓库 `Emir-Aksoy/Gotong` 已公开, push 解冻。（2026-07-04 全仓改名后 GitHub 侧仓库名尚为 AipeHub——**改名动作待用户在 GitHub 执行**, 改完旧 URL 自动 redirect, 再推送。）推送纪律: **只推 `main`**, fast-forward only, **绝不强推**; 推前 `git fetch` 校验 `git merge-base --is-ancestor origin/main main`。远端有 dependabot 分支 + PR, 不动它们。Actions 仍仓库级禁用 (公开后重新启用免费)。具体哪次该不该 push 仍按用户指令, 不擅自 push。
