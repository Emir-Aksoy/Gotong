...
- **PUB·KIT·CARE 发布/自运维/管家可靠 + FDE-M3（2026-07-05/06 一轮全完，`e015808`→`2c121a5` 共 13 commit）**
  — 用户指令「继续，都完成再发布」：计划 [`docs/zh/PUB-KIT-CARE-PLAN.md`](docs/zh/PUB-KIT-CARE-PLAN.md)
  全部里程碑串行收口，真发布只剩用户动作。**PUB**：M1（`f1526ce`）18 包翻公开=36 包全就绪 +
  unscoped `gotong` 薄壳（bin 转发 @gotong/cli、依赖闭包拉 host，`npx gotong start` 故事）+
  `check:publish` 会红门（元数据/repository.directory/prepack/bin 实存/内部闭包完整/examples
  全 private 保险丝）；M2（`05e06a8`）verdaccio 彩排门 `check:npx-smoke`（本地 registry 真发→
  真装→`npx -y gotong start` 起真 host 轮询 /healthz——漏包/断 bin/缺 dist 在彩排现形不在真 npm）
  + `check:pypi-pack`（build + twine check 不真传）；M3 我方收尾（`2c121a5`）发布 runbook
  [`docs/zh/PUBLISH-RUNBOOK.md`](docs/zh/PUBLISH-RUNBOOK.md)（前门全绿才发 / `pnpm -r publish`
  拓扑序自动 / OTP 两条路 / **回滚=deprecate 绝不 unpublish** / 发后 npx+pip 核验）+ npx 首选
  路径进 README（30 秒三选一升 A、重写过时 descoped 块）/QUICKSTART（梯前直达框，主阶梯不动——
  TTFR 门硬编码命令不解析文档，已核）/GO-LIVE T1.2。**CARE**：M1（`00f9400`）`classifyLlmError`
  纯函数 + 双语大白话翻译表；M2（`0bd1d2f`）断供不失联——IM canned 回复 + llm-outage 边沿播报 +
  im-bridge-wiring 腾挪；M3（`5d48a8e`）主动巡检值班表，黄红牌边沿播报骑 BE-M5 同意面；M4
  （`bde47c8`）开箱陪跑 capstone——零 LLM 现状卡注入 + 只读 key 活体校验（前置 `6611192`
  butlerFactory 出走腾预算）。**KIT**：M1（`8f8dbcd`）`gotong backup/restore` TS 原生（语义
  逐字对齐 backup.sh：master key 两世代排除/会话排除/WAL 诚实阶梯 driver→sqlite3 CLI→原拷+大声；
  manifest 先验后落盘）；M2（`3016d54`）`gotong migrate scan/apply` 改名残留医生 + OPERATIONS
  搬家配方；M3（`d6717da`）`gotong update` 形态探测原地更新——BUNDLE-INFO 便携指路 / git
  ff-only 纪律同 cloud-quickstart（脏树只看 tracked、非 ff 不代裁决、dist.prev 构建红逐字节
  还原、check 红仅告警、重启命令只打印绝不代跑）/ rsync 形态诚实拒 / npm 委托 `npm i -g`；真
  git fixture（本地 bare remote）9 条验收。**FDE-M3**（`aa51c51`）：`schedules[]` 第三可选块
  （模板带结构不带人——顶层 userId 装时拒；cadence 纯核挪 `@gotong/workflow` 守 web∌host）+
  装后 suggestions store（姿态逐字 M1b：意图/last-install-wins/损坏当空）+ advisory GET 无面
  `{packs:[]}` 绝不 503 + admin 补人启用走同一 schedule upsert（成员闸 fire 时兜）+
  `gotong provision` 开荒一条命令（装模板→落流→建调度→跑验收，exit 0/1/2/3）+ cloud-quickstart
  `--pack` 打印续跑命令（admin token 首启才铸，物理上只能 print-only）；host E2E 5 场景首跑全绿。
...
- **FDE 前置部署 track（M0→M2 收口）** — 用户战略定调：框架要
  ...
  [`docs/zh/FORWARD-DEPLOY.md`](docs/zh/FORWARD-DEPLOY.md)：五段（发现/翻译→构建/对接→部署→
  ...
  **现算**永不过期；未挂一律**黄牌**——optional:false 也不红，红牌只留 host 亲证事实，第三方
  ...
  四门 PASS（main 3247/3250）。M3（按方案开荒一条命令）已于上条 PUB·KIT·CARE 轮收口（`aa51c51`）。
- **REN 全仓改名 AipeHub → Gotong（2026-07-04 一次收口，`5645b9a`）** — 用户拍板传播友好定名。
  ...
  2026-07-04 经用户授权完成（旧 URL 自动 redirect）；**PyPI/npm/域名注册仍待用户**；既有
  生产机迁移三件事：rsync 排除集 `.aipehub`→`.gotong` / env 前缀 sed / healthcheck grep
  变量名（新部署零迁移）。
更早里程碑（LIFE 定时工作流 L1 M1→M3 + L2① 晨报 bundle 全完（零 LLM 调度环 + 成员闸派发 +
...
│                               provision / update / backup / restore / migrate)
