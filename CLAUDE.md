...
- **STD 标准对齐 track：名片签名 M0→M2b-2（2026-07-07，`c6ceab4`→`8868229` 共 6 commit）**
  ...
  **M0**（`c6ceab4`）计划 [`docs/zh/STD-STANDARDS-ALIGNMENT.md`](docs/zh/STD-STANDARDS-ALIGNMENT.md)
  ...
  refreshPolicy）；identity 616/web 1276/host 1882 全绿，四门 PASS（旋钮仍 106）。**M2b-3 显式推迟**:
  面板内实时「匹配/不符」徽章需服务端取对端 HTTP 名片复验,但 peer 的 `endpointUrl` 是 wss mesh 地址、
  名片在另一端口/协议的 well-known,**无法从 wss 稳妥推导 card URL**——补它要么给 web 开新出站 fetch 面
  （含 SSRF）+ 定 card URL 来源（admin 粘贴/另存列），是真架构岔口按 4.4 留用户拍板;**验证能力其实
  已由 M2b-1 `peer-card --expect-kid` 交付**（面板显示 pin,owner 复制去 CLI 复验），不缺能力只缺面板
  内一键徽章。
